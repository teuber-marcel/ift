...
/* Create Arrays: */
iftJson *cJSON_CreateIntArray(  int *numbers, int count) {
    int i;
...
iftJson *cJSON_CreateFloatArray(  float *numbers, int count) {
    int i;
    iftJson *n = 0, *p = 0, *a = cJSON_CreateArray();
    for (i = 0; a && i < count; i++) {
        n = cJSON_CreateNumber(numbers[i]);
...
iftJson *cJSON_CreateDoubleArray(  double *numbers, int count) {
    int i;
    iftJson *n = 0, *p = 0, *a = cJSON_CreateArray();
    for (i = 0; a && i < count; i++) {
        n = cJSON_CreateNumber(numbers[i]);
 ...
 * @return       true/false
 */
static inline bool iftIsJInt(  iftJNode *jnode) {
 ...
 * @date Sep 17, 2017
 */
bool iftIsJIntArray(  iftJNode *jarray);
 ...
 * @param key The key to be used.
 */
void iftAddIntArrayToJson(iftJson *json, const char *key,   iftIntArray *iarr);
 ...
 * @return The created iftJNode.
 */
iftJNode *iftCreateJIntArray(  iftIntArray *iarr) {
...
void iftWriteJsonAux(  iftDict *dict, FILE *fp, int tab, int newline) {
    fprintf(fp, "{");
...
void iftWriteJson(  iftDict *dict, const char *format, ...) {
    va_list args;
    char filename[IFT_STR_DEFAULT_SIZE];
...
void iftWriteJsonMinified(  iftDict *dict, const char *format, ...) {
    va_list args;
    char filename[IFT_STR_DEFAULT_SIZE];
