...
SUPERVOXEL_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/RegistrationError/isf_on_reg_error_magnitude.py"
FEAT_EXTRACTION_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/RegistrationError/extract_svoxel_feats_dilation.py"
OC_CLASSIFICATION_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/RegistrationError/classify_svoxel_datasets_by_ocsvm.py"
...
def build_argparse():
    ...
    parser.add_argument('-r', '--dilation-radius', type=float, required=True, default=3.0,
                        help='Dilation Radius for the second classification. Default: 3.0')
    ...
    parser.add_argument('--skip-second-classification', action='store_true',
                        help='Skip second round of feat. extraction and classification.')
...
def print_args(args):
    ...
    print('--------------------------------------------')
    print('### SECOND SUPERVOXEL FEAT EXTRACTION AND CLASSIFICATION')
...
def main():
    ...
    if not args.skip_supervoxel_segmentation:
        ...
        svoxel_cmd_args = f"--image {args.image} --template {args.template} " \
        ...
        if args.bias:
            svoxel_cmd_args = f"{svoxel_cmd_args} --bias {args.bias}"
    ...
    if not args.skip_feat_extraction:
        print("\n\n####### Feature Extraction")
        feat_extract_cmd_args = f"--test-image {args.image} --test-supervoxels {svoxels_path} " \
                                f"--template {args.template} --training-set {args.training_set} " \
                                f"--output-dir {datasets_dir} --bins {args.bins}"
        if args.bias:
            feat_extract_cmd_args = f"{feat_extract_cmd_args} --bias {args.bias}"
    ...
    if not args.skip_second_classification and args.dilation_radius > 0:
        print("\n\n####### Relabeling Supervoxels")

        clf_svoxels_path = os.path.join(working_dir, "clf_svoxels_first.nii.gz")
        shutil.move(args.out_classification_mask, clf_svoxels_path)

        datasets_dir = os.path.join(working_dir, "datasets_relabeled_svoxels")

        print("\n\n####### Feature Extraction")
        feat_extract_cmd_args = f"--test-image {args.image} --test-supervoxels {clf_svoxels_path} " \
                                f"--template {args.template} --training-set {args.training_set} " \
                                f"--output-dir {datasets_dir} --bins {args.bins} --dilation-radius {args.dilation_radius}"
        if args.bias:
            feat_extract_cmd_args = f"{feat_extract_cmd_args} --bias {args.bias}"
        ...
        print("\n\n####### One-Class SVM")
        ocsvm_cmd_args = f"--supervoxels {clf_svoxels_path} --supervoxels-datasets-entry {datasets_dir} " \
                        f"--output-anomalous-supervoxels-img {args.out_classification_mask} " \
                        f"--kernel {args.kernel} --nu {args.nu}"
        if args.kernel == "rbf":
            ocsvm_cmd_args = f"{ocsvm_cmd_args} -g {args.gamma}"
