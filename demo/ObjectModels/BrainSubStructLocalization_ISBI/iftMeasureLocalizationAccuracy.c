...
int main(int argc, const char *argv[]) {
    ...
    iftLabelBoundingBoxArray *lbbs = iftReadLabelBoundingBoxArray(mbb_json_path);
    iftDict *bb_labels_dict = iftIntArrayToDict(lbbs->labels->val, lbbs->labels->n);
    ...
    for (int o = 0; o < gt_labels->n; o++) {
        ...
        printf("* Object: %d\n", gt_labels->val[o]);
        puts("gt_bbs");
        iftPrintBoundingBox(gt_mbbs[o]);
        puts("bbs");
        iftPrintBoundingBox(lbbs->bbs[bb_obj_idx]);


        float score1 = iftDistanceBetweenMBBCenters(gt_mbbs[o], lbbs->bbs[bb_obj_idx], gt_img->dx);
        printf("Score 1 = %f mm\n", score1);

        float score2 = iftPercOfLostObjectVoxels(gt_img, lbbs->bbs[bb_obj_idx], gt_labels->val[o]);
        printf("Score 2 = %f\n", score2);

        float score3 = iftBgVoxelsError(gt_img, lbbs->bbs[bb_obj_idx], gt_labels->val[o]);
        printf("Score 3 = %f\n\n", score3);
    ...
    if (iftDictContainKey("--output-dir", args, NULL)) {
        ...
        for (int o = 0; o < gt_labels->n; o++) {
            int bb_obj_idx = iftGetLongValFromDict(gt_labels->val[o], bb_labels_dict);
            iftFillBoundingBoxInImage(bb_bin, lbbs->bbs[bb_obj_idx], 1);
        }
