...
namespace SelfReg{
  ...
  SelfRegDialog::SelfRegDialog(Scene *scnR, Scene *scnG, Scene *scnB)
  //      : wxDialog(APP->Window, wxID_ANY, _T("Verify the registration"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER, _T("SelfRegDialog")){ 
    : wxDialog(APP->Window, wxID_ANY, _T("Verify the registration"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER , _T("SelfRegDialog")){ 
    ...
    Connect( id_ori, wxEVT_COMMAND_CHOICE_SELECTED,
	     wxCommandEventHandler(SelfRegDialog::OnChangeSliceOrientation),
	     NULL, NULL );
    Connect( id_spin, wxEVT_COMMAND_SPINCTRL_UPDATED,
	     wxCommandEventHandler(SelfRegDialog::OnChangeSpinCtrl),
	     NULL, NULL );


  }

  SelfRegDialog::~SelfRegDialog(){
    delete chOri;
  }

  
  int SelfRegDialog::GetOrientation(){
    return chOri->GetSelection()+1;
  }

  int SelfRegDialog::GetSpinValue(){
    return spinSlice->GetValue();
  }

  
  void SelfRegDialog::OnChangeSliceOrientation(wxCommandEvent& event){
  ...
  void SelfRegDialog::OnChangeSpinCtrl(wxCommandEvent& event){
