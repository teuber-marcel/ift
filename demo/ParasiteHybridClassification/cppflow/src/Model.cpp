...
Model::Model(  std::string& model_filename,   std::vector<uint8_t>& config_options) {
    ...
    {
        TF_SetConfig(sess_opts, static_cast<  void*>(config_options.data()), config_options.size(), this->status);
...
void Model::save(  std::string &ckpt) {
    ...
    output_file.oper = TF_GraphOperationByName(this->graph, "save/ ");
    output_file.index = 0;
    TF_Output inputs[1] = {output_file};

    TF_Tensor* input_values[1] = {t};
      TF_Operation* restore_op[1] = {TF_GraphOperationByName(this->graph, "save/control_dependency")};
    if (!restore_op[0]) {
        TF_DeleteTensor(t);
...
void Model::restore(  std::string& ckpt) {
#ifdef TENSORFLOW_C_TF_TSTRING_H_
    std::unique_ptr<TF_TString, decltype(&TF_TString_Dealloc)> tstr(new TF_TString, &TF_TString_Dealloc);
    TF_TString_Copy(tstr.get(), ckpt.c_str(), ckpt.size());
    auto deallocator = [](void* data, size_t len, void* arg) {};
    ...
    TF_Tensor* input_values[1] = {t};
      TF_Operation* restore_op[1] = {TF_GraphOperationByName(this->graph, "save/restore_all")};
    if (!restore_op[0]) {
        TF_DeleteTensor(t);
...
TF_Buffer *Model::read(  std::string& filename) {
...
std::vector<std::string> Model::get_operations()   {
...
void Model::run(  std::vector<Tensor*>& inputs,   std::vector<Tensor*>& outputs) {

    this->error_check(std::all_of(inputs.begin(), inputs.end(), [](  Tensor* i){return i->flag == 1;}),
            "Error: Not all elements from the inputs are full");

    this->error_check(std::all_of(outputs.begin(), outputs.end(), [](  Tensor* o){return o->flag != -1;}),
    ...
    std::transform(inputs.begin(), inputs.end(), io.begin(), [](  Tensor* i) {return i->op;});

    // Get input values
    std::vector<TF_Tensor*> iv(inputs.size());
    std::transform(inputs.begin(), inputs.end(), iv.begin(), [](  Tensor* i) {return i->val;});

    // Get output operations
    std::vector<TF_Output> oo(outputs.size());
    std::transform(outputs.begin(), outputs.end(), oo.begin(), [](  Tensor* o) {return o->op;});
...
void Model::run(  std::vector<Tensor*> &inputs, Tensor &output) {
    this->run(inputs, &output);
}

void Model::run(Tensor &input,   std::vector<Tensor*> &outputs) {
...
bool Model::status_check(bool throw_exc)   {
...
void Model::error_check(bool condition,   std::string &error)   {
    if (!condition) {
