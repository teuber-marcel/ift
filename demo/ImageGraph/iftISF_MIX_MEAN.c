...
iftImage *iftMyExtract_ISF_MIX_MEAN_Superpixels(iftImage *img, int nsuperpixels, float alpha, float beta, int niters, int smooth_niters, int *nseeds, int *finalniters) {
...
int main(int argc, char *argv[]) 
{
  ...
  label     = iftMyExtract_ISF_MIX_MEAN_Superpixels(img, atoi(argv[2]), atof(argv[3]), atof(argv[4]), atoi(argv[5]), atoi(argv[6]), &(nseeds), &(nautoiters));
  ...
  if (!iftIs3DImage(img)){
    ...
    RGB.val[2] = 0;
    YCbCr      = iftRGBtoYCbCr(RGB, normvalue);
    sprintf(labelfilename, "%s.png", argv[7]);
    iftWriteImageByExt(label,labelfilename);    
    iftDrawBorders(img,border,A,YCbCr,A);
    sprintf(borderfilename, "%s_borders.png", argv[7]);        
    iftWriteImageByExt(img, borderfilename);
  } else {
    printf("%d\n",iftMaximumValue(label));
    sprintf(labelfilename, "%s.nii.gz", argv[7]);
    iftWriteImageByExt(label,labelfilename);
    ...
    sprintf(borderfilename, "%s_centers.nii.gz", argv[7]);
    iftWriteImageByExt(img,borderfilename);
  }
