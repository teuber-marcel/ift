...
//int   vet_nkernels  [4] = {32,64,96,128}; // 96
int   vet_pooling   [8] = {0,3,0,5,0,7,0,9};
int   vet_stride    [4] = {1,2,3,1};
float vet_alpha     [4] = {1.,2.,3.,10.};
int   vet_norm      [8] = {0,3,0,5,0,7,0,9};
...
//      continue;
    ...
    for(int l=0; l < nlayers; l++) {
      ...
      // V1-like
      convnet->nkernels[l]          = 96;
      if (convnet->k_bank[l] != NULL) iftDestroyMMKernel(&(convnet->k_bank[l])); 
      float freqs[] = {1./2,1./3,1./4,1./6,1./11,1./18};
      convnet->k_bank[l]            = iftV1likeMMKernel2D(convnet->k_bank_adj_param[l],convnet->k_bank_adj_param[l],1,16,6,freqs);
      // V1-like
    ...
    convnet->rescale         = 0;
    convnet->with_weights    = 1; // saving weights
