...
def validate_args(args):
    if not os.path.isdir(args.train_set_entry) and not args.train_set_entry.endswith('.csv'):
        sys.exit('Invalid SOURCE image entry: %s\nTry a CSV or a Directory' %
                 args.train_set_entry)
...
def train_autoencoder(Xsource, Xtarget, epochs=500, batch_size=1):
    ...
    x = Conv3D(16, kernel_size=3, activation='relu', padding='same')(input_img)
    ...
    encoded = Conv3D(256, kernel_size=3, activation='relu', padding='same')(x)

    x = UpSampling3D(size=2)(encoded)
    x = Conv3D(128, kernel_size=3, activation='relu', padding='same')(x)
    x = Conv3D(128, kernel_size=3, activation='relu', padding='same')(x)
...
def main():
    ...
    train_set_paths = ift.LoadFileSetFromDirOrCSV(
        args.train_set_entry, 0, True).GetPaths()
    train_set_paths = train_set_paths
    print(train_set_paths)

    X = util.read_image_list(train_set_paths)
    X = util.normalize_image_set(X)
    # the target network only reduces the data size by for cubic max pooling of 2
    X = util.adjust_image_domain(X, max_pooling_sizes=[2, 2, 2, 2])

    autoencoder = train_autoencoder(
        X, X, epochs=args.epochs, batch_size=args.batch_size)
