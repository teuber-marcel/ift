...
void iftGetInputArgs(  iftDict *args, char **test_img_path, iftFileSet **train_set, char **mask_path,
                        int *mov_obj, int *fix_obj, char **out_dir, iftFileSet **elastix_files, iftFileSet **df_symm_objs,
                        bool *use_stdev);
...
iftFileSet *iftExtractNativeTrainAsymMaps(  iftFileSet *train_set,   iftFileSet *elastix_files,
...
int main(int argc, const char *argv[]) {
    ...
    iftGetInputArgs(args, &test_img_path, &train_set, &mask_path, &mov_obj, &fix_obj, &out_dir, &elastix_files,
    ...
    iftFileSet *train_asym_maps = iftExtractNativeTrainAsymMaps(train_set, elastix_files, test_img, mask,
...
iftDict *iftGetArgs(int argc, const char *argv[]) {
    char program_description[2048] = \
        "- Map training images on to the test image NATIVE space, and then " \
    ...
    iftCmdLineOpt cmd_line_opts[] = {
        ...
        {.short_name = "-i", .long_name = "--train-set-entry", .has_arg=true, .arg_type=IFT_STR_TYPE,
         .required=true, .help="Dir. or CSV with the pathname from the Train. Images."},
...
iftFileSet *iftExtractNativeTrainAsymMaps(  iftFileSet *train_set,   iftFileSet *elastix_files,
    ...
    for (long i = 0; i < train_set->n; i++) {
        ...
        iftImage *timg_native = aux = iftRegisterImageByElastix(timg, test_img, NULL, NULL, elastix_files, NULL, NULL);
