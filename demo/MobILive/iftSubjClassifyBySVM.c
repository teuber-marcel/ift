...
int iftCompSampleX(  void * a,   void * b) {
  iftSampleX *first  = (iftSampleX*) a;
  iftSampleX *second = (iftSampleX*) b;
...
float iftEERThreshold(iftDataSet* Z) {

  int i, k, ntruefake=0, nfalsefake=0, nfalsereal=0, ntruereal=0;
  int n = Z->nsamples-Z->ntrainsamples;

  iftSampleX* sample = malloc(sizeof(iftSample)*n);
  float thres;

  // 1 - fake,  2 - real
  // Copying 
  for (i = 0, k = 0; i < Z->nsamples; i++){
    if (Z->sample[i].status != IFT_TRAIN){
      sample[k]->class  = Z->sample[i].class;
      sample[k]->label  = Z->sample[i].label;
      sample[k]->weight = Z->sample[i].weight;
      k++;
    }
  }
  free(sample);
...
#endif
    // Computing threshold for obtaining EER
    for(
	Z2[2]->
    if (acc[i] != 1.) {
