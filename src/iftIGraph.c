...
iftIGraph *iftMImageToIGraph(  iftMImage *img,   iftImage *mask);
...
iftIGraph *iftExplicitIGraph(  iftMImage *img,   iftImage *mask,   iftImage *label, iftAdjRel *A)
{
    iftIGraph *igraph = iftMImageToIGraph(img, mask);
    int        i, s, t, p, q;
    iftVoxel   u, v;
	...
	for (p=0; p < img->n; p++) {
      ...
      } else { /* This part IS LIKELY bugged since it is not taking
		  into account the mask, given igraph->index->val[]
		  when mask != null */
...
int iftIGraphISF_Mean(iftIGraph *igraph, iftImage *seeds, double alpha, double beta, int niters)
{
    ...
    for (it=0; (it < niters); it++) {
	  ...
	  frontier_nodes = iftIGraphTreeRemoval(igraph, &trees_for_removal, pvalue, IFT_INFINITY_DBL);
        ...
        while (new_seeds != NULL) { /* insert seeds in the priority
				       queue Q with cost zero */
	  s = iftRemoveSet(&new_seeds);
	  p = igraph->node[s].voxel;
	  if (igraph->label[p] != 0){ /* we must avoid removed seeds
					 from previous iteration */
        ...
        while (frontier_nodes != NULL) { /* insert frontier nodes in Q
...
iftIGraph *iftGenerateIGraphSuperpixelsByISF(  iftImage *img, int input_n_cells, float alpha,
...
iftImage *iftGenerateSuperpixelsByISF(  iftImage *img, int input_n_cells, float alpha, float beta,
...
void iftIGraphSetWeightForRegionSmoothing(iftIGraph *igraph,   iftImage *img)
{
    iftAdjRel *A;
...
void iftRenumerateBlockSuperpixels(iftImage *super_img,   iftImageTiles *blocks, int *out_n_cells) {
    if (super_img == NULL)
...
void iftWriteSuperpixelBorders(  iftImage *img,   iftImage *super_img, const char *out_path) {
    if (img == NULL)
