...
iftImage *iftExtract_ISF_MIX_MEAN_Superpixels(iftImage *img, int nsuperpixels, float alpha, float beta, int niters, int smooth_niters, int *nseeds, int *finalniters, float *proc_time) {
...
int main(int argc, char *argv[]) 
{
  ...
  for (i = 0; i < nimages; i++) {
    ...
    superpixels = iftExtract_ISF_MIX_MEAN_Superpixels(img, atoi(argv[2]), atof(argv[3]), atof(argv[4]), atoi(argv[5]), atoi(argv[6]), &(nseeds), &niters, &proc_time);
    //t2 = iftToc();
