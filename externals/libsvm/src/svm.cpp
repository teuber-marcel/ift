...
double svm_predict_values(const svm_model *model, const svm_node *x, double* dec_values, float *certainty)
{
	int i;
	   ...
	   model->param.svm_type == NU_SVR)
	{
	  ...
	  *dec_values = sum;
	...
	else
	  {
	    ...
	    float      **weights = Malloc(float*, nr_class);
	    for(i = 0; i<nr_class; i++){
	      weights[i] =  Malloc(float, nr_class);
	      ...
	      for(int j=i+1;j<nr_class;j++)
		{
		  ...
		  weights[i][j] = (float)(sum - model->rho[p]);
		  weights[j][i] = (float)-weights[i][j];
		  sum -= model->rho[p];
		  dec_values[p] = sum;
	    ...
	    int predicted_label = model->label[vote_max_idx];
		...
		second_vote = vote[1];
	      ...
	      for (i = 0; i< nr_class; i++){
		if (vote[i]>second_vote && i!=vote_max_idx){
		  second_vote = vote[i];
		  aux = i;
		}	
	      }
	      
	      if (weights[vote_max_idx][aux]< 0 )
		weights[vote_max_idx][aux] = 0;
	      *certainty = 1 - exp(-weights[vote_max_idx][aux]);
	    }
	    for (i = 0; i< nr_class; i++)
	      free(weights[i]);
	    free(weights);
...
double svm_predict(const svm_model *model, const svm_node *x, float *certainty)
{
	int nr_class = model->nr_class;
	double *dec_values;
	if(model->param.svm_type == ONE_CLASS ||
	...
	double pred_result = svm_predict_values(model, x, dec_values, certainty);
	free(dec_values);
	return pred_result;
}
