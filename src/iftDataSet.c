...
/********************** PRIVATE FUNCTIONS *************************/
void iftIsThereTrueLabelOrGroupZero(  iftDataSet *Z, const char *function_name) {
...
void iftDataSetSampling(iftDataSet *Z,   iftSampler *sampler, int iteration) {
...
iftSampler *iftStratifiedRandomSubsampling(  int *labels, int n_samples, int n_iters, int n_train) {
    if (labels == NULL)
...
iftSampler *iftBalancedRandomSubsampling(  int *labels, int n_samples, int n_iters, int n_train) {
    if (labels == NULL)
...
bool iftIsSupervisedDataSet(  iftDataSet *Z)
{
    if (Z->nclasses > 1)
        return(1);
    else
...
bool iftIsTrainingDataSet(  iftDataSet *Z)
{
...
bool iftIsTestingDataSet(  iftDataSet *Z)
{
...
bool iftIsNormalizedDataSet(  iftDataSet *Z)
{
...
bool iftIsCentralizedDataSet(  iftDataSet *Z)
{
    if (Z->fsp.mean != NULL)
        return(1);
    else
...
bool iftIsWhitenedDataSet(  iftDataSet *Z)
{
    if (Z->fsp.W != NULL)
        return(1);
    else
...
iftImage *iftDataSetObjectMap(  iftDataSet *Z,   iftImage *comp, int max_val, int label) {
    if (Z == NULL)
...
iftDataSet *iftSupervoxelsToDataSet(  iftImage *img,   iftImage *supervoxel,
                                      iftIntArray *true_labels) {
    iftDataSet *Z;
...
iftDataSet *iftSupervoxelsToHistogramDataSet(  iftImage *img,   iftImage *label, int nbins, iftIntArray *true_labels){
...
iftDataSet *iftSupervoxelsToLabOrGrayHistogramDataSet(  iftImage *img,   iftImage *label, int nbins, int *region_size){
...
typedef void (*iftExtractAdjacencyFeats)(  iftImage *img, int p,   iftAdjRel *A, iftSample *sample);

void iftExtractGrayAdjacencyFeats(  iftImage *img, int p,   iftAdjRel *A, iftSample *sample) {
...
void iftExtractColorAdjacencyFeats(  iftImage *img, int p,   iftAdjRel *A, iftSample *sample) {
    iftVoxel u = iftGetVoxelCoord(img, p);
...
iftDataSet *iftImageToDataSetByAdjacencyFeats(  iftImage *img,   iftImage *label_img,
                                                iftAdjRel *Ain) {
    iftAdjRel *A = NULL;
    if (Ain == NULL)
...
iftDataSet *iftImageROIToDataSetByAdjacencyFeats(  iftImage *img,   iftImage *label_img, iftBoundingBox bb,   iftAdjRel *Ain) {
    iftAdjRel *A = NULL;
    if (Ain == NULL)
        A = (iftIs3DImage(img)) ? iftSpheric(sqrtf(3.0)) : iftCircular(sqrtf(2.0));
    else A = iftCopyAdjacency(Ain);
...
iftMatrix* iftDataSetMeanFeatures(  iftDataSet* Z) {
...
iftMatrix* iftDataSetStdDevFeatures(  iftDataSet* Z) {
...
iftMatrix* iftMatrixMultTranspose(  iftMatrix* M) {
...
iftMatrix* iftTransposeMultMatrix(  iftMatrix* M) {
    iftMatrix* T = iftTransposeMatrix(M);
...
iftMatrix* iftLinearDiscriminantAnalysis(  iftDataSet *Z) {
...
iftDataSet *iftReadOPFDataSet(const char *pathname) {
    ...
    //////////////// READING DATASET HEADER AND SAMPLE
    //////////////// INFORMATION ///////////////////
    ...
    if (Z->ref_data_type == IFT_REF_DATA_IMAGE) {
        ...
        if (!iftFileExists(ref_data_path))
            iftError("There is no Reference Data FILE \"%s\" in Dataset",
                     "iftReadOPFDataset", ref_data_path);
...
void iftWriteOPFDataSet(  iftDataSet *Z, const char *pathname) {
  ...
  if (Z == NULL)
    ...
    //////////////// WRITING DATASET HEADER AND SAMPLE
    //////////////// INFORMATION ///////////////////
    char *info_path = iftJoinPathnames(2, tmp_dir, "info.data");
    ...
    if ((Z->fsp.mean != NULL)||(Z->fsp.w != NULL)||
	(Z->fsp.R != NULL)||(Z->fsp.W != NULL)){
      char *fsp_path = iftJoinPathnames(2, tmp_dir, "feat_space.fsp");
...
void iftSetRefData(iftDataSet *Z,   void *ref_data, iftRefDataType ref_data_type) {
    if (Z->ref_data != NULL) {
...
void iftCopyRefData(iftDataSet *Z,   void *ref_data, iftRefDataType ref_data_type) {
...
void iftWriteDataSet(  iftDataSet *dataset, const char *format, ...) {
    va_list args;
...
void iftWriteDatasetGivenVersion(  iftDataSet *dataSet, const char *pathname, iftDatasetVersion version){
...
bool iftCompareSamples(  iftSample* sample1,   iftSample* sample2, int nfeats) {
...
bool iftCompareDataSets(  iftDataSet* dataset1,   iftDataSet* dataset2, bool compareFeatures, bool verbose) {
...
void iftWriteDataSetFishStandard(  iftDataSet *dataSet, const char *pathname, iftDatasetVersion version){
    if (dataSet == NULL) {
...
//    float* alpha = iftAlloc(dimensionsFeat[1],sizeof(float));
    ...
    if (dataSet != NULL){
      if ((dataSet->fsp.mean != NULL)||(dataSet->fsp.w != NULL)||
	  (dataSet->fsp.R != NULL) || (dataSet->fsp.W != NULL)){
	char *fsp_path = iftJoinPathnames(2, tempDirPath, "feat_space.data");
	iftWriteFeatSpaceParam(&dataSet->fsp, fsp_path);
	iftFree(fsp_path);
      }
    }
...
iftDataSet *iftReadDataSet(const char *format, ...) {
    ...
    if(dataSet != NULL) {
      ...
      if (iftFileExists(fsp_path)){
	dataSet->fsp = iftReadFeatSpaceParam(fsp_path);
	iftFree(fsp_path);
      }
    }
...
iftFeatSpaceParam iftReadFeatSpaceParam(const char *path) {
  ...
  if (fread(&field,sizeof(char),10,fp) != 10){
    iftError("Cannot read the first field","iftReadFeatSpaceParam");
  }

  if (strcmp(field,"has_mean  ")==0){
    if (fread(&fsp.nfeats, sizeof(int), 1, fp)!=1) {
      iftError("Cannot read nfeats","iftReadFeatSpaceParam");
    }
    fsp.mean = iftAllocFloatArray(fsp.nfeats);
    if (fread(fsp.mean, sizeof(float), fsp.nfeats, fp)
	!= fsp.nfeats){
      iftError("Cannot read mean","iftReadFeatSpaceParam");
    }    
    if (fread(&field,sizeof(char),10,fp) != 10){
      iftError("Cannot read the second field","iftReadFeatSpaceParam");
    }
    if (strcmp(field,"has_stdev ")==0){
      fsp.stdev = iftAllocFloatArray(fsp.nfeats);
      if (fread(fsp.stdev, sizeof(float), fsp.nfeats, fp)
	  != fsp.nfeats){
	iftError("Cannot read stdev","iftReadFeatSpaceParam");
      }    
    }
  }

  if (fread(&field,sizeof(char),10,fp) != 10){
    iftError("Cannot read the third field","iftReadFeatSpaceParam");
  }

  if (strcmp(field,"has_w     ")==0){
    if (fread(&fsp.ncomps,sizeof(int),1,fp) != 1){
      iftError("Cannot read ncomps","iftReadFeatSpaceParam");
    }
    fsp.w = iftAllocCharArray(fsp.ncomps);
    if (fread(fsp.w,sizeof(char),fsp.ncomps,fp) != fsp.ncomps){
      iftError("Cannot read w","iftReadFeatSpaceParam");
    }    
  }

  if (fread(&field,sizeof(char),10,fp) != 10){
    iftError("Cannot read the fourth field","iftReadFeatSpaceParam");
  }

  if (strcmp(field,"has_R     ")==0){
    fsp.R = iftReadRawMatrixFILE(fp);
  }
  
  if (fread(&field,sizeof(char),10,fp) != 10){
    iftError("Cannot read the fifth field","iftReadFeatSpaceParam");
  }
...
void iftWriteFeatSpaceParam(  iftFeatSpaceParam *fsp, const char *path) {
  FILE *fp = fopen(path, "wb");
  char *field = iftAllocCharArray(10);
  
  if (fp == NULL)
    iftError(MSG_FILE_OPEN_ERROR, "iftWriteFeatSpaceParam", path);
  
  if ((*fsp).mean!=NULL){
    strcpy(field,"has_mean  ");
    if (fwrite(field,sizeof(char),10,fp) != 10){
      iftError("Cannot write has_mean","iftWriteFeatSpaceParam");
    }
    if (fwrite(&(*fsp).nfeats, sizeof(int), 1, fp)!=1) {
      iftError("Cannot write nfeats","iftWriteFeatSpaceParam");
    }
    if (fwrite((*fsp).mean, sizeof(float), (*fsp).nfeats, fp)
	!= (*fsp).nfeats){
      iftError("Cannot write mean","iftWriteFeatSpaceParam");
    }
    if ((*fsp).stdev != NULL) {
      strcpy(field,"has_stdev ");
      if (fwrite(field,sizeof(char),10,fp) != 10){
	iftError("Cannot write has_stdev","iftWriteFeatSpaceParam");
      }	
      if (fwrite((*fsp).stdev, sizeof(float), (*fsp).nfeats, fp)
	  != (*fsp).nfeats){
	iftError("Cannot write stdev", "iftWriteFeatSpaceParam");
      }
    } else {
      strcpy(field,"no_stdev  ");
      if (fwrite(field,sizeof(char),10,fp) != 10){
	iftError("Cannot write no_stdev","iftWriteFeatSpaceParam");
      }
    }    
  } else {
    strcpy(field,"no_mean   ");
    if (fwrite(field,sizeof(char),10,fp) != 10){
      iftError("Cannot write no_mean","iftWriteFeatSpaceParam");
    }
  }
  ...
  if ((*fsp).w != NULL) {
    strcpy(field,"has_w     ");
    if (fwrite(field,sizeof(char),10,fp) != 10){
      iftError("Cannot write has_w","iftWriteFeatSpaceParam");
    }
    if (fwrite(&(*fsp).ncomps,sizeof(int),1,fp) != 1){
      iftError("Cannot write ncomps","iftWriteFeatSpaceParam");
    }
    if (fwrite(&(*fsp).w,sizeof(char),(*fsp).ncomps,fp) != (*fsp).ncomps){
      iftError("Cannot write w","iftWriteFeatSpaceParam");
    }    
  } else {
    strcpy(field,"no_w      ");
    if (fwrite(field,sizeof(char),10,fp) != 10){
      iftError("Cannot write no_w","iftWriteFeatSpaceParam");
    }
  }
  if ((*fsp).R != NULL) {
    strcpy(field,"has_R     ");
    if (fwrite(field,sizeof(char),10,fp) != 10){
      iftError("Cannot write has_R","iftWriteFeatSpaceParam");
    }      
    iftWriteRawMatrixFILE((*fsp).R, fp);
  } else {
      strcpy(field,"no_R      ");
    if (fwrite(field,sizeof(char),10,fp) != 10){
      iftError("Cannot write no_R","iftWriteFeatSpaceParam");
    }
  }
...
iftDataSet *iftExtractSamples(  iftDataSet *Z, iftSampleStatus status)
{
    int         i, s, t, nsamples;
    iftDataSet *Z1;
...
iftImage *iftDataSetToLabelImage(  iftDataSet *Z,   iftImage *comp, bool decrement_labels, iftFeatureLabel label_type) {
    if (Z == NULL)
...
iftImage *iftDataSetClusterInformationToLabelImage(  iftDataSet *Z,   iftImage *comp, bool decrement_groups) {
    if (Z == NULL)
...
iftImage *iftDataSetClustersToQuantizedImage(  iftDataSet *Z, bool decrement_groups) {
    if (Z == NULL)
...
iftFeatSpaceParam iftComputeOverallMeanAndStdevFromDatasets(iftDataSet **Zs, int num_Z) {
    ...
    for (int i = 0; i < nfeats; i++)
      fsp.stdev[i] = sqrtf(fsp.stdev[i] / ntrainsamples);
...
void iftNormalizeDataSetByZScoreInPlace(iftDataSet *Z, iftFeatSpaceParam *fsp, float stdev_factor)
{
    ...
    Z->fsp.mean   = iftAllocFloatArray(Z->nfeats);
    Z->fsp.stdev  = iftAllocFloatArray(Z->nfeats);
    Z->fsp.nfeats = Z->nfeats;
    
    for (s=0; s < Z->nsamples; s++) {
      if(iftHasSampleStatus(Z->sample[s], IFT_TRAIN)) {
	for (i = 0; i < Z->nfeats; i++) {
	  Z->fsp.mean[i] += Z->sample[s].feat[i];
	}
      }
    }
    
    for (i=0; i < Z->nfeats; i++){
      Z->fsp.mean[i] /= Z->ntrainsamples;
    }

    for (s=0; s < Z->nsamples; s++) {
      if(iftHasSampleStatus(Z->sample[s], IFT_TRAIN)) {
	for (i = 0; i < Z->nfeats; i++) {
	  Z->fsp.stdev[i] += (Z->sample[s].feat[i] - Z->fsp.mean[i]) * (Z->sample[s].feat[i] - Z->fsp.mean[i]);
	}
      }
    }
    
    for (i=0; i < Z->nfeats; i++) {
      Z->fsp.stdev[i] = sqrtf( Z->fsp.stdev[i] / Z->ntrainsamples ) + stdev_factor;
    }
    
    for (s=0; s < Z->nsamples; s++) {
      for (i=0; i < Z->nfeats; i++) {
	Z->sample[s].feat[i] = (Z->sample[s].feat[i] - Z->fsp.mean[i])/
	  Z->fsp.stdev[i];  
      }
    }    
}
...
iftDataSet* iftDataSetTransformBasis(  iftDataSet *Z,   iftMatrix *T) {
...
iftDataSet *iftNormalizeContrastDataSet(iftDataSet *Z)
{
    ...
    for (s=0; s < Zn->nsamples; s++) {
        ...
        stdev = 0.0;
        for (i=0; i < Zn->nfeats; i++) {
	  stdev += (Zn->sample[s].feat[i] - mean)*(Zn->sample[s].feat[i] - mean);
        }
        stdev = sqrtf( stdev / Zn->nfeats ) + IFT_EPSILON;

        for (i=0; i < Zn->nfeats; i++) {
	  Zn->sample[s].feat[i] = (Zn->sample[s].feat[i]-mean)/stdev;
        }
    }
...
iftDataSet *iftCopyDataSet(  iftDataSet *Z, bool copy_feats) {
...
iftDistanceTable *iftCompDistanceTable(  iftDataSet *Z1,   iftDataSet* Z2)
{
...
iftDataSet *iftMImageToDataSet(  iftMImage *mimg,   iftImage *label_img, float coord_scale)
{
...
iftDataSet *iftExtractSamplesFromClass(  iftDataSet *Z, int labelClass) {
...
void iftGetMinMaxWeightInDataSet(  iftDataSet *Z, float *min_weight, float *max_weight) {
    if (Z == NULL)
...
iftIntArray * iftGetDataSetTrueLabels(  iftDataSet *dataset) {
...
void iftSampleFileSet(iftFileSet *Z,   iftSampler *sampler, int iteration) {
    if(iteration<0 || iteration>=sampler->niters) {
...
iftFileSet *iftExtractFileSamples(  iftFileSet *fileSet, iftSampleStatus status) {
    int nfiles = 0;
...
int iftCountSamples(  iftDataSet *Z, iftSampleStatus status) {
    int s, nsamples;
...
int iftCountSamplesTrueLabel(  iftDataSet* Z, int truelabel, iftSampleStatus status) {
    int nsamples = 0;
    for (int i = 0; i < Z->nsamples; ++i) {
...
int iftCountSamplesLabel(  iftDataSet* Z, int label, iftSampleStatus status) {
    int nsamples = 0;
    for (int i = 0; i < Z->nsamples; ++i) {
...
iftDistanceTable* iftCompEuclDistanceTable(  iftDataSet *Z1,   iftDataSet* Z2) {
...
iftDataSet *iftExtractDataSet(  iftDataSet *Z, int begin, int end) {
...
void iftSplitDataSetAt(  iftDataSet *Z, int sample_idx, iftDataSet **Z1, iftDataSet **Z2) {
...
iftDataSet *iftBoundingBoxArrayToDataSet(  iftImage *img,   iftBoundingBoxArray *bb_ary, int n_feats)
{
...
iftDataSet *iftDataSetFromAllSeeds(char *markers_dir, char *mimages_dir, \
				   iftAdjRel *A){
  ...
  iftDataSet *Z = iftCreateDataSet(nsamples,nfeats);
  int s         = 0;
  iftCSV *inverted_list = iftCreateCSV(nsamples,2);
  ...
  for (int i = 0; i < nimages; i++) {
    ...
    while (seed != NULL) {
      int label;
      int p = seed->elem;
      label = seed->label+incr;
      Z->sample[s].id = s; /* sampĺe/line of the inverted list */
      sprintf(inverted_list->data[s][0],"%d",p);
      sprintf(inverted_list->data[s][1],"%s",filename);      
  ...
  iftSetRefData(Z,inverted_list,IFT_REF_DATA_CSV);
  iftSetStatus(Z, IFT_TRAIN);
  iftAddStatus(Z, IFT_SUPERVISED);
...
iftDataSet *iftDataSetFromLabeledSet(iftLabeledSet *S, iftMImage *input, \
				     iftAdjRel *A){
  int nfeats = A->n*input->m, nsamples = iftLabeledSetSize(S);
