...
float iftFindMinimumCostAndClassifySample(  iftCplGraph *graph, iftDataSet *Ztest, int t)
{
...
float iftFindMinimumCostFromAnotherClass(  iftCplGraph *graph, iftDataSet *Ztest, int t) {
 ...
 */
iftDataSet *iftBuildTestImageDataSet(  iftImage *test_img, float smooth_factor) {
    if (test_img == NULL)
 ...
 * @date Dec 3, 2019
 */
iftDblArray *iftAdjacencyRadiusKnnGraphByRank(  iftKnnGraph *graph, int rank) {
    iftDblArray *adj_radius = iftCreateDblArray(graph->nnodes);

    #pragma omp parallel for
    for (int u = 0; u < graph->nnodes; u++) {
          iftAdjSet *adj = graph->node[u].adj;
...
void iftWriteLogReg(  iftLogReg *logreg, const char *pathname) {
    if (logreg == NULL)
...
void iftWriteCplGraph(  iftCplGraph *graph, const char *pathname) {
    FILE* fp = NULL;
    char *path = NULL;
    char *tmp_dir = NULL;
...
int iftClassify(  iftCplGraph *graph, iftDataSet *Ztest) {
    iftDataSet *Ztrain = graph->Z;
...
void iftClassifyByOneClassOPF(iftDataSet *Ztest,   iftKnnGraph *graph,
                              float quantile_of_k) {
...
int iftClassifyWithCertaintyValues(  iftCplGraph *graph, iftDataSet *Ztest) {
    iftDataSet *Ztrain = graph->Z;
 ...
 */
float iftGetKFoldCrossValidationMeanOPFAccuracy(iftDataSet *Z,   int k) {
...
iftCplGraph *iftTrainImageClassifierByOPF(  iftImage *train_img,   iftLabeledSet *train_markers,
                                          float smooth_factor) {
    if (train_img == NULL)
...
iftImage *iftClassifyImageByOPF(  iftImage *test_img,   iftCplGraph *clf, float smooth_factor,
