...
void iftSetSupervoxelObjectLabels(iftImage *label, iftImage *gt_img, iftDataSet *dataset) {
  ...
  int nsups, nobjs;
  // Assign object labels to supervoxels (by majority area)
...
int main(int argc, char *argv[]) 
{
  ...
  if (argc!=6)
    iftError("Usage: iftSupervoxelDataSet <image.[pgm,ppm,scn,png]> <label> <gt_img> <descriptor (0:MC, 1:BIC, 2:MC-CONTEXTUAL, 3:BIC-CONTEXTUAL)> <output_dataset>","main");
  ...
  // write dataset
  iftWriteOPFDataSet(dataset, argv[5]);
