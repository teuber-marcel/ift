...
def main():
	...
	parser.add_argument("smooth_niters", type=int, help="Number of iterations for smoothing")
	...
	beta = args.beta
	niters = args.niters
	smooth_niters = args.smooth_niters
	...
	#alpha_list = np.linspace(0.02,0.2,10)
	#nsup_list = np.linspace(100,1000,10)

	# default values
	alpha_list = np.linspace(0.04,0.2,9)
	nsup_list = np.linspace(100,500,9)
	
	# other experiments
	#alpha_list = np.array([1.0])
	#alpha_list = np.array([0.5])
	#alpha_list = np.array([0.04, 0.2])
	...
	for alpha in alpha_list:
		for nsup in nsup_list:
			output_csv = "{}{}_{}_{}/nseeds.csv".format(output_path, algo, str(alpha), str(int(nsup)))
			output_img_path = "{}{}_{}_{}/".format(output_path, algo, str(alpha), str(int(nsup)))
			if not os.path.exists(output_img_path):
				os.makedirs(output_img_path)
			exp_params = []
			exp_params.append(dataset_path)
			exp_params.append(int(nsup))
			exp_params.append(alpha)
			exp_params.append(beta)
			exp_params.append(niters)
			exp_params.append(smooth_niters)
