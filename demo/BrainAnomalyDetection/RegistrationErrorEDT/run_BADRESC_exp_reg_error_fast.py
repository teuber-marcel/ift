...
POSTPROCESSING_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/RegistrationErrorEDT/post_processing_classified_svoxels.py"
...
'''
    ...
    parser.add_argument('-c', '--max-attenuation-factor', type=float, default=0.75,
    ...
    parser.add_argument('-e', '--exponent', type=float, default=2.0,
                        help='Exponent for the exponent attenuation function. Default: 2.0.')
    ...
    parser.add_argument('-g', '--gamma', type=float,
                        default=0.01, help='GAMMA parameter for RBF classifier. Default: 0.01')
    
    parser.add_argument('-v', '--min-svoxel-volume', type=int, required=False, default=20,
                        help='Minimum SVoxel Value. Default: 20')
    parser.add_argument('-r', '--min-mean-reg-error-mag-on-svoxel', type=float, required=False, default=50.0,
    ...
    parser.add_argument('--skip-post-processing', action='store_true',
                        help='Skip Post-Processing step (SVoxel filtering).')
...
def print_args(args):
    ...
    print(f'- Attenuation Function: {args.max_attenuation_factor}')
...
def main():
    ...
    if not args.skip_registration_error_computation:
        reg_error_mag_cmd = f"iftAttenuatedRegErrorMagnitude --image-path {args.image} " \
                            ...
                            f"--out-reg-error-mag-map {reg_error_path} --attenuation-function exp " \
                            f"--max-attenuation-factor {args.max_attenuation_factor} --exponent {args.exponent}"
        if args.bias:
    ...
    clf_svoxels_path = os.path.join(working_dir, "clf_svoxels.nii.gz")

    print("\n\n####### One-Class SVM")
    ocsvm_cmd_args = f"--supervoxels {svoxels_path} --supervoxels-datasets-entry {datasets_dir} " \
                     f"--output-anomalous-supervoxels-img {clf_svoxels_path} " \
    ...
    if args.skip_post_processing:
        shutil.copy(clf_svoxels_path, args.out_classification_mask)
    else:
        print("\n\n####### POST-PROCESSING")
        postprocessing_cmd_args = f"--classified-svoxels-path {clf_svoxels_path} " \
                                  f"--reg-error-mag-path {reg_error_path} " \
                                  f"--post-classified-svoxels-path {args.out_classification_mask} " \
                                  f"--min-svoxel-volume {args.min_svoxel_volume} " \
                                  f"--min-mean-reg-error-mag-on-svoxel {args.min_mean_reg_error_mag_on_svoxel}"
        os.system(f"python {POSTPROCESSING_SCRIPT_PATH} {postprocessing_cmd_args}")
