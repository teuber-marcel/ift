...
if [ -z $4 ]; then
  echo "Usage: $0 <number of splits> <number of active learning iterations> <dataset> <output_prefix> [resume split]"
  exit 1
fi
...
do
  ...
  do
    ...
    do
      ...
      $BIN_ACTIVE -d $TRAIN_PATH -e $TEST_PATH -k 0.01 -a $activeAlgo -c $classifier -m $ACTIVE_ITERS -w $CSV_PATH
    done
  done
  idx=`expr ${idx} + 1`
done
