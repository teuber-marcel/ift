...
void iftGetRequiredArgs(  iftDict *args, char **img_path, char **asymmap_path, char **mask_path,
                        int *mov_obj, int *fix_obj, char **symm_super_path, iftFileSet **train_set,
...
int main(int argc, const char *argv[]) {
    ...
    iftGetRequiredArgs(args, &img_path, &asymmap_path, &mask_path, &mov_obj, &fix_obj, &symm_super_path, &train_set,
                       &train_asymmap_set, &out_dir);
    ...
    if (template_native_path && iftFileExists(template_native_path)) {
        ...
        char *super_img_path = iftJoinPathnames(2, out_dir, "supervoxels.nii.gz");
...
iftDict *iftGetArgs(int argc, const char *argv[]) {
    char program_description[2048] = \
        ...
        "- It saves in the output directory: symmetry supervoxel datasets will be saved and a mask with the supervoxels " \
        "in the most asymmetry sides (compared with a template on native space).\n\n" \
    ...
    iftCmdLineOpt cmd_line_opts[] = {
        ...
        {.short_name = "-i", .long_name = "--train-set-on-native-entry", .has_arg=true, .arg_type=IFT_STR_TYPE,
         .required=true, .help="Dir. or CSV with the pathname from the Train. Images on NATIVE space."},
...
void iftGetRequiredArgs(  iftDict *args, char **img_path, char **asymmap_path, char **mask_path,
    ...
    *symm_super_path = iftGetStrValFromDict("--symmetric-supervoxels-path", args);
    const char *train_set_entry = iftGetConstStrValFromDict("--train-set-on-native-entry", args);
...
iftImage *iftFilterSymmSupervoxels(  iftImage *img,   iftImage *template_native,   iftImage *symm_super_img,
    ...
    iftImage *super_img = iftCreateImageFromImage(symm_super_img);
    ...
    for (int p = 0; p < symm_super_img->n; p++) {
        ...
        super_img->val[p] = super_mov_img->val[p] + super_fix_img->val[p];
    }
