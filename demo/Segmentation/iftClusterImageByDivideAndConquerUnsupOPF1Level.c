...
int main(int argc, char *argv[])
{
  ...
  if (argc<7 || argc >9)
    iftError("Usage: iftClusterImageByDivideAndConquerUnsupOPF1Level <image.ppm(pgm,png)> <num_blocks> <train_samples_nb> <kmax(percent or value)> <area> <similarity threshold(0..1)> [<gt_image>] [<propagate_root_true_labels>(0:NO/1:YES)]","main");
...
//      iftError("The gt image must be binary","main");
  ...
  t1 = iftTic();
  iftClusterImageByDivideAndConquerUnsupOPF1Level(Z, atoi(argv[2]), atof(argv[3]), iftNormalizedCut, atof(argv[4]), 0);
  t2 = iftToc();

  fprintf(stdout,"%d %.2f ",Z->ngroups,iftCompTime(t1,t2));

  /* propagate the prototypes' true labels in the clusters if neccessary*/
  ...
  if (!propagate_cluster_true_labels){
    ...
    aux= remove_block_lines(img,label, block_img,atof(argv[6]));
    ...
    aux=iftVolumeOpen(label,atoi(argv[5]));
    iftDestroyImage(&label);
    label=aux;
    aux=iftVolumeClose(label,atoi(argv[5]));
    iftDestroyImage(&label);
    label=aux;
