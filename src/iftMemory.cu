...
void iftCopyIntArrayFromGPU(int* dst, int* src, size_t n) {
    iftCopyFromGPU((void*)dst, (void*)src, n*sizeof(int));
}

void iftCopyIntArrayToGPU(int* dst, int* src, size_t n) {
    iftCopyToGPU((void*)dst, (void*)src, n*sizeof(int));
}

int* iftAllocIntArrayGPU(size_t n) {
    return (int*) iftAllocGPU(n, sizeof(int));
}

void iftCopyFloatArrayFromGPU(float* dst, float* src, size_t n) {
    iftCopyFromGPU((void*)dst, (void*)src, n*sizeof(float));
}

void iftCopyFloatArrayToGPU(float* dst, float* src, size_t n) {
    iftCopyToGPU((void*)dst, (void*)src, n*sizeof(float));
}

float* iftAllocFloatArrayGPU(size_t n) {
    return (float*) iftAllocGPU(n, sizeof(float));
}

void *iftAllocGPU(size_t n, size_t size) {
    void *mem = NULL;
