...
class InteractiveSegmentationApp(Frame):
    def __init__(self, master):
        ...
        self.color_labels = ["Label 1", "Label 2"]
        self.color_dict = { self.color_labels[0] : (76,85,255), self.color_labels[1] : (225,1,148)}
    ...
    def create_widgets(self):
        ...
        filemenu.add_command(label = "Open Image Folder...", underline =5, command = self.open_image_folder)
        ...
        self.segmentation_frame = LabelFrame(self.left_frame, text = "Oversegmentation")
        ...
        label_volume_threshold = Label(self.segmentation_frame, text = "Volume threshold:")
        label_volume_threshold.grid(row = 1, column = 0)
        ...
        self.entry_volume_threshold.grid(row = 1, column = 1)
    ...
    def draw_superpixels_on(self,source,destination, write_label_image = False):
        if self.scaler_nsuperpixels.get() != 0:     
            ...
            #label = iftCreateRefinedLabelImage(self.ift_image, seed, spatial_radius, volume_threshold, self.STEPS, self.VOL_RATIO)
    ...
    def redraw_image(self, event = None):  
        ...
        if self.show_superpixels.get():
            ...
            iftWriteImageP6(self.markers_image,"markers.ppm")
    ...
    def show_classification_mask(self, classification_image):
        classification_mask = iftCopyImage(self.markers_image)
        ...
        if self.show_superpixels.get():
            ...
            iftWriteImageP6(classification_mask,"tmp_class_img.ppm")
    ...
    def create_classification_cl_image(self):
        ...
        seed = iftLabeledSetFromSeedImage(self.seed_image)
        iftSetTrainingSupervoxelsFromSeeds(self.dataset,self.label_image,seed);
        if self.dataset.ntrainsamples == 0:
            raise Exception("No training samples.")
                
        graph = iftCreateCplGraph(self.dataset)
        iftSupTrain(graph)
        iftClassify(graph,self.dataset)
        
        #Creating classification mask
        classification_image = iftCreateImage(self.ift_image.xsize,self.ift_image.ysize,self.ift_image.zsize)
    ...
    def create_superpixel_cl_image(self):
        seed = iftLabeledSetFromSeedImage(self.seed_image)
        iftSuperpixelClassification(self.region_graph,self.label_image, seed)    
    ...
    def create_pixel_cl_image(self):
        ...
        seed = iftLabeledSetFromSeedImage(self.seed_image)
        basins = iftEnhanceEdges(self.ift_image, a, seed, 0.5)
        
        classification_image = iftWatershed(basins, a, seed)
        #classification_image = iftWatershedOnPixelDist(self.dataset_pixel,a,seed)
    ...
    def create_image_data_structures(self):
        ...
        basins = iftImageBasins(self.ift_image,adj_relation)
        
        marker = iftVolumeClose(basins,volume_threshold)
        
        self.label_image = iftWaterGray(basins,marker,adj_relation)
        
        self.dataset = iftSupervoxelsToDataSet(self.ift_image, self.label_image)
        if self.dataset.nfeats == 3:
            self.dataset.set_alpha([0.2,1.0,1.0])
            
        adj_relation = iftCircular(1)
        self.region_graph = iftRegionGraphFromLabelImage(self.label_image,self.dataset,adj_relation)
        
        #Pixel dataset
        self.dataset_pixel = iftImageToDataSet(self.ift_image)
    ...
    def _open_image(self,image_path):
        ...
        self.ift_image = iftReadImageP6(image_path)
        ...
        if os.path.isfile(current_image_path[:-3] + "markers"):
            ...
            self.markers_image = iftCopyImage(self.ift_image)
            self.seed_image = iftCreateImage(self.ift_image.xsize,self.ift_image.ysize,1)
            iftSetImage(self.seed_image, -1)
    ...
    def _load_markers(self, seed_path):
       ...
       seed = iftReadSeeds2D(seed_path,self.ift_image)
       self.seed_image = iftSeedImageFromLabeledSet(seed,self.ift_image)
       self.markers_image = iftCopyImage(self.ift_image)
       
       #creating more labels if needed
       number_labels = iftMaximumValue(self.seed_image)
    ...
    def draw_circular_marker(self, label, pixel, radius, label_color, write_as_seed = True):
        ...
        for i in range(adj_rel.n):
            ...
            if iftValidVoxel(self.seed_image, adjacent) == chr(1):
    ...
    def _convert_ycbcr_to_rgb_str(self,tuple):
        ycbcr_color = iftColor()
        ycbcr_color.set_values(tuple)
        
        rgb_tuple = iftYCbCrtoRGB(ycbcr_color).get_values()
    ...
    def _save_markers(self, filename):
        ...
        seed = iftLabeledSetFromSeedImage(self.seed_image)
        iftWriteSeeds2D(filename,seed,self.ift_image)
    ...
    def save_super_mask(self):
        ...
        classification_image = self.create_superpixel_cl_image()
        output_image = iftCreateImage(classification_image.xsize,classification_image.ysize,classification_image.zsize)
        ...
        iftWriteImageP5(output_image,filename)
    ...
    def save(self):
        ...
        current_image_path = os.path.join(self.current_folder,self.current_images[self.current_image_index])
        self._save_markers(current_image_path[:-3] + "markers")
