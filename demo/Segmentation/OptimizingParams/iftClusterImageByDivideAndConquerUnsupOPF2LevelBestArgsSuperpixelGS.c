...
/*  if (!iftIsColorImage(img)){
    for (int p=0;p<img->n;p++){
      in_label->val[p]=(img->val[p] <= 20)?1:in_label->val[p]+1;
    }
    A=iftCircular(sqrtf(2.0));

    aux=iftRelabelRegions(in_label,A);
    in_label=aux;
...
#pragma omp parallel for
  ...
  aux=iftRelabelGrayScaleImage(new_label_img,true);
...
double find_best_args(iftDict* problem, iftDict* params) {
  ...
  float threshold=iftGetDblValFromDict("thresh_arg",problem);
  bool exited=false;
  ...
  for (int i=0;i<file_count && !exited;i++) {
    ...
    if (type_measure){
      /*computing under segmentation error, here we are minimizing not maximizing*/
      measure -= iftUnderSegmentation(gt_label, label);
    }
    else{
      /*compute boundary recall*/
      border  = iftBorderImage(label,0);
      gt_border=iftBorderImage(gt_label,0);
      measure += iftBoundaryRecall(gt_border, border, 2.0);
  ...
  if (!exited){
    int mean_groups = superpixels_acc / file_count;
    if (mean_groups > limit_nb_sup)
      measure =IFT_INFINITY_FLT_NEG;
    else
      measure /=file_count;
  }
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 18)
    iftError("Usage: iftClusterImageByDivideAndConquerUnsupOPF2LevelBestArgsSuperpixelGS <dir_orig_img> <dir_label_img> <count_train_img> <limit_nb_superp> <limit_secs> <optimizing measure 0(BR)/1(UE)> <count_train_samples> <orig_img_ext (ppm:png,...)> <init_nb_block> <init_kmax1> <final_kmax1> <step_kmax1> <init_kmax2> <final_kmax2> <step_kmax2> <area> <threshold>","main");
  ...
  iftInsertIntoDict("thresh_arg", atof(argv[17]),problem);
