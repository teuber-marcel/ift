...
iftKernel *iftReadKernel(char *filename)
{
  ...
  for (i=0; i < K->A->n; i++) {
    if(fscanf(fp,"%d %d %d %d %f",&K->A->dx[i],&K->A->dy[i],&K->A->dz[i],&K->A->dt[i],&K->weight[i])!=5)
...
void iftWriteKernel(iftKernel *K, char *filename)
{
  ...
  for (i=0; i < K->A->n; i++) {
    fprintf(fp,"%d %d %d %d %f\n",K->A->dx[i],K->A->dy[i],K->A->dz[i],K->A->dt[i],K->weight[i]); 
  }
...
iftMKernel *iftCopyMKernel(  iftMKernel *K) {
    iftMKernel *cpy = NULL;
...
iftMMKernel *iftReadMMKernelFILE(FILE* fp)
{
  ...
  iftAdjRel *A = iftCreateAdjRel(size);

  if (fread(A->dx, sizeof(int), A->n, fp) != A->n)
    iftError("Error when reading the dx array", "iftReadMMKernel");
  if (fread(A->dy, sizeof(int), A->n, fp) != A->n)
    iftError("Error when reading the dy array", "iftReadMMKernel");
  if (fread(A->dz, sizeof(int), A->n, fp) != A->n)
    iftError("Error when reading the dz array", "iftReadMMKernel");
  if (fread(A->dt, sizeof(int), A->n, fp) != A->n)
    iftError("Error when reading the dz array", "iftReadMMKernel");
...
void iftWriteMMKernel(  iftMMKernel *k_bank, const char *filename) {
...
void iftWriteMMKernelFILE(  iftMMKernel *K, FILE* fp)
{
  int dim;
  ...
  if (fwrite(&K->A->n, sizeof(int), 1, fp) != 1)
    iftError("Error when writing adjacency size", "iftWriteMMKernel");
        
  if (fwrite(K->A->dx, sizeof(int), K->A->n, fp) != K->A->n)
    iftError("Error when writing dx array", "iftWriteMMKernel");
  
  if (fwrite(K->A->dy, sizeof(int), K->A->n, fp) != K->A->n)
    iftError("Error when writing dy array", "iftWriteMMKernel");
  
  if (fwrite(K->A->dz, sizeof(int), K->A->n, fp) != K->A->n)
    iftError("Error when writing dz array", "iftWriteMMKernel");

  if (fwrite(K->A->dt, sizeof(int), K->A->n, fp) != K->A->n)
    iftError("Error when writing dt array", "iftWriteMMKernel");
...
float *iftGaborFilter(iftAdjRel *A, float theta, float freq, int sigma_x, int sigma_y)
{
  ...
  for (int i = 0; i < A->n; i++)
  {
    ...
    gabor[i] = exp(-0.5 * (((x0 * x0) / (sigma_x * sigma_y)) + ((y0 * y0) / (sigma_y * sigma_y)))) * cos(2 * IFT_PI * freq * x0);
  }
...
iftMMKernel *iftGaborKernelBank(int kernel_size, int n_orientations, int n_frequencies, int max_freq, int min_freq, int sigma_x, int sigma_y)
{
  ...
  for (int i = 0; i < n_orientations; i++)
  {
    ...
    {
      float *gabor = iftGaborFilter(A, theta[i], freq[j], sigma_x, sigma_y);
      float mag = 0.0;
      for (int l = 0; l < A->n; l++)
      {
...
{
  iftAdjRel *A = iftCircular(radius);
  iftMKernel *K = iftCreateMKernel(A, nbands);
  ...
  for (int b = 0; b < nbands; b++) {
    ...
    for (i=0; i < A->n; i++) {
      dist = A->dx[i]*A->dx[i] + A->dy[i]*A->dy[i];
      K->weight[b].val[i] = exp(-dist/(2*var));
      sum += K->weight[b].val[i];
    }

    for (i=0; i < A->n; i++) {
      K->weight[b].val[i] /= sum;
    }
