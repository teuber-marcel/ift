...
def run_experiment(exp_params):
	...
	args = ["iftFastSuperpixel2DExperiments"]
...
def main():
	...
	for rad in rad_list:
		for nsup in nsup_list:
			output_csv = "{}{}_{}_{}/nseeds.csv".format(output_path, algo, str(int(rad)), str(int(nsup)))
			output_img_path = "{}{}_{}_{}/".format(output_path, algo, str(int(rad)), str(int(nsup)))
			if not os.path.exists(output_img_path):
				os.makedirs(output_img_path)
			exp_params = []
			exp_params.append(dataset_path)
			exp_params.append(int(nsup))
			exp_params.append(rad)
