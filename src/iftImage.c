...
/**
 * @brief Alias for the original nifti header structures (file nifti1.h),
 *        written by Bob Cox, SSCC, NIMH.
 * 
 * @author Samuka Martins, Azael Sousa
 * @date Apr 20, 2018
 */
...
NIfTI1Header *_iftReadNIfTI1HeaderAndDataFromGZip(const char *path, char **data_out, iftImageDomain *dom_out, iftVoxelSize *voxel_sizes_out) {
    ...
    // reading the raw image data - from byte #352
    if (header->vox_offset < 352 || (int)header->vox_offset%16 != 0)
        iftError("Invalid NifTI1 file. Voxel offset %d should be 352", "_iftReadNIfTI1HeaderAndDataFromGZip", header->vox_offset);
    // we previously read 348 bytes, then move the file pointer in vox_offset - 348
    iftGZipSeek(gzip, (int)header->vox_offset - 348, SEEK_CUR);
...
NIfTI1Header *_iftCreateNIfTI1HeaderFromImage(  iftImage *img, bool is_analyze) {
 ...
 * @date Sep 13, 2018
 */
void _iftQuaternToNIfTIRotationMatrix(  NIfTI1Header *header, iftVector *vec_i, iftVector *vec_j, iftVector *vec_k) {
 ...
 */
void _iftFindImageOrientationOnNIfTI(  NIfTI1Header *header, ift3DAxisOrientation *x_axis_orient, ift3DAxisOrientation *y_axis_orient, ift3DAxisOrientation *z_axis_orient) {
...
/********************** PUBLIC FUNCTIONS *************************/
void iftCopyGrayImageVoxel(  iftImage *src, iftVoxel u, iftImage *dst, iftVoxel v) {
    iftImgVoxelVal(dst, v) = iftImgVoxelVal(src, u);
}

void iftCopyColorImageVoxel(  iftImage *src, iftVoxel u, iftImage *dst, iftVoxel v) {
    iftImgVoxelVal(dst, v) = iftImgVoxelVal(src, u);
...
void iftVerifyImageDomains(  iftImage *img1,   iftImage *img2, const char *function) {
...
iftImageDomain iftGetImageDomain(  iftImage *img) {
    iftImageDomain dom;
    dom.xsize = img->xsize;
    dom.ysize = img->ysize;
    dom.zsize = img->zsize;
...
void iftVerifyImages(  iftImage *img1,   iftImage *img2, const char *external_function) {
...
iftImageArray *iftReadImageSet(  iftFileSet *img_set) {
...
iftIntMatrix *iftReadGrayImageSetAsIntMatrix(  iftFileSet *img_set) {
...
void iftCopyCbCr(  iftImage *src, iftImage *dst) {
    if (iftIsColorImage(src)) {
...
int iftMaximumValueInRegion(  iftImage *img, iftBoundingBox bb) {

    if (img == NULL)
        iftError("Image is NULL", "iftMaximumValueInRegion");

    if (!iftValidVoxel(img, bb.begin))
      iftError("Invalid beginning voxel (%d, %d, %d)", "iftMaximumValueInRegion",bb.begin.x, bb.begin.y, bb.begin.z);
    
    if (!iftValidVoxel(img, bb.end))
        iftError("Invalid ending voxel (%d, %d, %d)", "iftMaximumValueInRegion",
...
void iftMinMaxValueInRegion(  iftImage *img,   iftImage *region, int *min, int *max) {
...
iftIntArray *iftMaximumObjectValues(  iftImage *img,   iftImage *label_img, iftIntArray **voxels_idxs_out) {
...
iftImage *iftReadImageNIfTI(const char *format, ...) {
    ...
    if (min < 0.0)
        iftWarning("Shifting negative values within [%f, %f] to a positive range\n", "iftReadImageNIfTI", min, max);
    ...
    /* shifting: move values into the positive range if needed. This */
    /* will preserve the original information, which would be */
    /* destroyed with normalization. */
...
bool iftIsValidFormat(const char *filename){
    ...
    if (iftCompareStrings(ext, ".zscn") || iftCompareStrings(ext, ".scn.gz") ||
        ...
        iftCompareStrings(ext, ".jpg") || iftCompareStrings(ext, ".jpeg") ||
...
void iftWriteImagePNG(  iftImage* img, const char* format, ...) {
...
//void iftWriteImageTIFF(  iftImage* image, const char* format, ...){
//    va_list args;
//    char filename[IFT_STR_DEFAULT_SIZE];
//
//    va_start(args, format);
...
void iftWriteImageJPEG(  iftImage* img, const char* format, ...){
    va_list args;
    char filename[IFT_STR_DEFAULT_SIZE];
...
void iftWriteImagePNGWithAlpha(  iftImage* img,   iftImage *alpha, const char* format, ...) {
...
void iftWriteImage(  iftImage *img, const char *format, ...) {
    FILE   *fp     = NULL;
    int    p;
    uchar  *data8  = NULL;
    ushort *data16 = NULL;
...
void iftWriteImageNIfTI(  iftImage *img, const char *format, ...) {
    va_list args;
...
void iftWriteImageNIfTIGZip(  iftImage *img, const char *format, ...) {
    va_list args;
    char path[IFT_STR_DEFAULT_SIZE];
...
void iftWriteImageAnalyze(  iftImage *img, const char *format, ...) {
    va_list args;
    char filename[IFT_STR_DEFAULT_SIZE];
...
void iftWriteImageGZip(  iftImage *img, const char *format, ...) {
    iftGZipFile fp = NULL;
    int    p;
    uchar  *data8  = NULL;
    ushort *data16 = NULL;
...
void iftWriteImageP5(  iftImage *img, const char *format, ...) {
    FILE   *fp     = NULL;
    int    p, hi, lo;
    uchar  *data8  = NULL;
    ushort *data16 = NULL;
...
void iftWriteImageP6(  iftImage *img, const char *format, ...) {
    FILE     *fp = NULL;
    int      p;
    ushort   rgb16[3];
    iftColor YCbCr, RGB;
...
void iftWriteImageP2(  iftImage *img, const char *format, ...) {
    FILE *fp = NULL;
    int  p;
...
iftImage *iftRemoveLabels(  iftImage *label_img,   iftIntArray *labels) {
...
iftImage *iftRelabelImage(  iftImage *label_img) {
...
void iftCopyImageInplace(  iftImage *src, iftImage *dest) {
    int p;
...
void iftCopyImageInsideRegion(  iftImage *src,   iftImage *mask, iftImage *dst) {
...
bool iftIsImageEqual(  iftImage *img1,   iftImage *img2) {
    if (!iftIsVoxelSizeEqual(img1, img2)) {
...
iftImage *iftExtractSlice(  iftImage *vol_img, iftImagePlaneOrientation plane_orientation, long slice) {
    // CHECKERS
    if (vol_img == NULL)
...
iftImage *iftAddRectangularBoxFrame(  iftImage *img, int dx, int dy, int dz, int value)
{
    iftImage *fimg;
...
iftImage *iftRemRectangularBoxFrame(  iftImage *fimg, int dx, int dy, int dz) {
...
iftVoxel iftGeometricCenterVoxel(  iftImage *bin_mask) {
    iftPoint c = {0, 0, 0};
    int n = 0;
...
iftVoxelArray *iftGeometricCenterVoxelsLabelImage(  iftImage *label_img) {
...
iftImage *iftImageGradientMagnitude(  iftImage *img, iftAdjRel *Ain)
{
    iftAdjRel *A = NULL;
    if (Ain == NULL) {
        if (iftIs3DImage(img))
...
inline iftVoxel iftGetVoxelCoord(  iftImage *img, int p)
{
    /* old
...
iftImage *iftLuminance(  iftImage *img)
{
...
iftImage *iftRegionBorders(  iftImage *label_img) {
...
iftImage *iftReadSlices(  iftFileSet *files)
{
...
iftVoxelArray *iftGeometricCentersFromLabelImage(  iftImage *label_img) {
...
void iftWriteImageByExt(  iftImage *img, const char *format, ...) {
    if (img == NULL)
...
iftVoxelArray *iftFindClosestObjectVoxels(  iftImage *label_img,   iftVoxelArray *voxel_arr) {
...
iftBoundingBox iftMinBoundingBox(  iftImage *img, iftVoxel *gc_out) {
    if (img == NULL)
...
iftBoundingBox iftMinObjectBoundingBox(  iftImage *img, int obj_label, iftVoxel *gc_out) {
    if (img == NULL)
...
iftBoundingBox *iftMinLabelsBoundingBox(  iftImage *img,   iftIntArray *labels, iftVoxelArray **gcs_out) {
    int n_objs = labels->n;
...
iftBoundingBox *iftAllMinBoundingBox(  iftFileSet *img_paths, iftVoxel **gcs_out) {
...
iftBoundingBox *iftAllMinObjectBoundingBox(  iftFileSet *img_paths, int obj_label, iftVoxel **gcs_out) {
    iftBoundingBox *mbbs  = (iftBoundingBox*) iftAlloc(img_paths->n, sizeof(iftBoundingBox));
    iftVoxel *gcs         = (iftVoxel*) iftAlloc(img_paths->n, sizeof(iftVoxel));
...
iftBoundingBox **iftAllMinLabelsBoundingBox(  iftFileSet *label_paths,   iftIntArray *labels,
                                            iftVoxel ***gcs_out) {
...
iftImage *iftExtractROI(  iftImage *img, iftBoundingBox bb) {
    if (img == NULL)
...
iftImage *iftExtractObjectInsideROI(  iftImage *src, iftBoundingBox bb, int obj_label) {
    if (src == NULL)
...
iftImage *iftExtractLabelsInsideROI(  iftImage *src_img, iftBoundingBox bb,   iftIntArray *labels) {
...
void iftInsertROI(  iftImage *roi, iftImage *target, iftVoxel begin) {
    if (roi == NULL)
...
void iftInsertROIByCenter(  iftImage *roi, iftImage *target) {
    if (roi == NULL)
...
void iftInsertROIByPosition(  iftImage *roi,   iftVoxel roi_pos, iftImage *target,   iftVoxel target_pos) {
    iftVoxel uo, uf;
...
iftImage *iftCopyImageROI(  iftImage *src, iftBoundingBox bb) {
    if (src == NULL)
...
iftImage *iftExtractObject(  iftImage *src, int obj_label) {
    if (src == NULL)
...
iftImage *iftExtractLabels(  iftImage *src_img,   iftIntArray *labels) {
    iftBoundingBox bb;
...
int iftCountBackgroundSpels(  iftImage *label_img) {
    return iftCountObjectSpels(label_img, 0);
}


int iftCountTotalObjectSpels(  iftImage *label_img) {
    return (label_img->n - iftCountBackgroundSpels(label_img));
}


int iftCountObjectSpelsFromBoundingBox(  iftImage *label_img, int obj_label, iftBoundingBox bb) {
    if (label_img == NULL)
...
iftIntArray *iftCountLabelSpels(  iftImage *label) {
    if (label == NULL)
        iftError("Label Image is NULL", "iftCountObjectVoxels");
...
float iftAreaVolumeOfObject(  iftImage *label, int obj_label) {
    if (label == NULL)
...
double iftAreaVolumeOfObjectFromBoundingBox(  iftImage *label_img, int obj_label, iftBoundingBox bb) {
    if (label_img == NULL)
        iftError("Label Image is NULL", "iftAreaVolumeOfObject");
...
iftFloatArray *iftAreaVolumeOfLabels(  iftImage *label) {
    if (label == NULL)
...
iftImageTiles *iftCopyImageTiles(  iftImageTiles *tiles) {
    if (tiles == NULL)
...
iftImageTiles *iftImageTilesByEquallyDividingAxes(  iftImage *img, int ntiles_x, int ntiles_y, int ntiles_z) {
    iftBoundingBox bb;
...
bool iftIsLabelImage(  iftImage *label_img, int n_objects) {
    if (label_img == NULL)
...
int iftGetNumberOfObjectsFromImage(  iftImage *label_img) {
    if (label_img == NULL)
...
iftImage *iftBMapToBinImage(  iftBMap *bmap, int xsize, int ysize, int zsize) {
    if (bmap == NULL)
...
iftBMap *iftBinImageToBMap(  iftImage *bin_img) {
    if (bin_img == NULL)
...
bool iftIsBinaryImage(  iftImage *img) {
...
iftIntArray *iftGetObjectLabels(  iftImage *label_img) {
    if (label_img == NULL)
...
iftIntArray *iftFindObjectLabels(  iftImage *label_img) {
    int max_label = iftMaximumValue(label_img);
...
iftIntArray *iftGetObjectVoxels(  iftImage *label_img) {
    iftSet *obj_set = NULL;
...
int iftGetLabelByMajorityVoting(  iftImage *label_img, bool ignore_zero) {
    if (label_img == NULL)
...
iftIntArray *iftGetSupervoxelTrueLabelByMajorityVoting(  iftImage *label_img,   iftImage *super_img) {
    if (label_img == NULL)
    ...
    for (int p = 0; p < super_img->n; p++) {
          int cell       = super_img->val[p] - 1; // ex: the supervoxel with label 2 has cell (idx) 1
          int true_label = label_img->val[p];
        if (true_label >= 0) {
...
iftImage *iftAdjRelToImage(  iftAdjRel *adj, int xsize, int ysize, int zsize, iftVoxel ref_voxel) {
    // checkers
    if (adj == NULL)
...
int iftMaximumValueInMask(  iftImage *img,   iftImage *mask)
{
    if (mask == NULL)
...
int iftMinimumValueInMask(  iftImage *img, iftImage *mask)
{
    char   no_mask=0;
...
void iftMinimumValuesForLabels(  iftImage *img,   iftImage *label_img,
...
iftImage *iftTranslateImageContent(  iftImage *img, iftVector disp_vec) {
    if (img == NULL)
...
iftImage *iftRelabelGrayScaleImage(  iftImage *img,bool increment_values){
...
iftImage *iftImageBasins(  iftImage *img, iftAdjRel *Ain) {
    iftAdjRel *A = NULL;
    if (Ain == NULL)
...
void iftGetLabeledPathSides(  iftImage *label,   iftSet *path, iftSet **obj, iftSet **bkg)
{
    if (!path) return;
    *obj = NULL, *bkg = NULL;
    ...
    for (  iftSet *s = path->next; s; s = s->next)
    {
...
iftImage *iftGrayImageToColorImage(  iftImage *img, iftColorTable *ctb){
