...
def main():
    if len(sys.argv) != 4:
        sys.exit(
            f"python {sys.argv[0]} <specific_svoxel_dataset.zip> <tsne_perplexity> <output_plot_file.html>")
    ...
    print("------------------------------------------")
    print(
        f"- Specific Symmetric Supervoxel DataSet Path: {symm_svoxels_dataset_path}")
    ...
    tsne = TSNE(n_components=2, verbose=1, n_iter=500,
                perplexity=perplexity, n_jobs=8)
    X_proj = tsne.fit_transform(X)

    trace_train = go.Scattergl(
        x=X_proj[:-1, 0], y=X_proj[:-1, 1], name='Healthy Images', mode='markers',
        marker=dict(
            color='blue',
            size=6,
            line=dict(
    ...
    trace_test = go.Scattergl(
        x=X_proj[-1:, 0], y=X_proj[-1:, 1], name='Abnormal Asymmetry', mode='markers',
        marker=dict(
            color='red',
            size=12,
            line=dict(
    ...
    symmetric_svoxels_feats_filename = os.path.basename(
        symm_svoxels_dataset_path).split(".zip")[0]
