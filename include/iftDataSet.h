...
iftImage  *iftThreshold(   iftImage *img, int lowest, int highest, int value); // function declaration to avoid headers cross-reference with iftSegmentation.h
 ...
 * @return The dataset
 */
iftDataSet *iftSupervoxelsToHistogramDataSet(   iftImage *img,    iftImage *supervoxel, int nbins, iftIntArray *true_labels);
...
* @date May, 2016
*/
void iftSampleFileSet(iftFileSet *fileSet,    iftSampler *sampler, int iteration);
 ...
 */
//! swig()
void iftWriteDataSet(   iftDataSet *Z, const char *pathname, ...);
...
/**
 * @brief Creates a patch dataset from all seed voxels of all images
...
/**
 * @brief Creates a patch dataset from all seed voxels in a given image
 * using the corresponding activations around the seeds. The seed
 * labels are used as true labels of the samples. The true labels are
 * incremented by 1 if the labels start at 0.
 * @param S: labeled seed set
 * @param input: multichannel image 
 * @param A: Adjacency relation that represents the patch 
 * @return The resulting patch dataset 
 * @author Alexandre Falcao 
 * @date Sep 3rd, 2024
*/
//! swig(newobject, stable)
