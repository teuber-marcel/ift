...
#define UE_ARG "--undersegmentation-error"
#define ASA_ARG "--achievable-segmentation"
#define COMP_ARG "--compactness"
#define SPMETRICS_ARG "--standard-superpixel-metrics"
#define DICE_ARG "--dice"
#define TOP_ARG "--topology"
#define COVER_ARG "--covering"
#define VOI_ARG "--variation-of-information"
#define PRI_ARG "--probabilistic-rand-index" 
...
int main(int argc, char* argv[])
{
  ...
  bool doBR = iftDictContainKey(BR_ARG, args, NULL) || iftDictContainKey(SPMETRICS_ARG, args, NULL) || iftDictContainKey(SEGMETRICS_ARG, args, NULL);
  bool doBP = iftDictContainKey(BP_ARG, args, NULL) || iftDictContainKey(SEGMETRICS_ARG, args, NULL);
  ...
  // Compute groundtruth based metrics
    float boundaryTolerance = (iftDictContainKey(TOLERANCE_ARG, args, NULL)) ?
...
iftDict *iftGetArguments(int argc, const char *argv[]) {
  iftCmdLineOpt cmd_line_opts[] = {
    ...
    {.short_name = "-sp", .long_name = SPMETRICS_ARG, .has_arg = false,
      .required = false, .help = "Compute standard superpixel metrics."},
    ...
    {.short_name = "-ls", .long_name = SEGMETRICS_ARG, .has_arg = false,
      .required = false, .help = "Compute standard large segment metrics."}
  };
  int n_opts = sizeof(cmd_line_opts) / sizeof (iftCmdLineOpt);

  char program_description[IFT_STR_DEFAULT_SIZE] = 
    "This is a general demo to compute segmentation metrics.";
