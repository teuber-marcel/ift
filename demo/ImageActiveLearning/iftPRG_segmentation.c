...
int main(int argc, char* argv[])
{
  ...
  float alpha = (iftDictContainKey(ALPHA_ARG, args, NULL)) ?
    iftGetDblValFromDict(ALPHA_ARG, args) : 0.0f;
  float beta = 1.0; 
  ...
  //iftImage *superpixelLabelMap = iftSuperpixelSegmentationByPRG(igraph, nSp);
  timer *t2 = iftToc();
...
iftDict *iftGetArguments(int argc, const char *argv[]) {
  iftCmdLineOpt cmd_line_opts[] = {
    ...
    {.short_name = "-a", .long_name = ALPHA_ARG, .has_arg = true,
      ...
      .help = "Multiplicative weight to geometric distance."},
