...
{
  if ((error->tp+error->fn)<IFT_EPSILON)
    return(1.0);
  else
...
{
  if (error->fp+error->tn < IFT_EPSILON)
    return(1.0);
  else
...
float iftPrecisionGivenErrors(iftErrorClassification* error) {
  if ((error->tp + error->fp) < IFT_EPSILON)
    return(1.0);
  else
...
float iftAccuracyGivenErrors(iftErrorClassification* error) {
  if ((error->tp + error->fp + error->tn + error->fn) < IFT_EPSILON)
    return(1.0);
  else
...
{
  if (iftFalsePositiveRateGivenErrors(error) < IFT_EPSILON)
    return(1.0);
  else
...
{
  if ((error->fp<IFT_EPSILON)&&(error->fn<IFT_EPSILON))
    return(1.0);
  else{
    if ((error->fp<IFT_EPSILON)||(error->fn<IFT_EPSILON))
      return(0.0);
...
float iftCosineDistance2(float *f1, float *f2, int n) {
    ...
    v1_norm = sqrtf(v1_norm) + 0.01;
    v2_norm = sqrtf(v2_norm) + 0.01;

    float cos_sim = (float) v1_dot_v2 / (v1_norm*v2_norm);

    return (float) 0.5 - cos_sim/2.0;
}
...
int iftBestSampleScoreSum(  iftMatrix *score_matrix, iftMetricType metric_type, double *out_best_score) {
...
float iftKullbackLeiblerDivergence(  iftFloatArray *P,   iftFloatArray *Q) {
    if (P->n != Q->n)
...
double iftMImageMahalanobis(  iftMImage *mimg,   iftMatrix *M, int p, int q)
{
...
double iftMImageMahalanobisDouble(  iftMImage *mimg,   double *M, int p, int q)
{
    double *diff = iftAlloc(mimg->m, sizeof *diff);
    double *aux = iftAlloc(mimg->m, sizeof *aux);
