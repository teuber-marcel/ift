...
int main(int argc, char *argv[]) 
{
  ...
  if (argc!=2)
    iftError("Usage: iftCTreeVolumeOpen <image.[pgm,ppm,scn]>","main");

  img[0]   = iftReadImageByExt(argv[1]);    
  ctree    = iftCreateMaxTree(img[0]);
  ...
  while (opt) { 
    ...
    img[1] = iftVolumeOpen(img[0],thres,ctree);
    
    t2     = iftToc();
    fprintf(stdout,"Volume opening executed in %f ms\n",iftCompTime(t1,t2));
