...
FEAT_EXTRACTION_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/AttentionMap/extract_svoxel_bic_feats.py"
...
def main():
    ...
    if not args.skip_feat_extraction:
        ...
        print("\n\n####### Feature Extraction")
        feat_extract_cmd_args = f"--attention-map {reg_error_path} --supervoxels {svoxels_path} " \
                                f"--training-attention-map-set {args.training_reg_error_set} " \
                                f"--output-dir {datasets_dir} --n-bins-per-channel {args.n_bins_per_channel}"
