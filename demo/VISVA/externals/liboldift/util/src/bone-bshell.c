...
int main(int argc, char **argv)
{
  ...
  if (argc != 3) {
    fprintf(stderr,"usage: bonebshell <inputfile> <outputfile>\n");
    exit(-1);
  }
  ...
  /* Compute threholding, filtering and get bone mask */
   ...
   lower  = (int)(1.05*max[0]);
   fprintf(stdout,"lower %d higher %d\n",lower,higher);

   tmp  = Thres(scn,lower,higher);
   A = Spheric(1.8);
   bin  = MedianFilter3(tmp,A);
   DestroyScene(&tmp);
   DestroyAdjRel3(&A);
   sh = CreateBodyShell(scn,bin);
   SetShellLabel(sh,bin); 
   DestroyScene(&bin);
   DestroyScene(&scn);
   gettimeofday(&toc,NULL);
   fprintf(ftime,"BoneBShell in %f milliseconds\n",((toc.tv_sec-tic.tv_sec)*1000.0 + (toc.tv_usec-tic.tv_usec)*0.001));


   WriteBodyShell(sh,argv[2]);
   DestroyShell(&sh);
   DestroyScene(&bin);
