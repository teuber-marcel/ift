...
def transform_norm_asym_map(args):
    out_path = pjoin(args.output_dir, "normal_asym_map.nii.gz")
    ...
    def_fields = sorted(glob.glob(pjoin(args.output_dir, "TemplateToNativeDefFields.*")))
...
def native_SymmISF(args):
    ...
    inv_df = sorted(glob.glob(pjoin(args.output_dir, "InverseSymmObjectDefFields.*")))[-1]
...
def transform_supervoxels_on_template(args):
    symm_super_path = pjoin(args.output_dir, "n_supervoxels_%d" % args.number_of_supervoxels, "symm_supervoxels.nii.gz")
    out_path = pjoin(args.output_dir, "n_supervoxels_%d" % args.number_of_supervoxels, "symm_supervoxels_registered.nii.gz")
    df = sorted(glob.glob(pjoin(args.output_dir, "NativeToTemplateDefFields.*")))[-1]
...
def extract_haa_feats(args):
    img_path = args.test_image
    asym_map_path = pjoin(args.output_dir, "asym_map.nii.gz")
    mask_path = pjoin(args.output_dir, "mask.nii.gz")
    ...
    symm_super_template_path = pjoin(out_dir, "symm_supervoxels_registered.nii.gz")
    template_on_native_path = pjoin(args.output_dir, "template_on_native.nii.gz")
...
def classify_asymmetries(args):
    ...
    super_path = pjoin(out_dir, "filtered_symm_supervoxels.nii.gz")
