...
int iftGifPartition(uint8_t* image,    int left,    int right,    int elt, int pivotIndex);
...
// changed pixels only.
int iftGifPickChangedPixels(    uint8_t* lastFrame, uint8_t* frame, int numPixels );

// Creates a palette by placing all the image pixels in a k-d tree and then averaging the blocks at the bottom.
// This is known as the "modified median split" technique
void iftGifMakePalette(    uint8_t* lastFrame,    uint8_t* nextFrame, uint32_t width, uint32_t height, int bitDepth, bool buildForDither, iftGifPalette* pPal );

// Implements Floyd-Steinberg dithering, writes palette value to alpha
void iftGifDitherImage(    uint8_t* lastFrame,    uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, iftGifPalette* pPal );

// Picks palette colors for the image using simple thresholding, no dithering
void iftGifThresholdImage(    uint8_t* lastFrame,    uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, iftGifPalette* pPal );
...
void iftGifWritePalette(    iftGifPalette* pPal, FILE* f );
...
bool iftGifWriteFrame( iftGifWriter* writer,    uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth, bool dither);
//bool GifWriteFrame( GifWriter* writer,    uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth, bool dither);
