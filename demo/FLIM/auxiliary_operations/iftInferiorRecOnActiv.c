...
/* Author: Alexandre Xavier Falcão (December 30th, 2023)
   ...
   Description: Substitutes activations in a deeper layer L+K by the
   inferior reconstruction of the corresponding activations in layer L
   using the activations in layer L+K as marker image.
...
int main(int argc, char *argv[])
{
  ...
  if (argc!=3){ 
    iftError("Usage: iftInferiorRecOnActiv <P1> <P2>\n"
  ...
  for(int i = 0; i < fs->n; i++) {
	...
	if (weight[b]<0.0){
	  iftImage *aux1 = iftComplement(irec);
	  iftDestroyImage(&irec);
	  irec = aux1;
	}
	for (int p=0; p < irec->n; p++) {
	  mimg2->val[p][b] = (float)irec->val[p]/norm_value;		        
	}
	iftDestroyImage(&irec);
      }
      iftDestroyImage(&img1);
    ...
    // Should we overwrite the first or the last layer?
    //sprintf(filename,"%s/%s.mimg",first_dir,basename);
