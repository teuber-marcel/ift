...
iftImage *_iftExtract_ISF_MIX_ROOT_Superpixels(iftImage *img, int nsuperpixels, float alpha, float beta, int niters, int smooth_niters, int *nseeds, int *finalniters) {
...
int main(int argc, char *argv[]) 
{
  ...
  label     = _iftExtract_ISF_MIX_ROOT_Superpixels(img, atoi(argv[2]), atof(argv[3]), atof(argv[4]), atoi(argv[5]), atoi(argv[6]), &(nseeds), &(nautoiters));
  ...
  if (!iftIs3DImage(img)){
    ...
    YCbCr      = iftRGBtoYCbCr(RGB, normvalue);
    sprintf(labelfilename, "%s_label.png", argv[7]);
