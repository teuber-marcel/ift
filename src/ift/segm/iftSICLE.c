...
inline float iftSICLE_EuclDistance
...
inline float iftSICLE_FastNatPow
(float base, int exp)
{
    #if IFT_DEBUG
  assert(exp >= 0);
  ...
  for(int z = z0; z <= zf; z = (int)(z + zstride))
  {
	  for(int y = y0; y <= yf; y = (int)(y + ystride))
	  {
	    for(int x = x0; x <= xf; x = (int)(x + xstride))
	    {
      	...
      	s_voxel.x = x; s_voxel.y = y; s_voxel.z = z; s_voxel.t = 0;
  ...
  for(int vi_index = 0; vi_index < sicle->mimg->n; ++vi_index)
  {
    if(data->cost_map[vi_index] != IFTSICLE_BKGCOST)
    {
      ...
      { tstats->sal[vi_label] += sicle->sal[vi_index]; }
    	...
    	tstats->centr[vi_label][3] += vi_voxel.t;
 ...
 */
inline float iftSICLE_ConnFunction
	...
	int vj_index)
{
	...
	root_feat_dist = iftSICLE_EuclDistance(vi_root_feats, vj_feats, sicle->mimg->m);
	...
	{ 
		...
		arccost = iftSICLE_FastNatPow(arccost, args->adhr) + spat_dist;
		pathcost += arccost;
	}
  	...
  	for(long tj_index = 0; tj_index < tstats->num_trees; ++tj_index)
    {
    	if(iftBMapValue(tstats->adj[ti_index],tj_index))
    	{
    		double sal_grad, grad, dist;
	     	...
	     	num_adjs++; 
	      grad= iftSICLE_EuclDistance(tstats->feats[ti_index], tstats->feats[tj_index],
	                             tstats->num_feats);
	      dist= iftSICLE_EuclDistance(tstats->centr[ti_index], tstats->centr[tj_index],
	      											 tstats->num_dims);
