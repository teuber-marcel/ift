 ...
 */
void _iftValidateCmdLineOpts(int n_opts,   iftCmdLineOpt cmd_line_opts[]) {
    // EXCEPTION CHECKERS
    if (n_opts < 0)
 ...
 */
bool _iftHasRequiredOpts(  iftCmdLineParser *parser) {
 ...
 * @ingroup CmdLineParse
 */
bool _iftHasOptionalOpts(  iftCmdLineParser *parser) {
    for (int i = 0; i < parser->n_opts; i++)
        if (!parser->opts[i].required)
            return true;
 ...
 */
void _iftCheckRequiredCmdLineOpt(  iftCmdLineParser *parser,   iftDict *args) {
    for (int i = 0; i < parser->n_opts; i++) {
        if (parser->opts[i].required) {
