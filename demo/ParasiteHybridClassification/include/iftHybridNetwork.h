...
/**
 *  @brief Load image as input for the Vgg network.
 *  @author Felipe Galvao
 *  @date Nov, 2019
 *
 ...
 *  @param img Image to be fed into the network
 ...
 *
 *  @param net Neural Network 
 *  @param img_path Path to image being fed into the network
 *  @return Remaining slots in current batch
 */
...
/**
 *  @brief Pre-processes image for the neural network.
 *  @author Felipe Galvao
 *  @date Nov, 2019
 *
 *  @param origImg Original image
 *  @param labelImg Segmentation mask (ground truth)
 *  @return Image ready for the neural network
 */
iftImage * iftHybridNetworkPreProcImg(iftImage *origImg, iftImage *labelImg);

/**
 *  @brief Update root directory for the sample in a fileset.
 *  @author Felipe Galvao
 *  @date Nov, 2019
 *
 *  @param fs Target fileset to be updated
 *  @param newDir New root directory to be inserted into fileset paths
 */
void iftUpdateFileSetDir(iftFileSet *fs, char *newDir);

/**
 *  @brief Get predictions of current batch.
 *  @author Felipe Galvao
 *  @date Nov, 2019
 *
 *  @details Note that the network should be forwarded beforehand with
 *    iftNetForward(net) as this function only checks the last layer.
 *
 *  @param net Neural Network
 *  @param batchSamplePred Pre-allocated array where predictions will be saved 
 */
