...
ATTENTION_MAP_PROG = "iftMeanDiffMapWithImageSet"
...
OC_OPF_PROG = "iftClassifySVoxelDataSetsByOCOPF"

os.system(f"make {ATTENTION_MAP_PROG} -C $NEWIFT_DIR/demo/BrainAnomalyDetection/RegistrationErrorEDT")
os.system(f"make {OC_OPF_PROG} -C $NEWIFT_DIR/demo/BrainAnomalyDetection/AttentionMap")
...
'''
    ...
    parser.add_argument('-x', '--training-reg-error-set', type=str, required=True,
    ...
    parser.add_argument('-k', '--kmax', type=float, default=0.15,
                        help='OC-OPF KMax. Default: 0.15')
...
def print_args(args):
    ...
    print(f'- Train Set: {args.training_set}')
    ...
    print(f'- Training Set: {args.training_reg_error_set}')
...
def main():
    ...
    reg_error_path = os.path.join(working_dir, "reg_errors.nii.gz")

    if not args.skip_registration_error_computation:
        reg_errors_cmd = f"iftMeanDiffMapWithImageSet --image {args.image} " \
                         f"--train-image-entry {args.training_set} --output-map {reg_error_path} " \
                         f"--label-image-path {args.label_image}"
        if args.bias:
            reg_errors_cmd = f"{reg_errors_cmd} --bias {args.bias}"
        
        os.system(reg_errors_cmd)
    ...
    if not args.skip_feat_extraction:
        ...
        feat_extract_cmd_args = f"--test-reg-error-mag {reg_error_path} --test-supervoxels {svoxels_path} " \
                                f"--training-reg-error-mag-set {args.training_reg_error_set} " \
    ...
    print("\n\n####### One-Class OPF")
    ocopf_cmd_args = f"--supervoxels {svoxels_path} --supervoxels-datasets-entry {datasets_dir} " \
                     f"--output-anomalous-supervoxels-img {args.out_classification_mask} " \
                     f"--kmax {args.kmax}"
    
    os.system(f"{OC_OPF_PROG} {ocopf_cmd_args}")
