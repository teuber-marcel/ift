...
void iftGetInputParameters(int argc, const char *argv[], char **input_img_file, char **output_img_file, int *dx, int *dy, int *dz, iftAxisOrder *axis_order)
{
  ...
  iftCmdLineOpt cmd_line_opts[] = {
    {.short_name = "-i", .long_name = "--input-img", .has_arg=true, .required=true, .arg_type=IFT_STR_TYPE, .help="Input image for reslicing (.scn)"},
    ...
    {.short_name = "-o", .long_name = "--output-img", .has_arg=true, .arg_type=IFT_STR_TYPE, .required=true, .help="Output image (.scn)"},
  };
...
int main(int argc, const char *argv[]) 
{
  ...
  img  = iftReadImage(input_img_file);
  rimg = iftResliceImageSimple(img,dx,dy,dz,axis_order);
  iftDestroyImage(&img);

  t2 = iftToc();

  iftWriteImage(rimg,output_img_file);
