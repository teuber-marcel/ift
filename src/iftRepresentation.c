...
iftImage *iftColorMSSkel2D(iftImage *bin)
{
  ...
  float minval       = iftFMinimumValue(msskel);
  iftAdjRel *B       = iftCircular(1.0);
  iftImage *border   = iftFindAndLabelObjectBorders(bin, B);
  iftImage *render   = iftCreateImageFromImage(bin);
  iftSetCbCr(render,128);
  iftColor RGB, YCbCr; 
  float    delta     = 0.8f;
  ...
  for(int p=0; p < msskel->n; p++) {   
	...
	float val = (msskel->val[p]-minval)/(maxval-minval);
	val = (6-2*delta)*val+delta;
	RGB.val[0] = iftRound(255.0*
			      iftMax(0.0f,(3-(float)fabs(val-4)-
					   (float)fabs(val-5))/2));
	RGB.val[1] = iftRound(255.0*
			      iftMax(0.0f,(4-(float)fabs(val-2)-
					   (float)fabs(val-4))/2));
	RGB.val[2] = iftRound(255.0*
			      iftMax(0.0f,(3-(float)fabs(val-1)-
					   (float)fabs(val-2))/2));
      }
    }
    YCbCr = iftRGBtoYCbCr(RGB, 255);
    render->val[p] = YCbCr.val[0];
    render->Cb[p]  = YCbCr.val[1];
    render->Cr[p]  = YCbCr.val[2];
  }
...
iftImage *iftEuclDistTrans(  iftImage *label_img,   iftAdjRel *Ain, iftSide side, iftImage **root_out,
    ...
    iftAdjRel *A = NULL;
    if (Ain == NULL)
      A = (iftIs3DImage(label_img)) ? iftSpheric(sqrtf(3.0)) : iftCircular(sqrtf(2.0));
...
{
  iftAdjRel *A = iftIs3DImage(mask) ? iftSpheric(sqrtf(3.0)) : iftCircular(sqrtf(2.0));
...
{
  iftFImage *cum_image      = iftCreateFImage(integral_image->xsize,
					      integral_image->ysize,
					      integral_image->zsize);

  Wx = iftRound((float)Wx/2.0);
  Wy = iftRound((float)Wy/2.0);
  Wz = iftRound((float)Wz/2.0);
  
  if (iftIs3DFImage(integral_image)){
    iftVoxel pt[8];
    for (int p=0; p < integral_image->n; p++) {
      iftVoxel u = iftFGetVoxelCoord(integral_image,p);
      iftVoxel v;
      /* apply the displacement */
      v.x = u.x + Dx; v.y = u.y + Dy; v.z = u.z + Dz;
      /* compute vertices of the region centered at v */
      pt[0].x = v.x - Wx; pt[0].y = v.y - Wy; pt[0].z = v.z - Wz;
      pt[1].x = v.x + Wx; pt[1].y = v.y - Wy; pt[1].z = v.z - Wz;
      pt[2].x = v.x - Wx; pt[2].y = v.y + Wy; pt[2].z = v.z - Wz;
      pt[3].x = v.x + Wx; pt[3].y = v.y + Wy; pt[3].z = v.z - Wz;
      pt[4].x = v.x - Wx; pt[4].y = v.y - Wy; pt[4].z = v.z + Wz;
      pt[5].x = v.x + Wx; pt[5].y = v.y - Wy; pt[5].z = v.z + Wz;
      pt[6].x = v.x - Wx; pt[6].y = v.y + Wy; pt[6].z = v.z + Wz;
      pt[7].x = v.x + Wx; pt[7].y = v.y + Wy; pt[7].z = v.z + Wz;
      bool valid_point = true;
      for (int i=0; i < 8; i++){
	if (!iftFValidVoxel(integral_image,pt[i])){
	  valid_point = false;
	  break;
	}
      }
      if (valid_point) 
	cum_image->val[p] = iftGetIntegralValueInRegion(integral_image, pt, 8);
    }
  } else {
    iftVoxel pt[4];
    for (int i=0; i < 4; i++) pt[i].z = 0;
    for (int p=0; p < integral_image->n; p++) {
      iftVoxel u = iftFGetVoxelCoord(integral_image,p);
      iftVoxel v;
      /* apply the displacement */
      v.x = u.x + Dx; v.y = u.y + Dy; v.z = u.z + Dz;
      /* compute vertices of the region centered at v */
      pt[0].x = v.x - Wx; pt[0].y = v.y - Wy; 
      pt[1].x = v.x + Wx; pt[1].y = v.y - Wy; 
      pt[2].x = v.x - Wx; pt[2].y = v.y + Wy; 
      pt[3].x = v.x + Wx; pt[3].y = v.y + Wy; 
      bool valid_point = true;
      for (int i=0; i < 4; i++){
	if (!iftFValidVoxel(integral_image,pt[i])){
	  valid_point = false;
      ...
      if (valid_point) 
	cum_image->val[p] = iftGetIntegralValueInRegion(integral_image, pt, 4);
    }
  }
...
void iftObjectAreaFromPixel(  iftImage* comp, int index, iftImage *area_img)
{
...
float iftWeightedIntegralValue(  iftFImage *int_img, iftVoxel center, int width, int height, float weight)
{
    iftVoxel pt[4];
...
double iftPerpendicularDist(  iftVoxel *cur,   iftVoxel *first,   iftVoxel *last)
{
    if (first->x == last->x)
...
double iftPointToLineSqrDist(  iftVoxel *cur,   iftVoxel *first,   iftVoxel *last)
{
...
iftVoxel *iftRDP(  iftVoxel *array, long len, double epsilon, long *out_len)
{
      iftVoxel *first = &array[0], *last = &array[len - 1];
    if (len <= 2)
    {
        *out_len = 2;
...
void iftSplitByMaxDist(  iftVoxelArray *array, iftVoxelArray **out1, iftVoxelArray **out2)
{
    int max_i = -1, max_j = -1;
    double max_dist = 0;
