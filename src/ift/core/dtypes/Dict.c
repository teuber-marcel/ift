...
iftDict *iftCopyDict(  iftDict *dict) {
    iftDict *copy = NULL;
...
bool iftIsDictEmpty(  iftDict *dict) {
    if (dict == NULL)
...
bool iftIsDictFull(  iftDict *dict) {
    if (dict == NULL)
...
bool iftDictContainGKey(iftGVal key,   iftDict *dict, size_t *key_index) {
    if (dict == NULL)
...
void iftPrintDictAux(  iftDict *dict, int tab, int newline) {
    if (dict != NULL) {
        printf("{");
...
void iftPrintDictMinified(  iftDict *dict) {
    iftPrintDictAux(dict, -1000, -1000);
}


void iftPrintDictAsArray(  iftDict *dict) {
    if (dict != NULL) {
        if (!iftIsDictEmpty(dict)) {
...
iftGVal iftGetGValFromDict(iftGVal key,   iftDict *dict, iftCDataType val_type) {
    size_t h;
...
iftKeyVal iftDictFirst(  iftDict* dict) {
    return dict->table[dict->firstKey];
}


iftKeyVal iftDictNext(  iftDict* dict,   iftKeyVal keyval) {
...
bool iftDictValidKey(  iftKeyVal keyval) {
    return keyval.key.type != IFT_UNTYPED && keyval.val.type != IFT_UNTYPED;
}


iftDict *iftIntArrayToDict(  int *arr, size_t n) {
...
char *iftDictToStringMinified(  iftDict *dict) {
