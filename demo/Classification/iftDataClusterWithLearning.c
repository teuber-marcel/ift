...
int main(int argc, char *argv[]) 
{
  ...
  if (argc!=6)
      iftError("Usage: iftDataClusterWithLearning <dataset.zip> <nb_train_samples> <kmax_perc> <propagation_type(0:root_label/1:cluster_majority)> <debug(0:NO/1:YES)--flag to output messages and generate images with tsne>","main");
...
//  iftRandomSeed(IFT_RANDOM_SEED);
  ...
  t1      = iftTic();
  graph = iftUnsupLearn2(Z, atof(argv[3]), iftNormalizedCut, 100,debug);
