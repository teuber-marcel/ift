 ...
 */
void iftISFRunIFT(  iftISFOptions *options, iftIGraph *igraph,   iftImage *seeds);
 ...
 */
void iftForestTreeRemovalImplicitAdj(iftSuperpixelIGraph *igraph, iftSet *markedTrees,   double trivialPathCost, iftSet **removedNodes, iftSet **frontierNodes);
 ...
 */
void iftRemoveSubTreeImplicitAdj(iftSuperpixelIGraph *igraph,   int markedNode,   double trivialPathCost, iftSet **removedNodes, iftSet **frontierNodes);
 ...
 * @pre \c markedTrees is not empty.
 */
void iftForestTreeRemovalExplicitAdj(iftForestAnnotationInfo *ann, iftSet *markedTrees, iftSet **nodeAdj,   double trivialPathCost, iftSet **removedNodes, iftSet **frontierNodes);
 ...
 * @pre \c markedNode is valid.
 */
void iftRemoveSubTreeExplicitAdj(iftForestAnnotationInfo *ann,   int markedNode, iftSet **nodeAdj,   double trivialPathCost, iftSet **removedNodes, iftSet **frontierNodes);
 ...
 * @param[in] igraph Working superpixel graph.
 */
void iftUpdateSuperpixelIGraphMeanAuxFeat(  iftSuperpixelIGraph *igraph);
 ...
 */
iftSet * iftRISFMedoidSeedRecomp(  iftSuperpixelIGraph *igraph, bool isParametric);
 ...
 */
void iftSeedUpdateThreshold(  iftSuperpixelIGraph *igraph, iftSet *seeds);

/**
 * @brief Build a mask with only the medoid of each superpixel.
 */
iftImage * iftSelectSuperpixelCenters(  iftMImage *mimg,   iftImage *label);

/**
 * @brief Convert an IGraph generated from superpixels into label image.
 */
iftImage * iftHierarchyIGraphLabel(iftIGraph *igraph,   iftImage *predLabel);
 ...
 */
double iftRISFGetPathCost(  iftSuperpixelIGraph *igraph,   int s,   int t,   iftISFOptions *optISF);
 ...
 * @pre \c s and \c are non-negative (i.e. valid nodes).
 */
int iftRISFOfferNewPath(iftSuperpixelIGraph *igraph,   int s,   int t,   iftISFOptions *optISF);

/**
 * @author Felipe Lemes Galvao
 * @date December 4, 2017
 */
void iftRISFInitializeDIFT(iftSuperpixelIGraph *igraph,   iftISFOptions *opt, iftDHeap *Q, double trivialPathCost);
 ...
 * @date December 4, 2017
 */
void iftRISFNeighborProcessingDIFT(iftSuperpixelIGraph *igraph, iftDHeap *Q, double trivialPathCost, int s, int t,   iftISFOptions *opt);
 ...
 */
iftSet * iftRISFSampling(  iftSuperpixelIGraph *igraph,   iftISFOptions *opt,   iftImage *img);
 ...
 */
iftSet * iftRISFGridSampling(  iftSuperpixelIGraph *igraph,   iftImage *img, int nSamples);
 ...
 * @pre \c nSamples is within (0, igraph->nNodes).
 */
iftSet * iftRISFMixedSampling(  iftSuperpixelIGraph *igraph,   iftImage *img, int nSamples);
 ...
 * @pre \c nSamples is within (0, igraph->nNodes).
 */
iftSet * iftRISFRandomSampling(  iftSuperpixelIGraph *igraph, int nSamples);
 ...
 * @pre \c igraph is not NULL.
 */
iftSet * iftRISFAllNodesSampling(  iftSuperpixelIGraph *igraph);
 ...
 * @pre \c nSamples is within (0, igraph->nNodes).
 */
iftSet * iftRISFGeodesicSampling(  iftSuperpixelIGraph *igraph,   iftImage *img, int nSamples);
 ...
 * @pre \c nSamples is within (0, igraph->nNodes).
 */
iftSet * iftRISFGridOnMaskSampling(  iftSuperpixelIGraph *igraph, int nSamples);
 ...
 * @pre \c nSamples is within (0, igraph->nNodes).
 */
iftSet * iftRISFHexGridSampling(  iftSuperpixelIGraph *igraph, int nSamples);
 ...
 */
iftSuperpixelIGraph * iftBuildGeodesicSamplingIGraph(  iftSuperpixelIGraph *igraph,   iftImage *img);
...
void iftRISFSmoothing(iftSuperpixelIGraph *igraph,   iftImage *img, int nSmoothIters)
{
...
iftSet * iftRISFGeodesicSampling(  iftSuperpixelIGraph *igraph,   iftImage *img, int nSamples)
{
  const char * funName = "iftRISFGeodesicSampling";
    double trivialPathCost = IFT_INFINITY_DBL;
    double seedPathCost = 0.0;
  assert(igraph != NULL);
  assert(nSamples > 0 && nSamples < igraph->nNodes);

  // Debug/visualization only
  //iftImage *heatMap = iftCopyImage(igraph->refImg);
  ...
  for (int iter = 0; iter < nSamples; ++iter) {
    ...
    // Debug: sanity test if R is working as intended
    /*for (int j = 0; j < geograph->nNodes; ++j) {
      if (R->color[j] == IFT_BLACK)
        continue;
      float gog = geograph->ann->pathValue[j];
      float geg = geograph->ann->pathValue[seeds->elem];
      if (gog > geg)
        printf("Iter %d: Node %d with cost %f should've been removed instead of node %d with cost %f.\n", iter, j, gog, seeds->elem, geg);
    }*/

    //timer *t2 = iftToc();
    //printf("Iter %d time = %fms\n", iter, iftCompTime(t1,t2));

    // Debug only, pathVal image
    /*for (int p = 0; p < heatMap->n; ++p) {
      int label = geograph->refImg->val[p] - 1;
      heatMap->val[p] = (geograph->ann->pathValue[label] * 255.0) /
        geograph->ann->pathValue[seeds->elem];
    }
    iftWriteImageByExt(heatMap, "heatmap_%03d.pgm", iter+1);*/
  }
  //timer *t2f = iftToc();
  //printf("Total processing time = %fms\n", iftCompTime(t1f,t2f));

  iftDestroyDHeap(&Q);
  iftDestroySuperpixelIGraph(&geograph);

  iftInvertSet(&seeds); // Insert adds to the head, inverting result
  return seeds;
}
...
iftSuperpixelIGraph * iftBuildGeodesicSamplingIGraph(  iftSuperpixelIGraph *igraph,   iftImage *img)
{
  ...
  // Uncomment if igraph will strictly have color mean features
  // return geograph;
...
void iftGeodesicSamplingNeighborProcessing(iftSuperpixelIGraph *igraph, iftDHeap *Q, iftDHeap *R, int s, int t)
{
  // This optimization requires more information with the way we are
  //   using DIFT. May check later but for now the algorithm is fast
  //   enough.
  //if (Q->color[t] == IFT_BLACK)
  //  return;
  ...
  /*double distCost = iftFeatDistance(
    ...
    iftMatrixRowPointer(igraph->pos, t),
    igraph->pos->ncols);*/
  double cost = pCost + igraph->spSize[s] * pCost;
...
void iftGeodesicSamplingRemoveInconsistentSubTree(iftSuperpixelIGraph *igraph, iftDHeap *Q, iftDHeap *R, int t)
{
  ...
  // We divide by 2 to avoid conflict with iftRemoveDHeapElem on R
    double trivialPathCost = IFT_INFINITY_DBL / 2.0;
...
iftForestAnnotationInfo * iftCopyForestAnnotationInfo(  iftForestAnnotationInfo *ann)
{
  assert(ann != NULL);
...
void iftInitForestAnnotationInfo(iftForestAnnotationInfo *ann,   double trivialPathCost, bool resetLabelData)
{
  assert(ann != NULL);
  if (resetLabelData)
    ann->nLabels = 0;
...
void iftInitForestAnnotationSingleSeed(iftForestAnnotationInfo *ann, int seed,   double seedPathCost,   bool createNewLabels)
{
  assert(ann != NULL);
...
void iftInitForestAnnotationSeeds(iftForestAnnotationInfo *ann, iftSet *seeds,   double seedPathCost,   bool createNewLabels)
{
...
iftSuperpixelIGraph *iftInitSuperpixelIGraph(  iftImage *refImg)
{
  assert(refImg != NULL);
...
iftSuperpixelIGraph *iftCopySuperpixelIGraph(  iftSuperpixelIGraph *igraph)
{
  assert(igraph != NULL);
...
void iftSetSuperpixelIGraphExplicitRAG(iftSuperpixelIGraph *igraph,   iftAdjRel *A)
{
  assert(igraph != NULL);
  assert(A != NULL);
...
void iftSetSuperpixelIGraphFeatures(iftSuperpixelIGraph *igraph,   iftMatrix *superpixelFeatures, iftArcWeightFun distFun, float alpha, float beta)
{
  // Clear any old data
...
iftImage * iftSuperpixelSegmentationByRISF(iftSuperpixelIGraph *igraph,   iftISFOptions *opt,   iftImage *img)
{
  ...
  for (int iter = 0; iter < nIters; ++iter) {
    ...
    //timer *tIFTInit2 = iftToc();
    //printf("IFT Init:%fms\n", iftCompTime(tIFTInit1, tIFTInit2));
  ...
  // Smoothing step
  // TODO remove dependency with original igraph
...
iftImage * iftSuperpixelSegmentationByRISFwithoutDIFT(iftSuperpixelIGraph *igraph,   iftISFOptions *opt,   iftImage *img)
{
  const char *funName = "iftSuperpixelSegmentationByRISF";
  assert(igraph != NULL);
  assert(opt != NULL);
...
iftMatrix * iftComputeSuperpixelFeaturesByGeometricCenter(  iftImage *superpixelLabelMap)
{
  assert(superpixelLabelMap != NULL);

  // Build an MImage containing voxel coordinates as features
    iftImage *map = superpixelLabelMap; // shorthand
...
iftMatrix * iftComputeSuperpixelFeaturesByMImageMean(  iftMImage *mimg,   iftImage *superpixelLabelMap)
{
