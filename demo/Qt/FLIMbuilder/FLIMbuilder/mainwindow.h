...
public:
    ...
    void extractActivationsFromGraph(int layer_index);
    ...
    int* getKernelsType(int num_bands,
                        int layer,
                        QString baseName);

    void SetKernelsType(QListWidget *currList,
                        int newType,
                        int *kernel_type);

    void writeKernelsTypeFile(int *kernel_type,
                              int num_bands,
                              int layer,
                              QString baseName);
    bool prepareGFLIM();
    void initKernelsType(int layer);
    void loadTrainImages(QString training_path);
    void loadMarkers(QString open_path);
    void LoadGraphs(QString graphs_path, QString labels_path);
    void LoadModel(QString model_path);
    void LoadArch(QString arch_path);

    bool areFilesMissingForFLIM(QSet<QString> listOfNewMarkers, QString activ_path);
    void deleteModel();
    void writeGFLIMConfigFile(int num_init_seeds, int num_nodes);
    void readGFLIMConfigFile();
...
private slots:
    ...
    void on_cbKernelsType_currentIndexChanged(int i);
    void on_tabWidgetCurrentIndexChanged(int i);
    ...
    void on_cbFLIMVariation_currentIndexChanged(int index);

    void on_pbOpenGraphs_clicked();
    void on_pbRemoveGraphs_clicked();
    void on_pbSaveGraphs_clicked();
    void on_pb_config_gflim_clicked();
