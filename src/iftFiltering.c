    ...
    for (int p = 0; p < mimg->n; p++) {
        ...
        for (int i = 0; i < A->n; i++) {
            ...
            if (iftMValidVoxel(mimg, v)) {
              ...
              for (int b=0; b< mimg->m; b++){
                iftMatrixElem(matrix, b + i*mimg->m, p) = fill_band_with[b];
              }
            } 
        }
    }
...
iftImage *iftMeanFilter(  iftImage *img,   iftAdjRel *Ain) {
    iftAdjRel *A = NULL;
    if (Ain == NULL)
...
iftImage *iftMedianFilter(  iftImage *img, iftAdjRel *Ain) {
    iftAdjRel *A = NULL;
    if (Ain == NULL)
        A = (iftIs3DImage(img)) ? iftSpheric(1.0) : iftCircular(1.0);
    else A = iftCopyAdjacency(Ain);
...
iftFImage *iftFMedianFilter(iftFImage *img, iftAdjRel *A)
{
	int       n,j,i,p,q;
	float 		*value;
	iftVoxel   u,v;
	...
	value = iftAllocFloatArray(A->n);
	med   = iftCreateFImage(img->xsize,img->ysize,img->zsize);
...
void iftFastBilateralFilter2DAux(  float * __restrict__ _input,
    ...
    int sw = (width -1) / s_sigma + 1 + (2 * padding_xy);
    int sh = (height-1) / 	 + 1 + (2 * padding_xy);
    int depth = (int)(1.0f / r_sigma) + 1 + (2 * padding_z);
  int err;
  ...
  for(int y = 0; y < height; y++)
    for(int x = 0; x < width; x++)
      for(int c = 0; c < channels; c++) {
          ...
          int small_x = (int)((float)(x) / s_sigma + 0.5f) + padding_xy;
          int small_y = (int)((float)(y) / s_sigma + 0.5f) + padding_xy;
          int small_z = (int)((float)(z) / r_sigma + 0.5f) + padding_z;
...
iftMImage *iftMConvolution(iftMImage *mimg, iftAdjRel *A, iftMatrix *kernels, int stride)
{
...
iftMImageArray *iftMConvolutionArray(iftMImageArray *mimgArray,
				     iftAdjRel *A,
				     iftMatrix *kernels,
				     int stride)
{
  ...
  int     nkernels          = kernels->ncols;
  iftMatrix *feats          = iftMImageArrayToFeatureMatrix(mimgArray, A);
  iftMatrix *conv           = iftMultMatrices(feats, kernels);
  iftDestroyMatrix(&feats);
  iftMImageArray *convArray = iftMatrixToMImageArray(conv, mimgArray->n,
						     mimg0->xsize,
						     mimg0->ysize,
						     mimg0->zsize,
						     nkernels, 'c');
  iftMImageArray *mimgArrayOut = convArray;
  iftDestroyMatrix(&conv);
  
  if (stride > 1){
    mimgArrayOut = iftCreateMImageArray(mimgArray->n);

    for (int i = 0; i < mimgArray->n; i++) {
      mimgArrayOut->val[i] = iftCreateMImage(ceilf(mimg0->xsize/(float)stride),
					     ceilf(mimg0->ysize/(float)stride),
				      iftMax(ceilf(mimg0->zsize/(float)stride),
					     1),
					     convArray->val[i]->m);
      int q = 0;
      iftVoxel u;
      for (u.z = 0; u.z < convArray->val[i]->zsize; u.z = u.z + stride){
	for (u.y = 0; u.y < convArray->val[i]->ysize; u.y = u.y + stride){
	  for (u.x = 0; u.x < convArray->val[i]->xsize; u.x = u.x + stride){
	    int p = iftMGetVoxelIndex(convArray->val[i], u);
	    for (int b = 0; b < convArray->val[i]->m; b++)
	      mimgArrayOut->val[i]->val[q][b] =
		convArray->val[i]->val[p][b];
	    q++;
	  }
	}
      }
...
iftMImage *iftMReLU(iftMImage *mimg, float *bias, bool inplace)
{
  iftMImage *relu=NULL;
...
#pragma omp parallel for
      for (int p = 0; p < mimg->n; p++) {
	for (int b = 0; b < mimg->m; b++) {
	  mimg->val[p][b] = iftMax(0, mimg->val[p][b]);
	}
      }
    } else {
#pragma omp parallel for
      for (int p = 0; p < mimg->n; p++) {
	for (int b = 0; b < mimg->m; b++) {
	  mimg->val[p][b] = iftMax(0, mimg->val[p][b]+bias[b]);
	}
      }
    }
    relu = mimg;
  } else { 
    relu = iftCreateMImage(mimg->xsize, mimg->ysize, mimg->zsize, mimg->m);
    if (bias == NULL){    
#pragma omp parallel for
      for (int p = 0; p < mimg->n; p++){
	for (int b = 0; b < mimg->m; b++){
...
#pragma omp parallel for
      for (int p = 0; p < mimg->n; p++){
	for (int b = 0; b < mimg->m; b++){
	  relu->val[p][b] = iftMax(0, mimg->val[p][b]+bias[b]);
	}
      }
    }
  }
...
iftMImageArray *iftMReLUArray(iftMImageArray *mimgArray, float *bias,
			      bool inplace)
{
  iftMImageArray *mimgArrayOut = mimgArray;

  if (!inplace)
    mimgArrayOut = iftCreateMImageArray(mimgArray->n);
  
  for(int i = 0; i < mimgArray->n; i++){
    mimgArrayOut->val[i] = iftMReLU(mimgArray->val[i], bias, inplace);
  }
    
  return mimgArrayOut;
}

iftMImage *iftMPooling(iftMImage *mimg, char *pool_type,
		       iftAdjRel *A, int stride)
{
  iftMImage *pool = NULL;
  
  if (strcmp(pool_type, "no_pool") != 0){
    pool = iftCreateMImage(mimg->xsize,mimg->ysize,mimg->zsize,mimg->m);
    if (strcmp(pool_type, "avg_pool") == 0) {
#pragma omp parallel for
      for (int p = 0; p < mimg->n; p++) {
        iftVoxel u = iftMGetVoxelCoord(mimg, p);
        for (int i = 0; i < A->n; i++) {
      ...
      if (strcmp(pool_type, "max_pool") == 0) { 
#pragma omp parallel for
	for (int p = 0; p < mimg->n; p++) {
	  iftVoxel u = iftMGetVoxelCoord(mimg, p);
	  for (int i = 0; i < A->n; i++) {
  ...
  if (pool == NULL)
    pool = iftCopyMImage(mimg);

  if (stride > 1){
    iftMImage *mimgRet = iftCreateMImage(ceilf(mimg->xsize/(float)stride),
					 ceilf(mimg->ysize/(float)stride),
			       iftMax(ceilf(mimg->zsize/(float)stride),1),
					 mimg->m);
    int q = 0;
    iftVoxel u;
    for (u.z = 0; u.z < pool->zsize; u.z = u.z + stride) {
...
iftMImageArray *iftMPoolingArray(iftMImageArray *mimgArray,
				 char *pool_type,
				 iftAdjRel *A,
				 int stride)
{
  ...
  for(int i = 0; i < mimgArray->n; i++){
    mimgArrayOut->val[i] = iftMPooling(mimgArray->val[i], pool_type, A, stride);
  }
