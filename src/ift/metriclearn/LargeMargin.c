...
iftImpostorSet *iftCopyImpostorSet(  iftImpostorSet *S)
{
    iftImpostorSet *C = NULL;
    for (  iftImpostorSet *s = S; s != NULL; s = s->next) {
...
int iftImpostorSetSize(  iftImpostorSet *S)
{
    int i = 0;
    for (  iftImpostorSet *s = S; s != NULL; s = s->next) {
        i++;
...
void iftCheckNumberOfTargets(  int *label, int n,  int *k_targets)
{
...
int *iftSelectTargetNeighbors(  double *data,   int *label, int n, int d, int k_targets)
{
...
iftImpostorSet *iftFindImpostors(  double *Ldata,   int *label,   int *targets, int n, int d, int k_targets)
{
...
iftImpostorSet *iftFindKImpostors(  double *Ldata,   int *label,   int *targets,
...
iftImpostorSet *iftFindPartialImpostors(  iftImpostorSet *old_imp,   double *Ldata, int d, iftImpostorSet **extra)
{
    iftImpostorSet *S = NULL;
    for (  iftImpostorSet *s = old_imp; s != NULL; s = s->next)
    {
        int i = s->example;
        int tk = s->target;
        int ik = s->impostor;
...
double *iftUpdateTransform(  double *grad_imp,   double *grad_target,   double *L, double learn_rate, int d, int d_out)
{
...
double *iftTargetGradient(  double *data,   int *targets, int n, int d, int k_targets)
{
...
double *iftImpostorGradient(  double *data,   iftImpostorSet *S, int d)
{
    double *grad = iftAlloc(d * d, sizeof *grad);
...
double *iftPartialImpGradient(  double *old_grad,   double *data,   iftImpostorSet *missing,
                                iftImpostorSet *extra, int d)
{
    double *grad = iftAlloc(d * d, sizeof *grad);
    ...
    for (int di = 0; di < d; di++)
    {
        ...
        for (int dj = 0; dj < d; dj++)
        {
            for (  iftImpostorSet *s = missing; s != NULL; s = s->next)
            {
                int i = s->example;
                int tk = s->target;
                int ik = s->impostor;
            ...
            for (  iftImpostorSet *s = extra; s != NULL; s = s->next)
            {
                int i = s->example;
                int tk = s->target;
                int ik = s->impostor;
...
// L_row = d_out, L_col = d
double iftGradientLoss(  double *grad,   double *L, int L_nrow, int L_ncol)
{
...
int iftCompareImpostors(  iftImpostorSet *a,   iftImpostorSet *b)
{
...
void iftImpostorsDiff(  iftImpostorSet *imp,   iftImpostorSet *next_imp,
                      iftImpostorSet **missing, iftImpostorSet **extra)
{
      iftImpostorSet *old = imp;
      iftImpostorSet *new = next_imp;
...
int *iftSelectTargetKLMCA(  double *gram,   int *label, int n, int k_targets)
{
    int *targets = iftAlloc(k_targets * n, sizeof *targets);
    double *t_dist = iftAlloc(k_targets * n, sizeof *t_dist);
    double *max_dist = iftAlloc(n, sizeof *max_dist);
...
double *iftGradientKLMCA(  double *omega,   double *gram, iftImpostorSet **imp,   int *targets,
...
double *iftUpdateOmega(  double *omega,   double *grad, int n, int dim_out, double learn_rate)
{
...
iftImpostorSet **iftFindImpKLMCA(  double *Ldata,   int *label,   int *targets,
