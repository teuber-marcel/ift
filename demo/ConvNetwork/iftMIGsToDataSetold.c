...
void iftMIGsToDataSet(char *input_dir, iftConvNetwork *convnet, char *output_dir)
{
  ...
  for (int s = 0; s < number_of_images ; s++){
    ...
    for(int b = 0; b < input->m; b++){
		for(int p = 0; p < input->n; p++){
			Z->sample[s].feat[p + b*input->n] = input->val[p][b];
		}
	}
    Z->nclasses = iftCountNumberOfClasses(image_names, number_of_images);
    sprintf(filename,"%s/%s",output_dir,"dataset.data");
    iftWriteOPFDataSet(Z, filename);
...
int main(int argc, char **argv) 
{
  ...
  t1 = iftTic();
  iftMIGsToDataSet(argv[1], convnet, argv[3]);
  t2 = iftToc();
