...
iftDoubleMatrix* iftCopyDoubleMatrix( iftDoubleMatrix* m){
...
void iftWriteMatrixCSV(iftMatrix* M,  char* filename) {
...
iftMatrix *iftReadMatrixCSV( char* filename) {
...
iftBoolMatrix *iftReadBoolMatrix( char *filename)
{
    FILE *fp = fopen(filename, "rb");
...
iftMatrix *iftMultMatrices( iftMatrix *A,  iftMatrix *B) {
    ...
    if (A->ncols != B->nrows) {
      iftError("Cannot multiply matrices (%d,%d) * (%d,%d)", "iftMultMatrices", A->nrows, A->ncols, B->nrows,B->ncols);
    }
...
void iftMultMatricesInPlace( iftMatrix *A,  iftMatrix *B, bool transposeA, bool transposeB, iftMatrix **C) {
...
iftVoxel iftTransformVoxel(iftMatrix *M, iftVoxel v) {
    iftVoxel vt;
...
iftMatrix *iftInvertMatrix( iftMatrix *A) {
    ...
    if (A->ncols != A->nrows){
      iftError("Matrix %d x %d is not square", "iftInvertMatrix",A->ncols,A->nrows);
    }
...
iftMatrix *iftInvertDiagonalMatrix( iftMatrix *A) {
    if (A->ncols != A->nrows)
...
iftMatrix *iftPseudoInvertMatrix( iftMatrix *A) {
...
iftDoubleMatrix *iftPseudoInvertDMatrix( iftDoubleMatrix *A) {
...
char *iftMatrixAsString( iftMatrix *mat) {
...
char *iftStrMatrixAsString( iftStrMatrix *mat) {
    char *str = iftCopyString("[");
    char *aux = NULL;
...
iftMatrix *iftLeastSquares( iftMatrix *A,  iftMatrix *B) {
...
iftMatrix *iftMatrixMeanColumn( iftMatrix *matrix) {
...
iftMatrix *iftMatrixSumColumn( iftMatrix *matrix) {
...
iftMatrix *iftMatrixSumRow( iftMatrix *matrix) {
...
iftMatrix *iftMatrixMaxColumn( iftMatrix *matrix) {
...
iftMatrix *iftMatrixMaxRow( iftMatrix *matrix) {
...
float iftMatrixMaxOneColumn( iftMatrix *matrix, int c) {
...
float iftMatrixMaxOneRow( iftMatrix *matrix, int r) {
...
float iftMatrixMinOneColumn( iftMatrix *matrix, int c) {

    float minVal = IFT_INFINITY_FLT;
    for (int i = 0; i < matrix->nrows; i++) {
        if(iftMatrixElem(matrix, c, i) < minVal)
            minVal = iftMatrixElem(matrix, c, i);
    }

    return minVal;
}

float iftMatrixMinOneRow( iftMatrix *matrix, int r) {

    float minVal = IFT_INFINITY_FLT;
    for (int j = 0; j < matrix->ncols; j++) {
        if(iftMatrixElem(matrix, j, r) < minVal)
            minVal = iftMatrixElem(matrix, j, r);
    }
...
double iftMatrixSum( iftMatrix *matrix) {
    long i;
    double sum = 0.0;
...
iftMatrix *iftCovarianceMatrix( iftMatrix *X) {
    iftMatrix *cov, *Xt;
...
void iftSumMatricesInPlace( iftMatrix *m1,  iftMatrix *m2, iftMatrix *out) {
...
iftMatrix *iftSumMatrices( iftMatrix *m1,  iftMatrix *m2) {
...
void iftSubtractMatricesInPlace( iftMatrix *m1,  iftMatrix *m2, iftMatrix *out) {
...
iftMatrix *iftSubtractMatrices( iftMatrix *m1,  iftMatrix *m2) {
...
bool iftIsMatricesEqual( iftMatrix *A,  iftMatrix *B) {
    if (A->ncols != B->ncols)
        return false;    
    if (A->nrows != B->nrows)
        return false;    
...
iftMatrix *iftCentralizeMatrix( iftMatrix *M) {
...
iftMatrix *iftUnitNormMatrix( iftMatrix *M) {
