...
int main(int argc, const char *argv[]) {
    ...
    char *df_basename = iftJoinPathnames(2, out_dir, "NativeDefFields");
    iftFileSet *df = NULL;
    iftImage *template_native = iftRegisterImageByElastix(template_img, test_img, NULL, NULL, elastix_params,
                                                          df_basename, &df);
    iftWriteImageByExt(template_native, iftJoinPathnames(2, out_dir, "template_on_native.nii.gz"));
...
iftDict *iftGetArgs(int argc, const char *argv[]) {
    char program_description[2048] = \
        "- Register a template to a testing image.\n" \
