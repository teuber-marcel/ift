...
//    if (mode == HALT_ANNOTATION){
    ...
    if ((v.x != -1) || (v.y != -1) || (v.z != -1)){
        ...
        view->setCurrentSagittalSliceNum(-v.x+1+view->getXsize());
         ...
         iftImage *label = view->getLabel();
        int p = iftGetVoxelIndex(label, v);

        int label_value = label->val[p];

        if (label_value != 0)
            emit labelClicked(label_value);
...
{
     iftImage *projection = view->getProjection();
...
void RenderingView::showProjection()
{
    ...
    iftWriteOnLog("Computing Projection", 0);
     ...
     iftImage *aux = view->getProjection();
