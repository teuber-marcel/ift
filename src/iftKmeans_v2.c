...
void iftKMeans(iftDataSet *Z, int n_clusters, int max_iter, float tol, int *init_centroids, iftCentroidsHist *centroids_hist, iftKmeansDistance distance) {
  ...
  if (distance == NULL){
    ...
    if (S[0] != NULL) {
      iftSetDataSetPrototypeStatus(Z, S, distance);
    } else {
...
void iftSetDataSetPrototypeStatus(iftDataSet *Z, iftDynamicSet **S, iftKmeansDistance distance) {
    float min_distance, distance_value;
    int min_sample, s;
    int n_clusters = Z->ngroups;
    int n_features = Z->nfeats;
    iftSet *aux = NULL;
    ...
    for (int i = 0; i < n_clusters; i++) {
        ...
        while (aux != NULL) {
            ...
            distance_value = distance(mean, Z->sample[s].feat, n_features);
