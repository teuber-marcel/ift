...
void  iftImageSequenceFreeVolume(iftImageSequence *img, int t) {
    if (img->allocated[t]) {
        for (int z = 0; z < img->zsize; z++) {
            for (int y = 0; y < img->ysize; y++) {
                for (int x = 0; x < img->xsize; x++) {
                    iftFree(img->val[t][z][y][x]);
                }
                iftFree(img->val[t][z][y]);
            }
            iftFree(img->val[t][z]);
        }
        iftFree(img->val[t]);
        img->val[t] = NULL;
        img->allocated[t] = false;
    }
}
...
inline char ift4DValidVoxel(  iftImageSequence *img, iftVoxel v) {
...
iftImageSequence *
iftReadImageAsImageSequence(const char *filename,   iftImageSequence *imgseq_template,
                            int current_time) {
...
inline bool iftIsImageCompatibleToImageSequence(  iftImageSequence *img,
                                         iftImage *vol) {
...
iftImageSequence *
iftCreateImageSequenceFromImageSequence(  iftImageSequence *img) {
...
iftImageSequence *ift4DObjectBorders(  iftImageSequence *label_img,   iftAdjRel *Ain, bool keep_border_labels,
...
iftImageSequence *ift4DFastLabelComp(  iftImageSequence *bin,   iftAdjRel *Ain) {
    iftAdjRel *A = NULL;
    if (Ain == NULL) {
...
void ift4DMinMaxValues(  iftImageSequence *img, int band, float *min,
                       float *max) {
...
iftImageSequence *
ift4DBorderImage(  iftImageSequence *label, bool get_margins) {
    iftAdjRel *A;
...
iftImageSequence *
ift4DThreshold(  iftImageSequence *img, float lowest,
...
iftImageSequence *
ift4DThresholdInTime(  iftImageSequence *img, float lowest,
...
long
iftCountObjectSpelsImageSequence(  iftImageSequence *img, int obj_label) {
    long sum = 0;
    for (int t = 0; t < img->tsize; t++) {
