...
void iftPrintDList(  struct _ift_dlist *DL) {
    if (DL != NULL) {
        iftDNode *node = NULL;
...
void iftPrintReversedDList(  struct _ift_dlist *DL) {
    if (DL != NULL) {
        iftDNode *node = NULL;
        printf("Number of nodes: %d\n", DL->n);
...
iftDblArray *iftDListToDblArray(  iftDList *DL) {
...
iftDList *iftDblArrayToDList(  iftDblArray *arr) {
