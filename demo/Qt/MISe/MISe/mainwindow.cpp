...
#include <segmentation/controlled/controlled.h>
#include <segmentation/gradient/server/serverprocessing.h>
    ...
    ui(new Ui::MainWindow)
{
    ...
    ArcWeightFunction *server    = new ServerProcessing(this);
    ArcWeightFunction *brain     = new Brain(this);
    ...
    ui->cbGradient->addItem(server->name(),   QVariant::fromValue(server));
    ui->cbGradient->addItem(brain->name(),   QVariant::fromValue(brain));
    ...
    connect(server, SIGNAL(update()), this, SLOT(slotResetGradient()));
    connect(brain, SIGNAL(update()), this, SLOT(slotResetGradient()));


    // Load segmentation methods
    SemiAutomatic *interactive      = new SemiAutomatic(this, view);
    Controlled *controlled      = new Controlled(this, view);
    ...
    AutoECE  *auto_ece              = new AutoECE(this, view);


    segmentationViews.append(interactive);
    segmentationViews.append(controlled);
    segmentationViews.append(altis);
    segmentationViews.append(auto_ece);
    ...
    ui->cbSegmentation->addItem(auto_ece->name(),    QVariant::fromValue(auto_ece));
    ...
    ui->cbSegmentation->addItem(controlled->name(),  QVariant::fromValue(controlled));
    ...
    // Initialize post processing window
    postProcessingWindow = PostProcessing::instance(this);
    postProcessingWindow->setWindowFlags(Qt::WindowStaysOnTopHint | Qt::Window);
    ...
    if (QFile::exists(imgs[0])) {
        ...
        for (QString img_path: imgs) {
            if (QFileInfo(img_path).suffix() == "imsq") {
                ...
                iftImage *img = NULL;
                if (QFileInfo(img_path).suffix() == "mimg") {
                    iftMImage *mimg = iftReadMImage(img_path.toUtf8().constData());
                    qDebug() << mimg->zsize;
                    img = iftMImageToImage(mimg, 255, 0);
                    iftDestroyMImage(&mimg);
                } else
                    img = iftReadImageByExt(img_path.toUtf8().constData());
                if (imgseq == nullptr) {
        ...
        QMessageBox::warning(this, "File not found!", "Could not find the file " +imgs[0]+ ".");
    }
    return;
}
...
void MainWindow::loadLabelFromCommandLine(QString label_path)
{
    if (QFile::exists(label_path)) {
        ...
        iftImageSequence *labelseq = NULL;
        if (QFileInfo(label_path).suffix() == "imsq") {
            labelseq = iftReadImageSequence(label_path.toUtf8().constData());
            label = iftExtractVolumeFromImageSequence(labelseq, 0);
        } else {
            label = iftReadImageByExt(label_path.toUtf8().constData());
            labelseq = iftVolumeToImageSequence(label);
        }
        ...
        // TODO allow opening several labels at once
        for (int i = 0; i < labelseq->tsize; i++) {
            iftDestroyImage(&label);
            label = iftExtractVolumeFromImageSequence(labelseq, i);
            view->setLabel(label, i, ctb, i == 0);
        }
        iftDestroyImage(&label);
        ...
        QMessageBox::warning(this, "File not found!", "Could not find the file " +label_path+ ".");
    }
    return;
...
void MainWindow::slotOpenFile()
{
    ...
    filters << "Multidimensional Image files (*.imsq *.scn *.nii *.nii.gz *.hdr *.png *.pgm *.jpg *.jpeg *.mimg)"
            << "Image Sequence (*.imsq)"
            << "Scene (*.scn)"
            << "NIfTI (*.nii)"
...
void MainWindow::slotImportLabel()
{
    ...
    filters << "Multidimensional Image files (*.scn *.nii *.nii.gz *.hdr *.pgm *.png *.imsq)"
            << "Scene (*.scn)"
            << "NIfTI (*.nii)"
            << "Compressed NIfTI (*.nii.gz)"
            << "Analyze (*.hdr)"
            << "Portable Network Graphics (*.png)"
            << "Portable Graymap Format (*.pgm)"
            << "Multidimensional Image Sequence File (*.imsq)";
    dialog.setNameFilters(filters);
...
void MainWindow::slotExportLabel()
{
    ...
    QString filter = QString("Multidimensional Image files (*.scn *.nii *.nii.gz *.hdr *.png *.pgm *.imsq);;") +
                     "Scene (*.scn);;NIfTI (*.nii);;Compressed NIfTI (*.nii.gz);;" +
                     "Analyze (*.hdr);;Portable Graymap Format (*.pgm);;" +
                     "Portable Network Graphics (*.png);;" +
                     "Image Sequence file (*.imsq);;";
    ...
    if (imageSet.size() != 0) {
        QSettings settings;
        QString file = imageSet[imageSetCurrentIndex - 1];
        QMap<QString, QVariant> segmented_files = settings.value(imageSetPath).value<QMap<QString, QVariant>>();
        segmented_files.insert(file, QVariant::fromValue(fileName));
        settings.setValue(imageSetPath, segmented_files);
        settings.sync();
    }
    ...
    if (QFileInfo(fileName).suffix() == "imsq") {
        FILE *fp = fopen(fileName.toStdString().c_str(), "w");

        if (fp != NULL) {
            //() << QFileInfo(fileName);
            fclose(fp);
            iftWriteImageSequence(view->getLabelSequence(), fileName.toUtf8().data());
        } else {
            QMessageBox::warning((QWidget*) parent(),
                                 tr("Warning"),
        ...
        iftImage *label = const_cast<iftImage*>(view->getLabel());
        iftCopyVoxelSize(view->getImageSequence(), label);
        iftWriteImageByExt(label, fileName.toUtf8());
    }
...
void MainWindow::loadNextImageFromImageSet()
{
    if (imageSetCurrentIndex - 1 < imageSetSize) {
        imageSetCurrentIndex++;
        QSettings settings;
        QString file = imageSet.at(imageSetCurrentIndex - 1);
        ui->gbImageSet->setTitle(QString::asprintf("Image Set (%d/%d)", imageSetCurrentIndex, imageSetSize));
        QStringList tmp; tmp << file;
        loadImageFromCommandLine(tmp);
        if (imageSet.empty()) {
            ui->gbImageSet->hide();
        }
        QMap<QString, QVariant> segmented_files = settings.value(imageSetPath).value<QMap<QString, QVariant>>();
        if (segmented_files.contains(file)) {
            QString label_path = segmented_files[file].value<QString>();
            if (QFile::exists(label_path)) {
                loadLabelFromCommandLine(label_path);
            } else {
                segmented_files.remove(file);
                settings.setValue(imageSetPath, segmented_files);
            }
        }
        ui->pbNext->setEnabled(imageSetCurrentIndex < imageSetSize);
        ui->pbPrev->setEnabled(imageSetCurrentIndex > 1);
    }
}

void MainWindow::loadPreviousImageFromImageSet()
{
    if (imageSetCurrentIndex > 1) {
        imageSetCurrentIndex--;
        QSettings settings;
        QString file = imageSet.at(imageSetCurrentIndex - 1);
        ui->gbImageSet->setTitle(QString::asprintf("Image Set (%d/%d)", imageSetCurrentIndex, imageSetSize));
...
void MainWindow::saveLabel()
{
    ...
    if (fail)
        ...
        iftWriteImageByExt(view->getLabel(), label_path.toUtf8());
        QMessageBox::information(this, tr("Operation successful!"),
                                         "The label image was sucessfully saved as " + label_path + ".");
    }
}
...
void MainWindow::slotOpenImageSet()
{
    QSettings settings;
    ...
    imageSetPath = QFileInfo(QFile(imageSet.first())).absolutePath();
    imageSet.sort();
    imageSetCurrentIndex = 0;
    imageSetSize = imageSet.size();
    ui->gbImageSet->show();
    ui->pbPrev->setEnabled(false);
    loadNextImageFromImageSet();
...
void MainWindow::slotChangeObjectVisibility()
{
    ...
    if(tb){
        ...
        CurvesView::getInstance()->changeCurveVisibility(row, visible);
...
void MainWindow::slotUndoMarker()
{
...
void MainWindow::slotTableDeleteObjectClicked()
{
    ...
    if(w){
        ...
        IMGSEQ_FORLOOP_PARALLEL(labelseq) {
            if (IMGSEQ_ITE_VAL(labelseq, 0) == label) {
                IMGSEQ_ITE_VAL(labelseq, 0) = 0;
            }
        }
        ...
        //TODO create updateLabel in view to avoid the allocation and reallocation of Label

        iftColorTable *ctb = view->getColorTable();
//        iftColorTable *new_ctb = (iftColorTable *)iftAlloc(1, sizeof (iftColorTable));
//        if (ctb->ncolors > 1) {
//            new_ctb->color = (iftColor*)iftAlloc(ctb->ncolors-1, sizeof (iftColor));
//        }
//        new_ctb->ncolors = ctb->ncolors-1;
//        for (int i = 0; i < new_ctb->ncolors; i++) {
//            int passed = (i + 1) >= label;
//            new_ctb->color[i] = ctb->color[i + passed];
//        }

        //TODO remove object from all times
        view->setLabel(label_img, currentTime(), ctb);//TODO unify with loadLabelFromCommandLine to LoadLabel(iftImage *)
        iftDestroyColorTable(&ctb);
//        iftDestroyColorTable(&new_ctb);

        view->setRenditionLabel();

        updateAllGraphicalViews();
        loadListOfObjects();
        CurvesView::getInstance()->CurvesView::removeCurve(row);
    }
}
...
void MainWindow::setSystemMode(QString mode)
{
    if (mode == "INITIALIZATION"){
        ...
        openImageSetAct->setEnabled(true);
...
void MainWindow::setAnnotationVisibility(bool visible)
{
    ...
    if (undoAction)
        undoAction->setEnabled(visible);
}

int MainWindow::currentTime()
{
    return ui->sliderTime->value();
}
...
void MainWindow::createHotkeys()
{
    ...
    openImageSetAct = new QAction(tr("Open Image Set"),this);
    connect(openImageSetAct,SIGNAL(triggered()),this,SLOT(slotOpenImageSet()));
...
void MainWindow::createActions()
{
    ...
    connect(ui->pbNext, SIGNAL(clicked(bool)), this, SLOT(loadNextImageFromImageSet()));
    connect(ui->pbPrev, SIGNAL(clicked(bool)), this, SLOT(loadPreviousImageFromImageSet()));
    connect(ui->pbSaveLabel, SIGNAL(clicked(bool)), this, SLOT(saveLabel()));
    ...
    connect(ui->pbUndoMarker, SIGNAL(clicked(bool)), this, SLOT(slotUndoMarker()));
   ...
   // connect(ui->widgetRendering, SIGNAL(labelClicked(int)), this, SLOT(slotClickOnRenderingOpenCurve(int)));
    ...
    connect(undoAction, SIGNAL(triggered()), this, SLOT(slotUndoMarker()));
...
void MainWindow::destroyActions()
{
    ...
    disconnect(ui->pbNext, SIGNAL(clicked(bool)), nullptr,nullptr);
    disconnect(ui->pbPrev, SIGNAL(clicked(bool)), nullptr,nullptr);
    disconnect(ui->pbSaveLabel, SIGNAL(clicked(bool)), nullptr,nullptr);
    ...
    disconnect(ui->pbUndoMarker, SIGNAL(clicked(bool)), nullptr, nullptr);
    ...
    disconnect(undoAction, SIGNAL(triggered()), nullptr, nullptr);
...
void MainWindow::setActions()
{
    ...
    fileMenu->addAction(openImageSetAct);
    ...
    connect (signalMapper, SIGNAL(mappedInt(int)), ui->cbSegmentation, SLOT(setCurrentIndex(int))) ;
...
void MainWindow::slotClickOnRenderingOpenCurve(int label)
{
    CurvesView::getInstance()->PlotCurves(label);
    CurvesView::getInstance()->raise();
}
