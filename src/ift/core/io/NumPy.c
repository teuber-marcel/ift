 ...
 * @author Samuel Martins
 */
void _iftWriteNumPyHeader(FILE *file,   iftNumPyHeader *header) {
    char magic[7];
    ...
    char format[512];
    sprintf(format, "{'descr': '<%s', 'fortran_order': %s, 'shape': (%ld,", header->dtype,header->fortran_order ? "True" : "False", header->shape[0]);
    if (header->n_dims > 1) {
      for (int i = 1; i < (header->n_dims - 1); i++){
	char aux[100];
	sprintf(aux," %ld,",header->shape[i]);
	strcat(format,aux);
      }
      char aux[100];
      sprintf(aux," %ld", header->shape[header->n_dims - 1]);
      strcat(format,aux);
    }
    strcat(format,")}");
...
/************ PUBLIC FUNCTIONS ***************/
iftNumPyHeader *iftCreateNumPyHeader(iftCDataType cdtype,   long *shape, size_t n_dims) {
    if (shape == NULL)
...
void iftWriteNumPy(  iftNumPyHeader *header,   void *data, const char *format, ...) {
    va_list args;
    char npy_path[IFT_STR_DEFAULT_SIZE];
