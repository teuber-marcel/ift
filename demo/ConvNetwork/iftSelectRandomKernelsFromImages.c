...
int main(int argc, char **argv) 
{
  ...
  int             xsize, ysize, zsize, nbands,npatches,whitening;
  ...
  if (argc!=7)
    iftError("Usage: iftSelectRandomKernelsFromImages <input_dir> <output_dir|NULL> <input.convnet> <output.convnet> <whitening:0|1> <npatches>","main");
  ...
  whitening = atoi(argv[5]);
  npatches = atoi(argv[6]);

  iftSelectRandomKernelsFromImages(input_dir,input_convnet,1,whitening,npatches);  
