...
iftDTForest *iftCreateDTForest(  iftMImage *mimg,   iftAdjRel *A,
                                 iftLabeledSet *seeds, float delta, float gamma)
{
    if (gamma < 0.0f)
    ...
    while (!iftEmptyFHeap(Q))
    {
        ...
        for (int i = 1; i < A->n; i++)
        {
            ...
            if (iftMValidVoxel(mimg, v))
            {
                ...
                if (Q->color[q] != IFT_BLACK)
                {
                    double arc_weight = iftCosineDistanceDynamicSetMImage(S[root->val[p] ], mimg, q);
