...
iftAdjRel *iftCircularEdges(float r)
{
  ...
  for(i=0;i<Ac->n;i++){
    if ((Ac->dy[i]*Ac->dy[i] + Ac->dx[i]*Ac->dx[i]) >= r*r)
      n++;
  }
...
iftAdjRel* iftReadAdjRel(char* filename){
  ...
  for (int i=0; i < A->n; i++)
    if (fscanf(fp,"%d %d %d %d",&A->dx[i],&A->dy[i],&A->dz[i],&A->dt[i])!=4)
        iftError("Reading error", "iftReadAdjRel");
...
void iftWriteAdjRelBinFile(  iftAdjRel *A, const char *filename) {
    if ((A != NULL) && (filename != NULL)) {
        ...
        if (fwrite(A->dt, sizeof(int), A->n, fp) != A->n)
            iftError("Error when writing dz array", "iftWriteAdjRelBinFile");
...
inline iftVoxel iftGetAdjacentVoxel(  iftAdjRel *A, iftVoxel u, int adj)
{
  iftVoxel v;
...
iftAdjRel *iftAdjacencyBoundaries(  iftAdjRel *A,   iftAdjRel *B_in) {
