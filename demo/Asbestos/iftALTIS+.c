/*
Created by azaelmsousa on 05/11/21.

 This software is the product of the following papers:
    1) ALTIS: A fast and automatic lung and trachea CT‐image segmentation method, published at the Medical Physics Journal, 2019.
    2) Improving Automated Lung Segmentation in CT Images by Adding Anomalies Adjacent to the Pleura.
...
#define EROSION_SQUARED_RADIUS      100      //(default 100)
#define N_PREDECESSOR               25
#define GEODESIC_DISTANCE           15       //(default 15)
#define PLEURAL_CLOSING_RADIUS      20       //(default 20)
...
/* Lungs and Trachea-bronchi Segmentation*/
...
iftImage *iftPleuralAnomalyDetectionByGeodesicDistance(iftImage *segm, float closing_radius, float geo_dist);
...
double geodesicDistance=GEODESIC_DISTANCE;
double pleuralClosingRadius=PLEURAL_CLOSING_RADIUS;
...
int main (int argc, const char **argv)
{
    ...
    char *img_out           = NULL;
    char *formatted         = NULL;
    iftImage *scn = NULL, *grad = NULL, *voi = NULL, *segm = NULL, *improved = NULL;
    iftAdjRel *A = NULL;
    timer *t1,*t2;
...
#ifdef SHOW_STEP_BY_STEP
    puts("");
    puts("Identifying mediastinum");
#endif
    improved = iftPleuralAnomalyDetectionByGeodesicDistance(segm,pleuralClosingRadius,geodesicDistance);
    ...
    iftCopyVoxelSize(scn,improved);
    iftWriteImageByExt(improved,img_out);
    iftFree(img_out);
...
iftDict *iftGetArgs(int argc, const char *argv[])
{
    ...
    iftCmdLineOpt cmd_line_opts[] = {
            ...
            {.short_name = "-p", .long_name = "--pleura-geodesic-distance", .has_arg=true, .arg_type=IFT_LONG_TYPE,
                    .required=false, .help="Geodesic distance to estimate mediastinum.\nDefault: 15."},
            {.short_name = "-r", .long_name = "--pleura-anomaly-closing-radius", .has_arg=true, .arg_type=IFT_LONG_TYPE,
                    .required=false, .help="Closing operation to detect pleural anomalies.\nDefault: 20."},
#endif
...
#ifdef PARAMETER_OPTIMIZATION
    if (iftDictContainKey("--pleura-geodesic-distance", args, NULL))
        geodesicDistance = iftGetLongValFromDict("--pleura-geodesic-distance", args);
    else
#endif
        geodesicDistance = GEODESIC_DISTANCE;

#ifdef PARAMETER_OPTIMIZATION
    if (iftDictContainKey("--pleura-anomaly-closing-radius", args, NULL))
        pleuralClosingRadius = iftGetLongValFromDict("--pleura-anomaly-closing-radius", args);
    else
#endif
        pleuralClosingRadius = PLEURAL_CLOSING_RADIUS;
...
iftImage *iftGeodesicDistanceInPleura(iftImage *mask, iftImage *lung, float geo_dist)
{
    ...
    border = iftCreateImageFromImage(mask);

    iftAdjRel *A  = iftSpheric(sqrt(3.));
    for(int p=0; p < lung->n; p++) {
        iftVoxel u = iftGetVoxelCoord(lung, p);
        for (int i = 1; i < A->n; i++) {
            iftVoxel v = iftGetAdjacentVoxel(A, u, i);
            if (iftValidVoxel(lung, v)) {
                q = iftGetVoxelIndex(lung, v);
                if (lung->val[p] != lung->val[q]) {
                    border->val[p] = lung->val[p];
                    break;
                }
            }
        }
...
iftImage *iftRemoveMediastinumForest(iftImage *mediastinum_bin, iftImage *closed, iftImage *closed_root, iftSet *seeds)
{
    ...
    for (int i = 0; i < mediastinum_bin->n; i++){
        if (mediastinum_bin->val[i] > 0){
            out->val[i] = 0;
        }
    }
