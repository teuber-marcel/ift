...
def main():
    if len(sys.argv) != 5:
        sys.exit(f"python {sys.argv[0]} <result_dir> <svoxels_dir> <gt_dir> <out_csv_path>\n"
                 ...
                 "                ...\n"
                 "svoxels_dir: + results/affine_affine/SymmISF/0400/SymmSupervoxels/\n"
                 "                + 000003_000001.nii.gz\n"
                 "                + 000005_000001.nii.gz\n"
                 "                ...\n"
    ...
    for detection_img_path in glob.glob(f"{result_dir}/*"):
        ...
        svoxels_img_path = os.path.join(svoxels_dir, image_filename)
        ...
        for obj in range(1, n_objs + 1):
            ...
            # ift.CreateImageFromNumPy(intersec_data.astype("int32"), True).Write("out.nii.gz")
            ...
            # ift.CreateImageFromNumPy(false_positive_svoxels_data.astype("int32"), True).Write(f"tmp/{image_key}_false_positive_svoxels_img.nii.gz")
            # false_positive_connected_svoxels_img.Write(f"tmp/{image_key}_false_positive_connected_svoxels_img.nii.gz")
