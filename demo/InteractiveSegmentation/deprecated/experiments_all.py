#!/usr/bin/python2.7
#experiments.py: Script used to find parameters for the experiments
import os
from pyift import *
import time
...
class Experiment:
    ...
    def create_superpixel_cl_image(self, image, seed, spatial_radius, volume_threshold ):
        ...
        for p in range(0,classification_image.n):
            ...
            #classification label5, 1, 2, 7.0,3000)
...
def main():
    ...
    for p in exp_param:
        filename = "stats/{}it{}seed{}mmax{}mmin{}mdist{}bdist{}vrob{}spr{}vth.png".format(p[2],p[3],p[4],p[5],p[6],p[7],p[8],p[0],p[1])
    ...
    exp_param = [ 
				  #Baseline
				  #(1.5, 150, 5, 8, 5, 1, 3, 15.0,15000),
				  (1.5, 150, 5, 8, 5, 1, 2, 7.0, 3000),
				  #(1.5, 150, 5, 8, 5, 1, 2, 8.0, 4000),
				  #(1.5, 150, 5, 8, 5, 1, 2, 9.0, 5000),
				  ]
    
    for p in exp_param:
        filename = "stats/{0}it{1}seed{2}mmax{3}mmin{4}mdist{5}bdist{6}vrob{7}spr{8}vth.png".format(p[2],p[3],p[4],p[5],p[6],p[7],p[8],p[0],p[1])
        if not os.path.exists(filename):
            exp = Experiment("grabcut/",*p)
            exp.run_robot("SUPER","grabcut_statistics_sup.txt")
            exp.run_robot("PIXEL", "grabcut_statistics_pix.txt")
