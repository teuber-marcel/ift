...
void dataNormalization(iftMatrix *data){
    ...
    iftMatrix *meansColumn = iftComputeMeanVectorByColumn(data);
...
void applyPCAInData(iftMatrix **data){
    ...
    meansColumn = iftComputeMeanVectorByColumn((*data));
    iftSubtractionMatrixVectorByRow((*data),meansColumn);
...
iftMatrix* computePairWiseDistanceMatrix(iftMatrix *data){
    ...
    distances = iftMultMatrices(data,dataTransposed);
...
iftMatrix *tSNE(iftMatrix* inputData, unsigned int dimensionOut,double perplexity){
    ...
    convertDistances2Probabilities(distances, perplexity,tolerance, &Probabilities,&beta);
...
int main(int argc, char *argv[]) {
    ...
    iftMatrix *X = iftCreateMatrix(3,8);
    X->val[0] = 3;
    X->val[1] = -1;
    X->val[2] = 1;
    ...
    iftMatrix* dataOut = tSNE(X,2,perplexity);
    iftPrintMatrix(dataOut);
    printf("\n");
