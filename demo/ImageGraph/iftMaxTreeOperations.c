...
int main(int argc, char *argv[]) 
{
  ...
  if (argc!=5)
    iftError("Usage: iftMaxTreeOperations <image.[pgm,ppm,scn]> <out.[pgm,ppm,scn]> <criterion (Height-0, Area-1, Volume-2)> <threshold>","main");
  ...
  iftIGraphDualWaterGray(igraph,atoi(argv[3]),atoi(argv[4]));

  resImg = iftIGraphPathValue(igraph);
  
  if (iftIs3DImage(resImg)){
      iftWriteImage(resImg,argv[2]);
  } else {
