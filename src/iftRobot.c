...
void iftEDT3D(iftImage *bin, iftImage **dist_img, iftImage **relabel_img) {
...
void iftObjectVolumeFromVoxel(  iftImage* comp, int index, iftImage *area_img, iftFIFO *F1, iftFIFO *F2)
{
    iftAdjRel *A = iftSpheric(1.0);

    int area = 0;
    iftInsertFIFO(F1, index);
    while (!iftEmptyFIFO(F1))
    {
        int p = iftRemoveFIFO(F1);
        area++;
        ...
        for(int i = 0; i <A->n; i++){
            ...
            if(iftValidVoxel(comp, v)){
                ...
                if(comp->val[q] == comp->val[p] && F1->color[q] == IFT_WHITE)
                    iftInsertFIFO(F1, q);
            }
        }
    }

    iftInsertFIFO(F2, index);

    while (!iftEmptyFIFO(F2)) {
        int p = iftRemoveFIFO(F2);
        area_img->val[p] = area;
        iftVoxel u = iftGetVoxelCoord(comp, p);
        ...
        for(int i = 0; i <A->n; i++){
            ...
            if(iftValidVoxel(comp, v)){
                int q = iftGetVoxelIndex(comp, v);
                if(comp->val[q] == comp->val[p] && F2->color[q] == IFT_WHITE)
                    iftInsertFIFO(F2, q);
            }
        }
    }
...
void iftRobotFindSeedsEDT3D(iftRobot *bot) {

    iftImage *relabel_img = NULL, *dist_img;
    iftAdjRel *A8nb = iftSpheric(sqrtf(2.0f));

    iftEDT3D(bot->error,  &dist_img, &relabel_img);
    ...
    iftFIFO *F1 = iftCreateFIFO(relabel_img->n);
    iftFIFO *F2 = iftCreateFIFO(relabel_img->n);
    for (int p = 0; p < relabel_img->n; p++) {
        ...
        if(area_img->val[p] == 0) {
            ...
            iftObjectVolumeFromVoxel(relabel_img, p, area_img, F1, F2);
        }

        if ((area_img->val[p] >= max_area->val[label]) && (dist_img->val[p] > max_dist->val[label]) ) {
            max_pos->val[label]  = p;
            max_area->val[label] = area_img->val[p];
            max_dist->val[label] = dist_img->val[p];
        }
...
void iftRobotFindSeedsCenterOfMass3D(iftRobot *bot)
{
    iftIntArray *labels = iftGetObjectLabels(bot->error);
    iftIntArray *max_pos = iftCreateIntArray(labels->n + 1);
    iftIntArray *max_area = iftCreateIntArray(labels->n + 1);// include bg
    ...
    iftFIFO *F1 = iftCreateFIFO(bot->error->n);
    iftFIFO *F2 = iftCreateFIFO(bot->error->n);
    for (int p = 0; p < bot->error->n; p++) {
        // bg pixels has object relabeled values after edt computing, then
        // we need to check its original value to guarantee the real label of bg pixels
        int label = bot->error->val[p];
        ...
        if (max_pos->val[label] == 0) {
            if (area_img->val[p] == 0) {
                iftObjectVolumeFromVoxel(bot->error, p, area_img, F1, F2);
            }
