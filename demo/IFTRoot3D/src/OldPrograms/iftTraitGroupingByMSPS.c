...
void iftMSDeltasForUnsupFeatSelec(iftMSPS *msps, float gamma)
{
  int i,j;

  for (i=0; i < msps->n; i++) {
    msps->delta->val[0][i] = 0.005; 
    for (j=1; j < msps->m; j++) {
      msps->delta->val[j][i] = msps->delta->val[j-1][i]*gamma;
    }
  }      
}

float iftMSPSFitnessForUnsupFeatSelec(void *problem, float *theta)
{
  float value=0.0;
  int i;
  iftKnnGraph *graph;
...
void iftUnsupFeatSelecByMSPS(iftDataSet *Z, int kmax)
{
  ...
  msps = iftCreateMSPS(Z->nfeats,3,iftMSPSFitnessForUnsupFeatSelec,prob);
  for (i=0; i < Z->nfeats; i++)
    msps->theta[i]=Z->alpha[i];
  iftMSDeltasForUnsupFeatSelec(msps,3.0);
  iftMSPSMin(msps);
...
int main(int argc, char *argv[]) 
{
  ...
  if (argc!=3)
    iftError("Usage must be: iftTraitGroupingByMSPS <basename> <maximum scale (0,1]","main");
