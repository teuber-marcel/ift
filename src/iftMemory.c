...
void iftCopyIntArray(int *array_dst,   int *array_src, int nelems) {
    #pragma omp parallel for
    for (int i = 0; i < nelems; i++) {
        array_dst[i] = array_src[i];
    }
...
#ifndef  __cplusplus
void iftCopyLongLongIntArray(long long *array_dst,   long long *array_src, int nelems) {
    #pragma omp parallel for
    for (int i = 0; i < nelems; ++i) {
        array_dst[i] = array_src[i];
    }
...
void iftCopySizeTArray(long *array_dst,   long *array_src, long nelems) {
    long i;
    for (i = 0; i < nelems; i++) {
        array_dst[i] = array_src[i];
    }
