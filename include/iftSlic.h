...
/**
 * @brief Slic Common parameters.
 * @authors Samuel Martins, John Vargas
 * @date May 24, 2016
 */
typedef struct ift_slic {
    /** Final Number of Cells/Clusters/Superpixels */
    int n_cells;
    /** Compactness: Relative Importance of color similarity and spatial proximity */
    double compactness;
    /** Side of the initial regular cell = (step) distance between two initial adjacency cluster's center */
    int step;
    /** Color Feature Vector for each superpixel from the LAB color space (3 feats) or
        intensity level (gray image = 1 feat) */
    iftFColor *color_feats;
    /** Spatial Feature Vector = Coordinates of the Center Voxels from the cells/clusters/superpixels */
    iftPoint *spatial_feats;
    /** Maximum range of the image to be oversegmented: e.g 255 (for 8 bits images) */
    int img_max_range;
    /** Tells if the image to be oversegmented is 3D or not */
    bool is_3D_img;
    /** Tells if the image to be oversegmented is colored or gray */
    bool is_color_img;
    /** */
    iftImage *region_map;
    /** */
 ...
 */
iftSlic *iftInitSlic(   iftImage *grad,    iftMImage *mimg, int input_n_cells, double comp,
                     int img_max_range);
 ...
 *                    (if the YCbCr Image is colored) or the original Grayscale Image (1 band)
 */
void iftUpdateCellCenters(iftSlic *slic,    iftImage *superpixels,    iftMImage *mimg);
 ...
 */
iftImage *iftGenerateSuperpixelsBySlic(   iftImage *img,    iftImage *input_grad, int input_n_cells,
 ...
 * @return                   The Superpixel Image.
 */
iftImage *iftGenerateSuperpixelsBySlicInBlocks(   iftImage *img,    iftImage *input_grad, int n_blocks,
