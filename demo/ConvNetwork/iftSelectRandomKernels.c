...
int main(int argc, char **argv) 
{
  ...
  if (argc!=3)
    iftError("Usage: iftSelectRandomKernels <input.[ppm,pgm]> <input_parameters.convnet> ","main");
  ...
  for (int l=0; l < NLAYERS; l++) {
    ...
    iftSelectRandomKernels(input[l],convnet,1);
  ...
  t2     = iftToc();
  fprintf(stdout,"Kernels selected in %f ms\n",iftCompTime(t1,t2));

  convnet->nkernels[convnet->nlayers-1] /= 2;
