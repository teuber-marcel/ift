...
// for charplates_location database - 1 layers 
int   vet_input_norm[8] = {1,3,5,7,9,0,0,0}; //
int   vet_k_bank    [8] = {3,5,7,9,3,5,7,9};
int   vet_nkernels  [8] = {32,64,96,128,160,192,224,256}; // for 100k features
int   vet_pooling   [8] = {3,5,7,9,0,0,0,0}; //
int   vet_stride    [4] = {1,2,3,4};
float vet_alpha     [4] = {1.,2.,3.,100.};
int   vet_norm      [8] = {1,3,5,7,9,0,0,0}; //

//// for charplates_location database - 1 layers 
//int   vet_input_norm[8] = {1,3,5,7,9,3,5,0}; //
//int   vet_k_bank    [8] = {3,5,7,9,3,5,7,9};
////int   vet_nkernels  [8] = {256,364,512,768,1024,536,2048}; //for 10-40k features
//int   vet_nkernels  [8] = {8,12,16,32,48,64,96,128}; // for 10-40k features
//int   vet_pooling   [8] = {3,5,7,9,3,5,0,0}; //
//int   vet_stride    [4] = {1,2,1,2};
//float vet_alpha     [4] = {1.,2.,3.,100.};
//int   vet_norm      [8] = {1,3,5,7,9,3,5,0}; //
...
int iftVerifyImageDimensionsAlongNetwork(iftConvNetwork *convnet,int cnnminsize,int cnnmaxsize)
{
...
//    fprintf(stderr,"pooling: %d (%d,%d,%d) - ",nbands[j],xsize[j],ysize[j],zsize[j]);
...
//    fprintf(stderr,"stride: %d (%d,%d,%d)\n",nbands[j],xsize[j],ysize[j],zsize[j]);
  ...
  if (ret) {
    ...
    if (ret) {
      if( (nbands[convnet->nstages-1] * xsize[convnet->nstages-1] * ysize[convnet->nstages-1] * zsize[convnet->nstages-1] < cnnminsize) || 
          (nbands[convnet->nstages-1] * xsize[convnet->nstages-1] * ysize[convnet->nstages-1] * zsize[convnet->nstages-1] > cnnmaxsize) ){
        ret = 0;
      }
    }
  }
...
int main(int argc, char **argv) {
  if (argc != 10)
    iftError("Please provide the following parameters:\n<random_seed> <ysize> <xsize> <nbands> <nlayers> <mincnnsize> <maxcnnsize> <output_directory> <num_convnets>\n\n", "main");
  ...
  int cnnminsize = atoi(argv[6]);
  int cnnmaxsize = atoi(argv[7]);

  char outputfile[100],dirArchOut[100]; strcpy(dirArchOut, argv[8]);
  if (dirArchOut[strlen(dirArchOut)-1] == '/')
    dirArchOut[strlen(dirArchOut)-1] = '\0';
  int nconvnets = atoi(argv[9]);

  srand(random_seed);

  iftConvNetwork *convnet,**lconvnet;
  int randvalue,hlayers[nlayers+1];

  for(int l=1;l<=nlayers+1;l++) hlayers[l] = 0;
  ...
  while(ncurconvnet < nconvnets ) {
    ...
    //    randvalue = rand();
    //    nlayers = iftGetFirstNBits(&randvalue,3);
    //    if (nlayers < 1 || nlayers > nlayers)
    //      continue;
    //    nlayers = 3;
    ...
    for(int l=0; l < nlayers; l++) {
      ...
      convnet->nkernels[l]          = vet_nkernels    [iftGetFirstNBits(&randvalue,3)];
      convnet->activ_option[l]      = 0;
      convnet->activ_thres[l]       = 0.;
      convnet->pooling_adj_param[l] = vet_pooling     [iftGetFirstNBits(&randvalue,3)];
      convnet->stride[l]            = vet_stride      [iftGetFirstNBits(&randvalue,3)];
	...
	convnet->alpha[l]  = 1.;
      ...
      if ( (l == 0) && (convnet->nkernels[l] > 10000) ) //64
	stop = 1;
      if (l > 0)
	if ( (convnet->nkernels[l] / convnet->nkernels[l-1] < 1) ||
             (convnet->nkernels[l] / convnet->nkernels[l-1] > 2) )
	  stop = 1;

      if ( (convnet->pooling_adj_param[l] != 0) && (convnet->pooling_adj_param[l] < convnet->stride[l]) )
	stop = 1;
    }
    //    convnet->norm_adj_param[nlayers-1] = 3; // empirically imposed
    ...
    if (  !stop && (iftVerifyImageDimensionsAlongNetwork(convnet,cnnminsize,cnnmaxsize) ) && 
      ...
      iftWriteIntValues(stdout,convnet->activ_option,"ACTIV_OPTION"); // to be updated
      iftWriteFloatValue(stdout,convnet->activ_thres,"ACTIV_THRES"); // to be updated
