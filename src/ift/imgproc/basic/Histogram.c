...
/*********************** PRIVATE FUNCTIONS ***********************/
iftHist *_iftCalcHist(  int *val, long n,   int *mask, long nbins, int max_val, bool normalize) {
    if (val == NULL)
...
iftHist *iftReadHistogram(char *filename)
{
  ...
  if (fscanf(fp,"%ld",&nbins)!=1);  
  hist = iftCreateHist(nbins);
  for (int i = 0; i < hist->nbins; i++)
    if (fscanf(fp,"%f",&hist->val[i])!=1);
