...
iftFImage *iftCreateFImageFromFImage(  iftFImage *src) {
...
iftFImage *iftImageToFImage(  iftImage *img1) {
...
iftImage *iftFImageToImage(  iftFImage *img1, int Imax) {
...
iftFImage *iftImageToFImageMaxVal(  iftImage *img, float fmax) {
    iftFImage *fimg = iftCreateFImage(img->xsize, img->ysize, img->zsize);
...
iftFImage *iftFCopyImage(  iftFImage *fimg) {
    iftFImage *copy = NULL;
...
void iftWriteFImage(  iftFImage *fimg, const char *format, ...) {
    va_list args;
    char npy_path[IFT_STR_DEFAULT_SIZE];
...
iftFImage *iftFExtractROI(  iftFImage *fimg, iftBoundingBox bb) {
...
void iftFInsertROI(  iftFImage *roi, iftFImage *target, iftVoxel begin) {
    iftVoxel uo = begin;
    iftVoxel uf;
...
void iftFInsertROIByPosition(  iftFImage *roi,   iftVoxel roi_pos, iftFImage *target,   iftVoxel target_pos) {
    iftVoxel uo, uf;
...
iftBoundingBox iftFMinBoundingBox(  iftFImage *fimg, iftPoint *gc_out) {
    if (fimg == NULL)
...
iftFImage *iftFShiftImage(  iftFImage *fimg, int dx, int dy, int dz) {
    if (fimg == NULL)
...
iftFImage *iftFExtractSlice(  iftFImage *vol_fimg, iftImagePlaneOrientation plane_orientation, long slice) {
    // CHECKERS
    if (vol_fimg == NULL)
...
iftFImage *iftFMask(  iftFImage *fimg,   iftImage *mask) {
...
iftFImage *iftFlipFImage(  iftFImage *fimg, char axis) {
...
void iftFImageSetValue(iftFImage *fimg,   iftSet *S, float value)
{
