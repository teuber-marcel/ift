...
Image *MBB(Image *img)
{
    ...
    BB(img, &(left.x),&(left.y),&(right.x),&(right.y));
...
int ImageVolume(Image *img)
{
  ...
  int vol=0,p,n=img->ncols*img->nrows;
...
Image *TransformImage(Image *img, float M[4][4], int ncols, int nrows)
{
    ...
    transf = CreateImage(ncols,nrows);

    for (v.y=0; v.y < transf->nrows; v.y++)
        for (v.x=0; v.x < transf->ncols; v.x++)
        {
     	  orig.x = v.x;
 	  orig.y = v.y;
	  orig.z = 0;
            ...
            if (ValidPixel(img,u.x,u.y))
            {
                ...
                transf->val[v.x+transf->tbrow[v.y]] = ROUND(If);
            }
        }
