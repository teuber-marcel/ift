...
char* iftFramePath( char *folder,   char *name, int frame_id)
{
...
iftImage* iftReadImageFolderAsVolume( char* folder_name)
{
	...
	for(z = 0; z < files->n; z++)
	{
		 char *path = files->files[z]->path;
		if(!iftIsImageFile(path) || iftCompareStrings(iftFileExt(path),".scn")) {
            iftError("File %s cannot be read as an image for ructing a 3D volume!", "iftReadImageFolderAsVolume",
                     path);
		}
	}
