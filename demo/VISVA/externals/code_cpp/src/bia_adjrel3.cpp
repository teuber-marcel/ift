  ...
  namespace AdjRel3{
	...
	*A = NULL;
    ...
    int      GetFrameSize(AdjRel3 *A){
      ...
      for (i=0; i < A->n; i++){
	if (fabs(A->d[i].axis.x) > sz) sz = fabs(A->d[i].axis.x);
	if (fabs(A->d[i].axis.y) > sz) sz = fabs(A->d[i].axis.y);
	if (fabs(A->d[i].axis.z) > sz) sz = fabs(A->d[i].axis.z);
      }
      return(sz);
    }
    ...
    void     DestroyAdjVxl(AdjVxl **N){
      ...
      aux = *N;
      if(aux != NULL){
	if(aux->dp != NULL) bia::FreeIntArray(&aux->dp);
	free(aux);
	*N = NULL;
      }
    }
