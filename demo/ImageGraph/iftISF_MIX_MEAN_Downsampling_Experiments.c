...
int main(int argc, char *argv[]) 
{
  if(argc!=10)
  {
    iftError("Usage: <dataset path> <nsamples> <alpha (e.g., [0.005-0.2])> <beta (e.g., 12)> <niters (e.g., 10)> <smooth niters (e.g., 0,1,2,...)> <downsampling_factor (e.g., 0.5)> <output path> <output csv nseeds file>", "iftISF_MIX_MEAN_Downsampling_Experiments");
    return 1;
  }
  ...
  int i, niters, mean_niters, nseeds, smooth_niters;
  float mean_nseeds, mean_time, partial_proc_time;
  ...
  for (i = 0; i < nimages; i++) {
    ...
    iftImage *label1 = iftExtract_ISF_MIX_MEAN_Superpixels(img1, atoi(argv[2]), atof(argv[3]), atof(argv[4]), atoi(argv[5]), smooth_niters, &(nseeds), &niters, &partial_proc_time);
    mean_time += partial_proc_time;
    ...
    if (smooth_niters == 0) {
      ...
      superpixels = iftSmoothSuperpixels(img, label2, smooth_niters);
    ...
    iftWriteImageP2(superpixels, "%s/%s.pgm", argv[8], filename);
