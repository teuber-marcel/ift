...
int main(int argc, const char *argv[])
{
    ...
    iftMImage *mimg, *saliency_mimg;
    int  maxIter = 30, max_number_of_clusters, number_superpixels, cluster_number, iterations; //ARG
    float thr = 0.7, alpha=0.5, beta=12, gamma=2.5, iter=1,minImprovement=0.01;
    char      filename[250];
...
//    iftWriteImageByExt(saliency_img, "global-result.png");
    ...
    for(int i = 0; i<iterations; i++) {
        printf("HERE1\n");
        number_superpixels = number_superpixels + (number_superpixels/(iterations));
...
iftFloatArray *saliencySmoothing(iftDataSet *Z, iftImage *label_img, iftFloatArray *saliency, iftColorTable *colors, int cluster_number, iftMatrix *region_color_prob, iftMatrix *region_has_color, double *border_region_ratio){
    ...
    saliency_new = iftCreateFloatArray((int)saliency->n-1);
    ...
    for(int ri = 0; ri < (int)saliency->n-1; ri++){
        ...
        for(int ci = 0; ci < cluster_number; ci++) {
            if (!iftMatrixElem(region_has_color, ci, ri))
                continue;
            new_saliency+= iftMatrixElem(region_color_prob, ci, ri) * Z_color_saliency_new->sample[ci].feat[0]; //applying weight
...
iftImage *imageToRegionSaliencyImage(iftMImage *img, iftDataSet *Z, iftImage *label_img, int cluster_number){
    ...
    int  c =0, border_radius = 15; //m is used for smoothing
    double  max_saliency = 0, max_region_size = 0, min_saliency = IFT_INFINITY_DBL, *border_region_ratio;
    float sigma_squared = -0.4, border_penalty = 0.01;
...
#pragma omp parallel for
    ...
    #pragma omp parallel for
        ...
        if(border_counter > (Z_region->nsamples / 100)) {
    ...
    saliency = iftCreateFloatArray(region_sizes->n);
...
#pragma omp parallel for
    for(int ri = 0; ri < (int)region_sizes->n-1; ri++){
        ...
        for(int rj = 0; rj < (int)region_sizes->n-1; rj++){
            ...
            sum_rk = dr * (float)  (region_sizes_prob->val[rj+1]) * exp(centroid_distances->distance_table[ri][rj]/sigma_squared); //w(r_i) on the paper
            dr = 0;
        }
        saliency->val[ri] = sum_rk * border_region_ratio[ri] * (1.0/region_to_center->distance_table[ri][0]); // region_to_center_distances is the paper's w_s(r_k)
        sum_rk = 0;
    }
