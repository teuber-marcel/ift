...
void iftGMMAddSample(iftGMM *gmm,   float *colors, int grp)
{
...
iftGMM *iftCreateGMM(  iftDataSet *Z)
{
    if (Z->nfeats != 3) {
...
double iftGMMPixPDF(iftGMM *gmm,   double *colors, int comp)
{
    double res = 0;
    if (gmm->coefs[comp] > 0)
    {
...
double iftGMMPixCDF(iftGMM *gmm,   double *colors, int comp, double jitter)
{
    double res = 0;
    if (gmm->coefs[comp] > 0)
    {
        ...
        radii += jitter;
          double approx_rv = pow(radii / CHI_SQ_DF, 1.0 / 3.0);
...
int iftGMMGetComp(iftGMM *gmm,   iftMImage *mimg, int p)
{
    int idx = 0;
    double max = -1.0;
...
iftImage *iftGMMAssignComp(iftGMM *gmm_obj, iftGMM *gmm_bkg,   iftMImage *mimg, iftImage *regions)
{
...
iftFImage *iftGMMGetProb(iftGMM *gmm,   iftMImage *mimg)
{
...
void iftGMMsUpdate(iftGMM *gmm_obj, iftGMM *gmm_bkg,   iftMImage *mimg, iftImage *regions, iftImage *comp)
{
...
iftImage *iftUpdateUncertMap(  iftImage *map,   iftImage *segm)
{
...
iftImage *iftGrabCut(  iftMImage *mimg,   iftImage *regions_, double beta, int n_iters)
{
...
iftFImage *iftGMMDataSetDist(iftDataSet *train,   iftMImage *mimg, int n_comp)
{
