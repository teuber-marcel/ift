...
void iftDrawBoundingBoxBorders2DInPlace(iftImage *img, iftBoundingBox bb, iftColor YCbCr, iftAdjRel *A, bool drawCentralPoint) {
    ...
    for (int y = bb.begin.y; y <= bb.end.y; y += (bb.end.y-bb.begin.y))
        for (int x = bb.begin.x; x <= bb.end.x; x++) {
            ...
            iftDrawPoint(img, u, YCbCr, A, norm_val);
        }
    ...
    if(drawCentralPoint)
        iftDrawPoint(img, iftBoundingBoxCenterVoxel(bb), YCbCr, A, norm_val);
}
...
void iftDrawSeeds(iftImage *img, iftLabeledSet *S)
{
    ...
    int  nlabels = iftNumberOfLabels(S);
    int norm_val = iftNormalizationValue(iftMaximumValue(img));
    iftColorTable *ctb = iftCreateRandomColorTable(nlabels+1,norm_val);
    int p, label;
    int maxRangeValue = iftNormalizationValue(iftMaximumValue(img));
    ...
    while (Saux!=NULL) {
        p   = Saux->elem;
	label = Saux->label;
	img->val[p] = ctb->color[label].val[0];
        img->Cb[p]  = ctb->color[label].val[1];
        img->Cr[p]  = ctb->color[label].val[2];
        Saux = Saux->next;
    }
...
iftImage *iftColorizeComp(iftImage *label)
{
    ...
    ctb = iftCreateRandomColorTable(iftMaximumValue(label)+1,255);
...
iftImage *iftColorizeCompOverImage(iftImage *orig, iftImage *label)
{
    ...
    int norm_val = iftNormalizationValue(iftMaximumValue(orig));
    ctb = iftCreateRandomColorTable(iftMaximumValue(label)+1,norm_val);
    img = iftCreateImage(label->xsize,label->ysize,label->zsize);
    for (p=0; p < img->n; p++)
...
void iftDrawRoiBordersInPlace(iftImage *img, iftRoi *roi, iftColor YCbCr, iftAdjRel *A, bool drawCentralPoint)
{
    ...
    for(int i = 0; i < roi->n; i++) {
        ...
        if(drawPx)
            iftDrawPoint(img, v, YCbCr, A, norm_val);
    }

    /* draw the central point */
    if(drawCentralPoint)
        iftDrawPoint(img, iftRoiCenterVoxel(img, roi), YCbCr, A, norm_val);
...
void iftDrawBinaryLabeledSeeds(iftImage *img,iftLabeledSet *seeds,iftColor YCbCr, iftAdjRel *A)
{
    ...
    YCbCr_compl.val[1] = norm_val-YCbCr.val[1];
    YCbCr_compl.val[2] = norm_val-YCbCr.val[2];
    ...
    while (S != NULL) {
        ...
        if (S->label == 1){
            ...
            iftDrawPoint(img,u,YCbCr_compl,A,norm_val);
        }
        S = S->next;
    }
}
...
void iftDrawColorLegend(iftImage *img, iftColorTable *ctb, int firstColor, int lastColor, char *location, int colorBoxSize)
{
    ...
    iftColor whiteColor;
    whiteColor.val[0] = norm_val;
    whiteColor.val[1] = norm_val;
    whiteColor.val[2] = norm_val;
    whiteColor = iftRGBtoYCbCr(whiteColor, norm_val);

    iftColor blackColor;
    blackColor.val[0] = 0;
    blackColor.val[1] = 0;
    blackColor.val[2] = 0;
    blackColor = iftRGBtoYCbCr(blackColor, norm_val);
