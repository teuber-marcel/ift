/**
 * @file
 * @brief Computes the gradient for a Brain Image as a linear combination of the gradient from two Input Images.
 * The resulting gradient might be: \n
 * (1) only the gradient from the first input brain image(function iftBrainGrad())\n
 * (2) a linear combination of the gradient from the first input brain image with the gradient of 
 ...
 * @note See the source code in @ref iftComputeBrainGrad.c
 *
 * @example iftComputeBrainGrad.c
 * @brief Computes the gradient for a Brain Image as a linear combination of the gradient from two Input Images.
 * The resulting gradient might be: \n
 * (1) only the gradient from the first input brain image(function iftBrainGrad())\n
 * (2) a linear combination of the gradient from the first input brain image with the gradient of 
...
int main(int argc, const char *argv[]) {
    ...
    iftImage *grad_img1      = iftBrainGrad(img1);
    ...
    if (img2_path == NULL) {
        ...
        iftImage *grad_img2      = iftBrainGrad(img2);
        ...
        out_grad_img = iftLinearCombination(norm_grad_img1, norm_grad_img2, alpha);
...
iftDict *iftGetArgs(int argc, const char *argv[]) {
    char program_description[2048] = \
        "Computes the gradient for a Brain Image as a linear combination of the gradient from two Input Images.\n" \
        "The resulting gradient might be:\n" \
        "(1) only the gradient from the first input brain image(function iftBrainGrad())\n" \
        "(2) a linear combination of the gradient from the first input brain image with the gradient of " \
        "a second input image (eg. an object map), according to the formula: \n" \
