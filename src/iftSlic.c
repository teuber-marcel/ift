 ...
 */
iftFColor iftGetColorFeats(  iftMImage *mimg, int voxel_idx, int img_max_range) {
    iftFColor cfeats;
 ...
 */
void iftFindLocalMinimumForSlicSeeds(iftSlic *slic,   iftMImage *mimg,   iftImage *grad) {
    iftAdjRel *A = NULL;
 ...
 */
iftImage *iftPerformSuperpixelsSlic(iftSlic *slic,   iftMImage *mimg) {
 ...
 */
void iftInit2DSeeds(iftSlic *slic,   iftMImage *mimg,   iftImage *grad) {
 ...
 * @note The initial cells are regular grids
 */
void iftInit3DSeeds(iftSlic *slic,   iftMImage *mimg,   iftImage *grad) {
    int n_cells_x = iftRound(mimg->xsize / (1.0 * slic->step));
    int n_cells_y = iftRound(mimg->ysize / (1.0 * slic->step));
 ...
 */
int iftGetLabelFromAdjRelabeledCell(iftVoxel u,   iftImage *new_superpixels,
                                      iftAdjRel *A) {
    int adj_label = -1;
 ...
 */
int iftCellRelabeling(  iftImage *superpixels, iftImage *new_superpixels, int initial_voxel_idx,
                      int new_label,   iftAdjRel *A, int *idxs_in_cell) {
