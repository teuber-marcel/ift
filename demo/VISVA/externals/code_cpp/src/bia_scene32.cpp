...
namespace bia{
  ...
  Scene32 *Create(int xsize,int ysize,int zsize){
    ...
    if(scn == NULL) 
      bia::Error((char *)MSG1,(char *)"Scene32::Create");
  ...
  void     Copy(Scene32 *dest, Scene32 *src){
    if(dest->xsize!=src->xsize ||
      ...
      bia::Error((char *)"Incompatible data",(char *)"Scene32::Copy");
    dest->dx     = src->dx;
    dest->dy     = src->dy;
    dest->dz     = src->dz;
  ...
  void     Write(Scene32 *scn, char *filename){
    ...
    if ( (len<=4) || (strcasecmp(filename + len - 4, ".scn")!=0)) {
      bia::Error(MSG2,"WriteScene: Invalid file name or extension.");
    }
    */
    // Writing the scn file
    fp = fopen(filename,"wb"); 
    if(fp == NULL) 
      bia::Error((char *)MSG2,(char *)"Scene32::Write");
