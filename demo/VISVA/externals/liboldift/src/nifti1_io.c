...
static char * gni_history[] = 
{
  ...
  "   - merged in 4 changes by Mark, and re-added his   keywords\n"
  ...
  "   - added   in all appropraite parameter locations (30-40)\n"
...
nifti_image *nifti_image_read_bricks(const char * hname, int nbricks,
                                       int * blist, nifti_brick_list * NBL)
{
   nifti_image * nim;
...
*//*--------------------------------------------------------------------*/
int disp_nifti_1_header( const char * info,   nifti_1_header * hp )
{
   int c;
...
nifti_1_header * nifti_read_header(const char * hname, int * swapped, int check)
{
   ...
   if( hfile == NULL ){
      if( g_opts.debug > 0 )
         LNI_FERR(fname,"11failed to find header file for", hname);
      return NULL;
...
nifti_image *nifti_image_read( const char *hname , int read_data )
{
   ...
   if( hfile == NULL ){
      if(g_opts.debug > 0)
         LNI_FERR(fname,"22failed to find header file for", hname);
...
{
   nifti_1_header * nhdr;
     int        default_dims[8] = { 3, 1, 1, 1, 0, 0, 0, 0 };
     int      * dim;  /* either passed or default dims  */
...
*/
static void
compute_strides(int *strides,  int *size,int nbyper)
{
  int i;
  strides[0] = nbyper;
  for(i = 1; i < 7; i++)
...
static int rci_read_data(nifti_image * nim, int * pivots, int * prods,
      int nprods,   int dims[], char * data, znzFile fp, size_t base_offset)
{
