 ...
 * @date July 21, 2018
 */
typedef void (*iftCopyVoxelToMImageFunc)(iftMImage *mimg, int p,   iftImage *img, int q, int initial_band,
 ...
 * @date July 21, 2018
 */
void iftCopyVoxelToMImageGRAYNorm(iftMImage *mimg, int p,   iftImage *img, int q, int initial_band,
                                  int normalization_value) {
 ...
 * @date July 21, 2018
 */
void iftCopyVoxelToMImageLAB2Norm(iftMImage *mimg, int p,   iftImage *img, int q, int initial_band,
                                         int normalization_value) {
    iftColor YCbCr;
 ...
 */
void iftExtendMVoxelFeatVector(iftMImage *emimg,   iftMImage *mimg, int p, int n_adj_voxels, float norm_val) {
...
inline iftVoxel iftMGetVoxelCoord(  iftMImage *img, int p)
{
    /* old
...
inline char iftMValidVoxel(  iftMImage *img, iftVoxel v) {
...
iftMImage *iftImageToMImage(  iftImage *img1, char color_space)
{
  iftMImage *img2=NULL;
  iftImage  *input=NULL;
...
#pragma omp parallel for shared(input, img2, normalization_value)
 ...
 iftImage   *iftMImageToImage(  iftMImage *img1, int Imax, int band)
{
...
void iftWriteMImageAsNumPy(  iftMImage *mimg, const char *format, ...) {
  va_list args;
  char npy_path[IFT_STR_DEFAULT_SIZE];
...
iftImage *iftMImageGradient(  iftMImage *img, iftAdjRel *A, int Imax) {
...
iftImageTiles *iftMImageTilesByEquallyDividingAxes(  iftMImage *mimg, int ntiles_x, int ntiles_y, int ntiles_z) {
  iftBoundingBox bb;
...
float iftMImageSqDist(  iftMImage *mimg, int p, int q) {
    float dist = 0.0;
...
float iftMImageDist(  iftMImage *mimg, int p, int q) {
    float dist = 0.0;
...
iftMImage *iftExtendMImageByVoxelCoord(  iftMImage *mimg,   iftImage *label_img,   int n_adj_voxels, bool normalize) {
    float norm_val = 1.0f;
...
iftMImage *iftExtractImageFeatures(  iftImage *img,   iftImage *label_img,   iftAdjRel *A, bool voxel_coord) {
...
iftMImage *iftTransformMImage(  iftMImage *input,   double *L, int L_nrow, int L_ncol)
{
    if (input->m != L_ncol)
...
iftMImage *iftKernelTransformMImage(  iftMImage *input,   iftDoubleMatrix *train_data,
                                      iftDoubleMatrix *omega, iftKernelFunction *K,
...
iftMImage *iftColorQuantization(  iftMImage *input, float lower_bound, float upper_bound, int bins_per_band)
{
...
iftMImage *iftMImageCentralize(  iftMImage *mimg)
{
...
iftMImage *iftMGaussianFilter(  iftMImage *mimg, float radius, float stddev)
{
...
iftMImage *iftBinarySetsNCA(  iftMImage *mimg,   iftSet *obj,   iftSet *bkg,
...
iftMImage *iftStackGrayImages(int n, ...) {
    ...
    va_list img_list;
    va_start(img_list, n);
      iftImage *img0 = va_arg(img_list, iftImage *);
    ...
    #pragma omp parallel for
    for (int b = 0; b < n; b++) {
          iftImage *img = va_arg(img_list, iftImage *);
...
iftMImageArray *iftReadMImageBatch(iftFileSet *fs, int first, int last)
{
  iftMImageArray *batch = iftCreateMImageArray(last-first+1);

  for (int i = first, j=0; i <= last; i++, j++) {
    batch->val[j]  = iftReadMImage(fs->files[i]->path);
  }

  return(batch);
}

void iftWriteMImageBatch(iftMImageArray *batch, iftFileSet *fs, int first, int last, char *out_path)
{
  char filename[400];
  
  for (int i = first, j=0; i <= last; i++,j++) {
    char *basename = iftFilename(fs->files[i]->path,".mimg");
    sprintf(filename,"%s/%s.mimg",out_path,basename);
    iftWriteMImage(batch->val[j],filename);
    iftFree(basename);
  }
}
