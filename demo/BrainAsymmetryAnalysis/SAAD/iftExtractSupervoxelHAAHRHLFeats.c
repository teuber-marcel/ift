...
void iftSupervoxelHAAHRHLFeatsInImage(  iftImage *img,   iftImage *asym_map,   iftImage *svoxels_right_side,
                                        iftImage *svoxels_left_side, iftDataSet **Zarr, int s, int n_bins,
                                      int max_val, int n_supervoxels);
iftDataSet **iftExtractSupervoxelHAAHRHLFeats(  iftImage *test_img,   iftImage *test_sym_svoxels,
                                                iftFileSet *train_set, int n_bins,   iftImage *normal_asym_map, int *n_svoxels_out);
/*************************************************************/
...
int main(int argc, const char *argv[]) {
    ...
    int n_supervoxels;
    iftDataSet **Zarr = iftExtractSupervoxelHAAHRHLFeats(test_img, test_supervoxels, train_set, n_bins, normal_asym_map, &n_supervoxels);
...
void iftSupervoxelHAAHRHLFeatsInImage(  iftImage *img,   iftImage *asym_map,   iftImage *svoxels_right_side,
                                        iftImage *svoxels_left_side, iftDataSet **Zarr, int s, int n_bins,
                                      int max_val, int n_supervoxels) {
    iftHist **hists_asyms = iftCalcGrayImageHistForLabels(asym_map, svoxels_right_side, n_bins, max_val, true, NULL); // size n_supervoxels + 1
    ...
    #pragma omp parallel for
    for (int label = 1; label <= n_supervoxels; label++) {
        if (Zarr[label]->nfeats != (3 * n_bins)) {
            iftError("Number of Feats for Supervoxel DataSet [%d] is != from the 3 * number of histogram bins: %d != %d",
                        "iftSupervoxelHAAHRHLFeatsInImage", label, Zarr[label]->nfeats, 3 * n_bins);
        }

        for (int b = 0; b < n_bins; b++) {
            Zarr[label]->sample[s].feat[b] = (0.5 / 128) * hists_asyms[label]->val[b];
            Zarr[label]->sample[s].feat[b + n_bins] = (0.25 / 128) * hists_right[label]->val[b];
            Zarr[label]->sample[s].feat[b + (2 * n_bins)] = (0.25 / 0.28) * hists_left[label]->val[b];
        }
        iftDestroyHist(&hists_asyms[label]);
        iftDestroyHist(&hists_right[label]);
        iftDestroyHist(&hists_left[label]);
    }
    iftDestroyHist(&hists_asyms[0]);
...
iftDataSet **iftExtractSupervoxelHAAHRHLFeats(  iftImage *test_img,   iftImage *test_sym_svoxels,
    ...
    for (long s = 0; s < train_set->n; s++) {
        ...
        iftSupervoxelHAAHRHLFeatsInImage(train_img, train_asym_map, svoxels_right_side, svoxels_left_side, Zarr, s, n_bins, norm_val, n_svoxels);
    ...
    iftSupervoxelHAAHRHLFeatsInImage(test_img, test_asym_map, svoxels_right_side, svoxels_left_side, Zarr, n_samples - 1, n_bins, norm_val, n_svoxels);
