    ...
    ui(new Ui::SliceView)
{
    ...
    sliceImageIft = nullptr;
    sliceImageOrig = nullptr;
...
SliceView::~SliceView()
{
    ...
    iftDestroyImage(&sliceImageIft);
    iftDestroyImage(&sliceImageOrig);
    delete ui;
}
...
void SliceView::updateSlicePixmap(QGraphicsPixmapItem *imgPixmap, iftImage *img)
{
    QImage *qimg = iftImageToQImage(img);
    imgPixmap->setPixmap(QPixmap::fromImage(*qimg));
    delete qimg;
}
...
void SliceView::slotUpdateSliceMousePosition(int x, int y)
{
    if (view->isImageEmpty())
        return;
    //qDebug() << "SliceView::slotUpdateSliceMousePosition";
    QString s;
    iftVoxel u,v;
    ...
    img = view->getImage();
     iftImage *norm = view->getNormalizedImage();
    if (iftValidVoxel(img,v)) {
        if (iftIs3DImage(img)) {
            s = QString::asprintf("(%4d, %4d, %4d) = ", v.x, v.y, v.z);
        } else {
            s = QString::asprintf("(%4d, %4d) = ", v.x, v.y);
        }
        ...
        if (iftIsColorImage(img)) {
            ...
            s += QString::asprintf("R:%3d G:%3d B:%3d", rgb.val[0], rgb.val[1], rgb.val[2]);
        } else {
            int val = view->iftGetImageIntensityAtVoxel(v);
            s += QString::asprintf("%4d", val);
        }
...
void SliceView::slotGraphicalViewClicked(int x, int y, Qt::MouseButtons bts, Qt::KeyboardModifiers modifiers)
{
    ...
    if ((x >= sliceImageIft->xsize) || (x < 0) || (y >= sliceImageIft->ysize) || (y < 0))
        return;
    ...
    if (bts == Qt::MiddleButton) {
        ...
        switch (sliceType) {
            case AXIAL:
                h_slice = y;
                v_slice = sliceImageIft->xsize - x;
                break;
            case CORONAL:
                h_slice = sliceImageIft->ysize - y;
                v_slice = sliceImageIft->xsize - x;
                break;
            case SAGITTAL:
                h_slice = sliceImageIft->ysize - y;
        ...
        view->annotation->paintMarkersOnImage(view->orientation, sliceImageIft, point, marker_label, sliceType, getCurrentSliceNumber(), erasing);
        SliceView::updateSlicePixmap(imagePixmapItem, sliceImageIft);
...
void SliceView::slotSaveGraphicalView()
{
    ...
    if (!fileName.isEmpty())
    {
      this->imagePixmapItem->pixmap().toImage().save(fileName);
    }
}
...
void SliceView::showCurrentSlice()
{
    ...
    iftImage *border = nullptr;
    iftImage *label = nullptr;
    iftImage *markers = nullptr;
    iftImage *img = nullptr;
    ...
    if (!view->isLabelEmpty()){
        label = getCurrentLabelSlice();
        t = view->getColorTable();
        obj_visibility = view->rendition->getObjectsVisibility();
        border = getCurrentBorderSlice();
    }
    iftImage *res = iftCreateSliceViewImage(img,label,border,t,obj_visibility,markers, removal_markers, tm,markers_visibility);
    iftImage *corrected = view->orientation->applyOrientationOnSlice(res, sliceType);
    iftDestroyImage(&sliceImageOrig);
    iftImage *origSlice = getCurrentSliceImage();
    sliceImageOrig = view->orientation->applyOrientationOnSlice(origSlice, sliceType);
    updateSlicePixmap(imagePixmapItem, corrected);
    sliceImageIft = corrected;

    iftDestroyImage(&res);
    iftDestroyImage(&origSlice);
    ...
    Segmentation *segMethod = view->currentSegmentationMethod();
    if (segMethod) {
        segMethod->renderGraphics(sliceImageOrig, sliceImageIft, imagePixmapItem, sliceType);
    }

    total_time += elapsedTime.elapsed();
    n++;
    iftWriteOnLog("Showing axial slice took", elapsedTime.elapsed());
    qDebug() << "Showing slice took an average of" << float(total_time) / n;
}
...
void SliceView::placeOrientationIndicators() {
    ...
    orientationIndicators[0]->setPos(Scorrection,(sliceImageIft->ysize/2)-halfLetterWidth);
    halfLetterWidth = int(orientationIndicators[1]->textWidth())/2;
    orientationIndicators[1]->setPos(sliceImageIft->xsize-Lcorrection,(sliceImageIft->ysize/2)-halfLetterWidth);
    orientationIndicators[2]->setPos((sliceImageIft->xsize/2)-halfLetterHeight,Scorrection);
    orientationIndicators[3]->setPos((sliceImageIft->xsize/2)-halfLetterHeight,(sliceImageIft->ysize)-Lcorrection);
}
