...
def main():
    if len(sys.argv) != 5:
        sys.exit(f"python {sys.argv[0]} <result_dir> <template_obj_mask> <gt_dir> <out_csv_path>\n"
                 "E.g:\n"
                 "result_dir: + exps/max_attenuation_factor_1.0/exp_4.0/hemispheres\n"
                 "                + 000003_000001\n"
                 "                    - result.nii.gz\n"
                 "                ...\n"
                 "template_obj_mask: template/cropped/mni152_nonlinear_sym_hemispheres.nii.gz\n\n"
    ...
    template_mask = ift.ReadImageByExt(template_mask_path).AsNumPy()
    ...
    for image_key in image_key_list:
        ...
        detection_data_in_mask = np.array(detection_data)
        detection_data_in_mask[template_mask == 0] = 0
        
        gt_data = ift.ReadImageByExt(gt_path).AsNumPy()
        gt_data_in_mask = np.array(gt_data)
        gt_data_in_mask[template_mask == 0] = 0
        ...
        for obj in range(1, n_objs + 1):
            ...
            gt_obj_bool_data = np.array(gt_data_in_mask == obj)
            intersec_data = detection_data_in_mask * gt_obj_bool_data
            intersec_bin_data = intersec_data != 0

            false_positives_data = detection_data_in_mask - intersec_data
    ...
    template_mask_volume = template_mask[template_mask != 0].size
    print(f"template_mask_volume = {template_mask_volume}\n\n")
    ...
    print(f"# False Positive Rate: {n_false_positive_voxels_mean / template_mask_volume}")
