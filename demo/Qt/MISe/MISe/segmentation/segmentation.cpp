...
void Segmentation::renderGraphics(iftImage *, iftImage *, QGraphicsPixmapItem *, int)
{
...
void Segmentation::updateLabel(QList<iftImage*> label)
{
    parent->setSystemMode("OPEN_LABEL");

    int current_label_index = label.size() == 1 ? this->parent->currentTime() : 0;

    updateProgress(0.65, "Post-processing image.");
    for (iftImage* &l: label) {
        iftImage *aux = PostProcessing::instance(parent)->applyPipelineOnImage(l);
        iftDestroyImage(&l);
        l = aux;
    }
    ...
    for (iftImage *&l: label) {
        if (l) {
            ...
            view->setLabel(l, current_label_index++, ctb, false);
