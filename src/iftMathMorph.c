...
iftImage *iftHOpen(  iftImage *img, int H,   iftImage *mask)
{
  ...
  if (H < 0)
      iftError("Invalid height for opening domes", "iftHOpen");
...
iftImage *iftCloseBasins(  iftImage *img, iftSet *seed,   iftImage *_mask)
{
  iftImage   *cbas=NULL;
  iftGQueue  *Q=NULL;
  int         i,p,q,tmp;
...
iftImage *iftOpenDomes(  iftImage *img, iftSet *seed,   iftImage *_mask)
{
  iftImage   *odom=NULL;
  iftGQueue  *Q=NULL;
  int         i,p,q,tmp;
...
iftImage *iftLeveling(iftImage *orig, iftImage *impaired)
{
  ...
  if (iftIs3DImage(impaired))
    A = iftSpheric(1.0);
  else
    A = iftCircular(1.0);
  
  iftImage *dil = iftDilate(impaired, A, NULL);
  iftImage *X   = iftAnd(dil, orig);
  iftDestroyImage(&dil);
  iftImage *IR  = iftInferiorRec(orig, X, NULL);
  iftDestroyImage(&X);    
  iftImage *ero = iftErode(orig, A, NULL);
  iftImage *Y   = iftOr(ero,IR);
  iftDestroyImage(&ero);      
  iftImage *SR  = iftSuperiorRec(IR, Y, NULL);
  iftDestroyImage(&Y);    
  iftDestroyImage(&IR);    
