...
def build_argparse():
    prog_desc = \
'''
Extract supervoxels for a given attention map.

This script performs the ISF method for an input image as follows:
If a target image (used somehow to generate the attention map) is passed, ISF
is performed in the concatenation between the input image and its target image (multi-band image).
Otherwise, ISF only performs on the input image.
...
'''
    ...
    parser.add_argument('-i', '--image', type=str, required=True, help='Registered Image pathname..')
    ...
    parser.add_argument('-o', '--out-supervoxel-image', type=str, required=True, help='Output Supervoxel Image.')

    parser.add_argument('-r', '--target-image', type=str, required=False, help='Target Image used to generate the attention map.')
...
def print_args(args):
    ...
    print(f'- Attention Map: {args.attention_map}')
...
def validate_args(args, label_img):
    ...
    if len(args.threshold_otsu_factors) != len(objs):
        if len(args.threshold_otsu_factors) == 1:
            args.threshold_otsu_factors = args.threshold_otsu_factors * len(objs)
        else:
    ...
    if len(args.min_euclidean_distances) != len(objs):
        if len(args.min_euclidean_distances) == 1:
            args.min_euclidean_distances = args.min_euclidean_distances * len(objs)
        else:
    ...
    if len(args.number_seeds_on_correct_regions) != len(objs):
        if len(args.number_seeds_on_correct_regions) == 1:
            args.number_seeds_on_correct_regions = args.number_seeds_on_correct_regions * len(objs)
        else:
...
def main():
    ...
    target_image = None
    if args.target_image:
        target_image = ift.ReadImageByExt(args.target_image)
    ...
    number_seeds_on_correct_regions = np.insert(args.number_seeds_on_correct_regions, 0, -1).astype(np.int32)

    svoxel_img = ift.ISFOnAttentionMap(img, attention_map, target_image, label_img,
                                       ift.CreateDblArrayFromNumPy(alpha_factors),
                                       ift.CreateDblArrayFromNumPy(beta_factors),
                                       ift.CreateDblArrayFromNumPy(threshold_otsu_factors),
                                       ift.CreateDblArrayFromNumPy(min_euclidean_distances),
