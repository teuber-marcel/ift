 ...
 * @date Sep 21, 2018
 */
void _iftPerformDIFTForGridSampling(  iftImage *img,   iftLabeledSet *seeds,   iftAdjRel *A,
...
iftLabeledSet *     iftReadSeeds(  iftImage *img, const char *_filename, ...)
{
    va_list args;
	...
	iftError("Reading error", "iftReadSeeds");
      ...
      float scale[2];
      scale[0] = (float) img->xsize / (float) w; 
      scale[1] = (float) img->ysize / (float) h;
      v.z = 0;
      
      for (i=0; i < nseeds; i++){
	if (fscanf(fp,"%d %d %d %d %d",&v.x,&v.y,&mk,&l, &handicap)!=5)
	  iftError("Reading error", "iftReadSeeds");
	...
	if (iftValidVoxel(img,v)){
	  iftUnionLabeledSetElemMarkerAndHandicap(&seed, iftGetVoxelIndex(img,v), l, mk, handicap);
	}
      }
    } else {
	...
	iftError("Reading error", "iftReadSeeds");
      ...
      scale[0] = (float) img->xsize / (float) w; 
      scale[1] = (float) img->ysize / (float) h;
      scale[2] = (float) img->zsize / (float) d;
...
iftLabeledSet *iftMReadSeeds(iftMImage *img, char *filename)
{
    ...
    // SEmentes de 
	  iftUnionLabeledSetElemMarkerAndHandicap(&seed, iftMGetVoxelIndex(img,v), l, mk, handicap);
	}
      }
    } else { /* 3D image */
      if (fscanf(fp,"%d %d %d %d",&nseeds, &w, &h, &d)!=4) 
...
void iftWriteSeeds(iftLabeledSet* seed,   iftImage* img, const char *_filename, ...)
{
    va_list args;
    char filename[IFT_STR_DEFAULT_SIZE];
    va_start(args, _filename);
...
void iftMaskImageToSet(  iftImage *bin_img, iftSet **S) {
    if (bin_img == NULL)
...
void iftSetToImage(  iftSet *S, iftImage *img, int obj) {
    for (  iftSet *node = S; node != NULL; node = node->next)
        img->val[node->elem] = obj;
}

void iftListToImage(  iftList *L, iftImage *img, int obj) {
    for (  iftNode *node = L->head; node != NULL; node = node->next)
        img->val[node->elem] = obj;
}
...
void iftLabeledSetToImage(  iftLabeledSet *S, iftImage *img, bool increment_label) {
    for (  iftLabeledSet *node = S; node != NULL; node = node->next)
...
iftImage *iftComponentArea(iftImage *bin, iftAdjRel *Ain)
{
  iftImage *label;
  int      *size, ncomps, p;
  iftAdjRel *A;
...
(  iftImage *binMask, int initialObjVoxelIdx, int nSamples)
{
...
(  iftImage *label, iftImage *mask, int nSeeds )
{
  // Variables
...
iftIntArray *iftGridSamplingByMaxPathValues(  iftImage *img,   iftImage *bin_mask, long n_samples) {
...
iftIntArray *iftHybridGridSamplingOnMask(  iftImage *img,   iftImage *bin_mask, int n_samples) {
...
iftIntArray *iftHybridGridSamplingOnMaskISFCostFunction(  iftImage *img,   iftImage *bin_mask, int n_samples,
...
{  
  /* Label components from 1 to n */

  iftImage *aux1 = iftFastLabelComp(bin,NULL);
    
  /* Eliminate components near the image's border (frame) */

  iftImage *aux2 = iftCreateImage(aux1->xsize, aux1->ysize, aux1->zsize);
  int  nlabels           = iftMaximumValue(aux1);
  int *label_for_removal = iftAllocIntArray(nlabels+1);

  if (frame_dz >= 0) { /* For 3D images only */
    for (int p = 0; p < aux1->n; p++){
      if (bin->val[p]!=0){
	iftVoxel u = iftGetVoxelCoord(aux1,p);
	if ((u.x <= frame_dx)||(u.y <= frame_dy)||(u.z <= frame_dz)||
	    (u.x >= aux1->xsize - 1 - frame_dx) ||
	    (u.y >= aux1->ysize - 1 - frame_dy) ||
	    (u.z >= aux1->zsize - 1 - frame_dz) ){
	  label_for_removal[aux1->val[p]]=1;
	} 
      }
    }
  } else { /* For 2D and 3D images when frame_dz < 0 */
    for (int p = 0; p < aux1->n; p++){
      if (bin->val[p]!=0){
	iftVoxel u = iftGetVoxelCoord(aux1,p);
	if ((u.x <= frame_dx)||(u.y <= frame_dy)||
	    (u.x >= aux1->xsize - 1 - frame_dx) ||
	    (u.y >= aux1->ysize - 1 - frame_dy) ){
	  label_for_removal[aux1->val[p]]=1;
	} 
  ...
  for (int p=0; p < aux1->n; p++) {
    if (label_for_removal[aux1->val[p]]==0){
      aux2->val[p] = bin->val[p];
    }
  }

  iftDestroyImage(&aux1);
  iftFree(label_for_removal);
