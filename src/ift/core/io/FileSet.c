...
iftFileSet *iftCopyFileSet(  iftFileSet* fileset) {
    if (fileset == NULL)
        return NULL;
...
void iftWriteFileSetAsCSV(  iftFileSet *fset, const char *format, ...) {
    va_list args;
...
void iftRemoveFileSet(  iftFileSet *fset) {
    if (fset != NULL) {
        for (long i = 0; i < fset->n; i++)
...
iftFileSet *iftMergeFileSet(  iftFileSet *fset1,   iftFileSet *fset2) {
    if (fset1 == NULL)
...
iftFileSet *iftFilterFileSetByFilename(  iftFileSet *src,   iftFileSet *dst) {
...
iftFileSet *iftFilterFileSetByIndexes(  iftFileSet *fset,   iftIntArray *idxs) {
...
int iftFindFileByLabel(  iftFileSet *files, int label) {
    int i, file = IFT_NIL;
...
int iftFindFileByLabelAndSample(  iftFileSet *files, int label, int sample) {
    int i, file = IFT_NIL;
...
int iftFindFileByBasename(  iftFileSet *files, const char *bname, const char *suffix) {
    int i, file = IFT_NIL;
...
int iftFileSetLabelsNumber(  iftFileSet* fileset) {
...
int* iftFileSetLabels(  iftFileSet* fileSet) {
    int n = fileSet->n;
    int i;
...
iftCSV *iftFileSetToCSV(  iftFileSet *files) {
...
iftDict *iftFileSetToDict(  iftFileSet *fset) {
    if (fset == NULL)
...
void iftValidateFileSet(  iftFileSet *img_files) {
    if (img_files->n == 0)
...
iftFileSet *iftExtractClassFileset(  iftFileSet *fileset, int label)
{
    int nSamp = 0;
    for(int i = 0; i < fileset->n; i++) {
        iftUpdateDataSetFileInfo(fileset->files[i]);
