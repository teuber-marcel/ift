...
iftLabeledSet *iftIGraphISF_Resuming_Root(iftIGraph *igraph,   iftImage *input_label, iftDHeap *Q, iftImage *seeds,
...
ulong iftSelectSeedsWithinDistance(iftVoxel center_marker_voxel,   iftImage *gt_image,   iftImage *dist,
                                     iftImage *root,   iftImage *error_components, iftImage *seed_image,
...
void iftIGraphDiffISF_Resuming_Root(iftIGraph *igraph,   iftImage *input_label, iftLabeledSet *new_seeds,
...
iftLabeledSet *iftLabelToForestISF_Root(  iftImage *img,   iftImage *input_label, double alpha, double beta,
...
//}
//
//void iftIGraphDiffISF_IFT_Resuming_Mean(iftIGraph *igraph,   iftImage *input_label, iftDHeap *Q, double *pvalue,
                                                              ...
                                                              bool select_seeds_on_axes_only)
{
    ...
    do
    {
        ...
        iftDrawSeeds(tmp_img, all_delineation_seeds);
...
int iftFurthestInError(  iftImage *source,   iftImage *target,   iftImage *mask, int from)
{
...
iftSet *_sort_along_contour(  iftImage *obj,   iftImage *contour,   iftSet *S)
{
    ...
    for (  iftSet *s = S, *prev = NULL; s; prev = s, s = s->next)
    {
                                     ...
                                     iftImage *gt_mask,   iftSet *anchors)
{
    ...
    for (  iftSet *s = anchors; s; s = s->next)
    {
        int p = s->elem;
        dist->val[p] = 0;
        roots->val[p] = p;
