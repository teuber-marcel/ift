...
void iftWriteFeatures(  iftFeatures *feats, const char *filename) {
    if (filename == NULL)
        iftError("filename is NULL", "iftReadFeatures");
...
iftImageArray *ift3DLBPTOP(  iftImage *img) {
    if (!iftIs3DImage(img))
...
iftFeatures *iftExtract3DLBPTOPFeats(  iftImage *img,   iftImage *mask,
...
iftFeatures *iftExtractVLBPFeats(  iftImage *img,   iftImage *mask,
...
iftFeatures *iftExtractBIC(  iftImage *img,   iftImage *bin_mask_in,
...
void iftHogComputeClosestCellCenters(  iftHog* hog,   iftVoxel v, iftVoxel centers[4]) {
...
void iftHogClosestOrientationBins(  iftHog *hog, float orientation, float *bins) {
...
iftFeatures *iftExtractHOG2D(  iftHog *hog, iftImage *img) {
