...
FEAT_EXTRACTION_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/AttentionMap/extract_svoxel_texture_feats.py"
...
os.system("make iftMeanDiffMapWithImageSet -C $NEWIFT_DIR/demo/BrainAnomalyDetection/RegistrationErrorEDT")
...
'''
    ...
    parser.add_argument('-n', '--bins', type=int, default=128,
                        help='Number of bins of the 3D LBP. Default: 128.')
...
def main():
    ...
    if not args.skip_feat_extraction:
        ...
        print("\n\n####### Feature Extraction")
        feat_extract_cmd_args = f"--image {args.image} --attention-map {reg_error_path} " \
                                f"--supervoxels {svoxels_path} --training-set {args.training_set} " \
                                f"--training-attention-map-set {args.training_reg_error_set} " \
