...
namespace bia{
  namespace Curve{
    ...
    Curve *Create(int n){
      ...
      if(curve != NULL){
	curve->X = bia::AllocFloatArray(n);
	curve->Y = bia::AllocFloatArray(n);
	curve->n = n;
      } 
      else bia::Error((char *)MSG1,
		      (char *)"Curve::Create");
      return(curve);
    }
    ...
    void   Destroy(Curve **curve){
      ...
      aux = *curve;
      if(aux != NULL){
	if(aux->X != NULL) bia::FreeFloatArray(&aux->X);
	if(aux->Y != NULL) bia::FreeFloatArray(&aux->Y);
	free(aux);
	*curve = NULL;
      }
    }
    ...
    Curve *Read(char *filename){
      ...
      fp = fopen(filename,"r");
      if(fp == NULL)
	bia::Error((char *)"Cannot open file",
		   (char *)"Curve::Read");
