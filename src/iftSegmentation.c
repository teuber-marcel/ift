 ...
 */
typedef float (*iftDistColorVoxels)(  iftImage *img, int p, int q);


float iftDistColorVoxelsColorImage(  iftImage *img, int p, int q) {
...
float iftDistColorVoxelsGreyImage(  iftImage *img, int p, int q) {
...
void iftMeansAboveBelowT(  iftImage *img, int T, int *T1, int *T2) {
...
iftImage *iftBrainGrad(  iftImage *brain_img) {
    int a, b, c;
...
iftImage *iftApplySShape(  iftImage *img, int a, int b, int c) {
...
iftImage *iftTextGradient(  iftImage *img) {
    float   dist,gx,gy,gz;
    int     i,p,q;
    iftVoxel   u,v;
...
iftImage *iftComputeGradient(  iftImage *img, iftGradientAlg grad_alg) {
...
iftImage *iftWatershedWithPredMap(  iftImage *basins, iftAdjRel *Ain, iftLabeledSet *seeds, iftSet *forbidden, iftImage **pred)
{
...
iftImage *iftWatershedMeanCut(  iftImage *basins, iftAdjRel *Ain,   iftLabeledSet *seeds,
                                iftSet *forbidden, float *result) {
    if (basins == NULL)
...
iftImage *iftWaterGray(iftImage *basins, iftImage *marker, iftAdjRel  *A)
{
  ...
  label   = iftCreateImage(basins->xsize,basins->ysize,basins->zsize);
  Q       = iftCreateGQueue(iftMaximumValue(marker)+2,marker->n,marker->val);
  ...
  while(!iftEmptyGQueue(Q)) {
    ...
    basins->val[p] = marker->val[p]; // set the reconstruction value
	...
	q = iftGetVoxelIndex(basins,v);
	if (Q->L.elem[q].color != IFT_BLACK){
	  tmp = iftMax(marker->val[p], basins->val[q]);
	  if (tmp < marker->val[q]){ 
	    iftRemoveGQueueElem(Q,q);
	    label->val[q]      = label->val[p];
	    marker->val[q]    = tmp;
...
iftImage *iftDualWaterGray(iftImage *domes, iftImage *marker, iftAdjRel  *A)
{
  ...
  while(!iftEmptyGQueue(Q)) {
    ...
    domes->val[p] = marker->val[p]-1; // set the reconstruction value
...
iftImage *iftFastSmoothObjects(  iftImage *labelIn,   iftFImage *weight, int n_iters) {
  iftAdjRel *A;
  iftImage  *label2;
...
*/
float iftFScore(iftImage *bin, iftImage *gt)
{
  float fscore = 0.0;
...
float *iftFScoreMultiLabel (iftImage *mlabel, iftImage *mlgt, int number_of_objects)
{
    ...
    for (k = 1; k <= number_of_objects; k++)
    {
        ...
        output[k] = iftFScore(tmp_gt, tmp_label);
        accu_sum += output[k];
...
/**
@brief Jaccard index for object detection.\n

Given a groundtruth and binary image, compute the Jaccard index.\n
 
@param  bin                 iftImage.    Binary image. 0 for background and 1 for object.
@param  gt                  iftImage.    Correct segmentation. 0 for background and 1 for object.

@return A float with the Jaccard index [0, 1].
*/

float iftJaccard(iftImage *bin, iftImage *gt)
{
  unsigned long n_intersection = 0;
  unsigned long n_union = 0;
  iftVerifyImageDomains(bin,gt,"iftJaccard");
  ...
  if (n_union == 0)
    ...
    return((float)n_intersection/(float)n_union);
}
...
iftLabeledSet *iftShiftCoordsFromLabeledSet(  iftLabeledSet *S,   iftImage *img, int dx, int dy, int dz) {
...
iftSet *iftShiftCoordsFromSet(  iftSet *S,   iftImage *img, int dx, int dy, int dz) {
    iftSet *shift = NULL;
...
iftImage *iftConnectedThreshold(  iftImage *img,   iftAdjRel *A, iftImage *marker,   int minthres,   int maxthres)
{
...
iftImage *iftMajorityVoting(  iftImage *img,   iftIntMatrix *label_occurs) {
...
iftImage *iftConnCompLabeling(  iftImage *label_img, int *n_objs) {
...
// private
static int _get_direction(  iftImage *obj,   iftAdjRel *A, int src)
{
...
float iftGetIoUDetection(iftBoundingBox gt_bb, iftBoundingBox pred_bb) { 
  ...
  width = iftMin(gt_bb.end.x, pred_bb.end.x) 
          - iftMax(gt_bb.begin.x, pred_bb.begin.x);
  height = iftMin(gt_bb.end.y, pred_bb.end.y) 
          - iftMax(gt_bb.begin.y, pred_bb.begin.y);
  volume = iftMin(gt_bb.end.z, pred_bb.end.z) 
        - iftMax(gt_bb.begin.z, pred_bb.begin.z);
  if (width <= 0 || height <= 0 || volume < 0) {
    return 0.0;
  }
  // Volume equals 1 for 2D IoU
  volume = iftMax(1, volume);
  
  int intersection_area = width * height * volume;

  width = gt_bb.end.x - gt_bb.begin.x;
  height = gt_bb.end.y - gt_bb.begin.y; 
  volume = iftMax(1, gt_bb.end.z - gt_bb.begin.z); 
  int gt_area = width * height * volume;

  width = pred_bb.end.x - pred_bb.begin.x;
  height = pred_bb.end.y - pred_bb.begin.y; 
  volume = iftMax(1, pred_bb.end.z - pred_bb.begin.z); 
  int pred_area = width * height * volume;

  int union_area = gt_area + pred_area - intersection_area;

  float IoU = (float) intersection_area / (float) union_area;

  return IoU;
}

iftImage *iftDynamicTrees(iftMImage *mimg, iftImage *seeds_in, iftImage *seeds_out){
  iftImage   *pathval  = NULL, *label = NULL, *root = NULL;
  iftMImage  *tree   = NULL;
  int        *nnodes   = NULL;
  int Imax             = iftRound(sqrtf(mimg->m)*iftMMaximumValue(mimg,-1));
  iftGQueue  *Q        = NULL;
  iftAdjRel  *A; 
  int         i, p, q, r, tmp;
  iftVoxel    u, v;
  ...
  tree       = iftCreateMImage(mimg->xsize, mimg->ysize, mimg->zsize, mimg->m);
  Q          = iftCreateGQueue(Imax+1, mimg->n, pathval->val);
  nnodes     = iftAllocIntArray(mimg->n);
    ...
    for (i = 1; i < A->n; i++){
      ...
      if (iftValidVoxel(mimg, v)){
        ...
        if (Q->L.elem[q].color != IFT_BLACK) {
          float Wi = 0.0;	  
	  for (int b=0; b < mimg->m; b++)
	    Wi +=  powf(mimg->val[q][b]-(tree->val[r][b]/nnodes[r]),2.0);
	  
	  Wi = iftRound(sqrtf(Wi));
      
          tmp = iftMax(pathval->val[p], (int)Wi);
...
iftImage *iftDelineateEggComp(
  iftImage *comps, iftSet *SelectedComps, iftMImage *mimg,
  float seed_erosion, float seed_dilation
)
{
  /* Set inner markers */
  ...
  iftImage *label = iftDynamicTrees(
    mimg, seeds_in, seeds_out
  );
  iftDestroyImage(&seeds_in);
...
iftImage *iftSelectEggCompFromSaliencyMap(
  iftImage *saliency, int border_dist,
  int min_obj_area, int max_obj_area
) 
{
  if (iftMaximumValue(saliency) > 0) {
    iftImage *aux1 = NULL, *aux2 = NULL;
    /* Detect binary components */
    aux1 = iftThreshold(
    			saliency, iftOtsu(saliency), IFT_INFINITY_INT, 255
    );

    /* Eliminate components near the image's border */
    if (border_dist == -1) {
      aux2 = iftCopyImage(aux1);
    }
    else {
      aux2 = iftRemoveCompNearFrame(aux1, border_dist, border_dist, -1);
    }

    /* Select components within a given area interval */
    iftDestroyImage(&aux1);
    aux1 = iftSelectCompInAreaInterval(
      aux2, NULL, min_obj_area, max_obj_area
    );

    /* Label componentes and select the candidates */
  ...
  // If return was not reached, return an mask with 0's only 
  else {
    return saliency;
  }
}

iftImage *iftSMansoniDelineation(
  iftMImage *mimg, iftImage *saliency, int border_dist,
  int min_obj_area, int max_obj_area, float seed_erosion,
  float seed_dilation
)
{
  iftImage *aux1 = NULL, *aux2 = NULL;
  ...
  if ((Lmax=iftMaximumValue(aux1)) > 0) {
    /* iftSet *SelectedComps = iftSalientCompsAboveOtsu(aux1, saliency);  */
    /* if (SelectedComps != NULL) {  */
      //Select all components
    iftSet *SelectedComps = NULL;
    for (int j=1; j <= Lmax; j++)
      iftInsertSet(&SelectedComps, j);
    // Delineate components
    aux2 = iftDelineateEggComp(aux1, SelectedComps, mimg, \
			       seed_erosion, seed_dilation);
    /* Select components within a given area interval */
    iftDestroyImage(&aux1);
    aux1 = iftSelectCompInAreaInterval(aux2, NULL, \
  ...
  // If return was not reached, return an mask with 0's only
  iftImage *bin = iftCreateImageFromImage(saliency);
...
void iftFilterSaliencyByAreaIntervalInPlace(iftFImage *saliency, int min_obj_area, int max_obj_area)
{
  // A area filter similar based on iftSelectCompInAreaInterval that return the original saliency map in continuos range 
  // Binarization with a lower bound of 0 is applied before selecting components based on the area interval

  iftImage *bin_sal = iftCreateImage(saliency->xsize, saliency->ysize, saliency->zsize);
  int p;
  int sum = 0;
  for (p = 0; p < bin_sal->n; p++)
  {
    if (saliency->val[p] > 0.0f)
    {
      bin_sal->val[p] = 1;
      sum = sum + 1;
    }
  ...
  iftImage *bin_sal_filtered = iftSelectCompInAreaInterval(bin_sal, NULL, min_obj_area, max_obj_area);

  // Update the saliency map based on the filtered binary image
  for (p = 0; p < bin_sal->n; p++)
  {
    if (bin_sal_filtered->val[p] == 0)
    {
      saliency->val[p] = 0.0f; // Remove corresponding components in saliency map
    }
  }

  iftDestroyImage(&bin_sal);
  iftDestroyImage(&bin_sal_filtered);

  // printf("\n[INFO] Area binary is %d\n", sum);
}

void Min_Max_Normalize(iftFImage *fimg, int scaling_value, bool apply_sigmoid)
{

  if (scaling_value <= 0)
  {
    iftError("The scaling value must be higher than 0.", "Min_Max_Normalize");
  }
  ...
  if (apply_sigmoid == true)
  {
    for (p = 0; p < fimg->n; p++)
    {
      if (fimg->val[p] > 0)
      {
        fimg->val[p] = 1.0f / (1.0f + expf(-fimg->val[p]));
      }
    }
  }
  ...
  for (p = 0; p < fimg->n; p++)
  {
    if (fimg->val[p] > 0)
    {
      if (fimg->val[p] < min_val)
      {
        min_val = fimg->val[p];
      }
      if (fimg->val[p] > max_val)
      {
        max_val = fimg->val[p];
      }
    }
  ...
  for (p = 0; p < fimg->n; p++)
  {
    if (fimg->val[p] > 0)
    {
      fimg->val[p] = ((fimg->val[p] - min_val) / (max_val - min_val)) * scaling_value;
    }
  }
}


void iftCopyFImageInPlace(iftFImage *src, iftFImage *dest)
{
  int p;

  iftVerifyFImageDomains(src, dest, "iftCopyFImageInPlace");
...
iftFImage *iftRelaxedDynamicTrees(iftMImage *mimg, iftImage *seeds_in, iftImage *seeds_out, iftFImage *arc_costs, iftFImage *salie_orig)
{
  iftImage *pathval = NULL, *root = NULL;
  iftFImage *label = NULL;
  iftMImage *tree = NULL;
  iftFImage *propagated_saliency_map = NULL;
  int *nnodes = NULL;
  int Imax = iftRound(sqrtf(mimg->m) * iftMMaximumValue(mimg, -1));
  iftGQueue *Q = NULL;
  iftAdjRel *A;
  ...
  pathval = iftCreateImage(mimg->xsize, mimg->ysize, mimg->zsize);
  label = iftCreateFImage(mimg->xsize, mimg->ysize, mimg->zsize);
  propagated_saliency_map = iftCreateFImage(mimg->xsize, mimg->ysize, mimg->zsize);
  root = iftCreateImage(mimg->xsize, mimg->ysize, mimg->zsize);
  tree = iftCreateMImage(mimg->xsize, mimg->ysize, mimg->zsize, mimg->m);
  Q = iftCreateGQueue(Imax + 1, mimg->n, pathval->val);
  nnodes = iftAllocIntArray(mimg->n);
  /*
  float max_salie_value = FLT_MIN;

  if (salie_orig != NULL)
  {
    for (int p = 0; p < seeds_in->n; p++)
    {
      if (seeds_in->val[p] != 0)
      { // just intern seed
        if (salie_orig->val[p] > max_salie_value)
        {
          max_salie_value = salie_orig->val[p];
        }
      }
    }
  }
 ...
 // float sum = 0.0;
  ...
  if (salie_orig != NULL)
  {
    for (int p = 0; p < seeds_in->n; p++)
    {
      if (seeds_in->val[p] != 0)
      { // just intern seed
        salie_orig->val[p] = salie_orig->val[p];
       // sum += salie_orig->val[p];
     //   count++;
      }
      else
      {
  ...
  for (p = 0; p < mimg->n; p++)
  {
    ...
    if (seeds_in->val[p] != 0)
    {
      ...
      if (salie_orig != NULL)
      {
        label->val[p] = 1.0f; // mean; 
        propagated_saliency_map->val[p] = (float)salie_orig->val[p];
      }
    ...
    else
    {
      if (seeds_out->val[p] != 0)
      {
        root->val[p] = p;
        label->val[p] = 0.0f;
        propagated_saliency_map->val[p] = 0.0f;
        pathval->val[p] = 0;
      }
    }
    iftInsertGQueue(&Q, p);
        ...
        if (Q->L.elem[q].color != IFT_BLACK)
        {
          ...
          if (tmp < pathval->val[q])
          {
            ...
            /*if (label->val[p] != 0)
            {
              ...
              if (salie_orig != NULL)
              {
                propagated_saliency_map->val[q] = propagated_saliency_map->val[p];
              }
            }
            else
            {
              ...
              if (salie_orig != NULL)
              {
                propagated_saliency_map->val[q] = 0.0f;
              }
            }*/
  ...
  if (salie_orig != NULL)
  {
    ...
    iftCopyFImageInPlace(propagated_saliency_map, salie_orig);
  }
  return (label);
}


void CombineSaliencyByAlphaConfidence(iftFImage *S1, iftFImage *S2, float alpha)
{
    for (int p = 0; p < S1->n; p++)
    {
       
        // fusion rule
        S1->val[p] =  S1->val[p]  + alpha * S2->val[p];
    }
}


void relaxationProcedureOptimized(iftFImage *L0, iftFImage *costs, iftImage *seeds_in, iftImage *seeds_out, int iterations)
{
  /*
      Updates the label of pixel based on the weighted average of its neighbors' labels based on arc cost w(p,q) returned from Dynamic Trees.
      The seed points \( S \) remain unchanged.
  */

  iftAdjRel *Ain = NULL;
  iftFImage *LN = iftFCopyImage(L0);
  iftIntMatrix *stationaryPoints = iftCreateIntMatrix(LN->xsize, LN->ysize);
  ...
  {
    iftFImage *newLN = iftFCopyImage(LN);
    // #pragma omp paralell for schedule(dynamic) reduction(+:sumWeights, weightedSum)
    for (int p = 0; p < L0->n; p++)
    {
      // skip stationary points
      if (stationaryPoints->val[p] == 1)
      {
        continue;
      }
      ...
      if (seeds_in->val[p] != 0 || seeds_out->val[p] != 0)
      {
        // check stationarity: skip seed points
        continue;
      }
     

      iftVoxel u = iftFGetVoxelCoord(L0, p);

      float sumWeights = 0.0;
      float weightedSum = 0.0;
      int isStationaryPoint = 0;

      for (int j = 1; j < Ain->n; j++)
      {
        iftVoxel v = iftGetAdjacentVoxel(Ain, u, j);
        if (iftFValidVoxel(L0, v))
        {
          int q = iftFGetVoxelIndex(L0, v);
          float wPQ = costs->val[q];
          // the inverse of arch cost (weight) is the local similarity in range [0,1]
          float inverseWeight = 1 / (1 + wPQ);
          // printf("%f, ->, %f\n", wPQ, inverseWeight);

          sumWeights += inverseWeight;
          weightedSum += inverseWeight * LN->val[q];

          // check stationarity: all neighbors have the same value
          if ((int)LN->val[p] == (int)LN->val[q])
          {
            isStationaryPoint++;
          }
        }
      ...
      if (isStationaryPoint == Ain->n)
      {
        stationaryPoints->val[p] = 1; // mark point as stationary
      }

      float newLabel = weightedSum / sumWeights;
      if (newLabel < 0.01)
      { // to avoid nano values around boundaries
        newLabel = 0.0;
      }

      newLN->val[p] = newLabel;
    }
  ...
  iftCopyFImageInPlace(LN, L0);
  iftDestroyFImage(&LN);
  iftDestroyIntMatrix(&stationaryPoints);
}



iftFImage *iftRelaxedDelineateEggComp(
    iftImage *comps, iftSet *SelectedComps, iftMImage *mimg,
    float seed_erosion, float seed_dilation, int relaxation_iterations,  iftImage *salie_orig, float alpha_confidence)
{
  ...
  iftFImage *costs = iftCreateFImage(seeds_in->xsize, seeds_in->ysize, seeds_in->zsize);

  iftFImage *salie_orig_fimg = iftImageToFImage(salie_orig);// store propagated saliency values

  iftFImage *label = iftRelaxedDynamicTrees(
      mimg, seeds_in, seeds_out, costs, salie_orig_fimg); // return label map

  //printf("[INFO] S1 interval is %f %f ", iftFMaximumValue(label), iftFMinimumValue(label));
  //printf("[INFO] S2 interval is %f %f ", iftFMaximumValue(salie_orig_fimg), iftFMinimumValue(salie_orig_fimg));
  
  Min_Max_Normalize(salie_orig_fimg, 1, false); // normalize propagated flim saliency to ensure posterior combination
  /*for(int idx = 0; idx < salie_orig_fimg->n; idx++) {
    if(salie_orig_fimg->val[idx]>0) {
      printf("\n[INFO] saliency value is %f\n", salie_orig_fimg->val[idx]);
    }
  }*/

  /* Combine label map with propagated saliency  */
  CombineSaliencyByAlphaConfidence(label, salie_orig_fimg, alpha_confidence); 

  iftDestroyFImage(&salie_orig_fimg);
 

  /* Apply the Iterative Relaxation */
  relaxationProcedureOptimized(label, costs, seeds_in, seeds_out, relaxation_iterations);
  

  /* TODO Deffuzify values */

  iftDestroyImage(&seeds_in);
  iftDestroyImage(&seeds_out);
  iftDestroyFImage(&costs);
//  iftDestroyFImage(&label);
...
iftFImage *iftSMansoniDelineationADMRDT(
    iftMImage *mimg, iftImage *saliency, int border_dist,
    int min_obj_area, int max_obj_area, float seed_erosion,
    float seed_dilation, int relaxation_iterations, float alpha_confidence)
{
  ...
  if ((Lmax = iftMaximumValue(aux1)) > 0)
  {
    ...
    // Delineate components
    iftFImage *output_salie = iftRelaxedDelineateEggComp(
        aux1, 
        SelectedComps, 
        mimg, 
        seed_erosion, 
    ...
    iftFilterSaliencyByAreaIntervalInPlace(output_salie, min_obj_area, max_obj_area); //[NEW]
  ...
  // If return was not reached, return a mask with 0's only
  iftFImage *zeroed = iftCreateFImage(saliency->xsize,saliency->ysize,saliency->zsize );
