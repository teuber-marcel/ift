...
{
  strcpy(params->image_database, image_database);
  strcpy(params->label_database, label_database);
}

ExperimentResults *createExperimentResults(ExperimentParameters *params)
{
  ExperimentResults *expm = (ExperimentResults *) malloc(sizeof(ExperimentResults) * params->number_of_images);
  for (int i = 0; i < params->number_of_images; i++)
  {
    ...
    expm[i].number_of_supervoxels = 0.0;
    expm[i].setup_time            = 0.0;
    expm[i].sum_final_iteration   = 0;

    expm[i].distance_error_object = iftAllocFloatArray(params->max_number_of_objects + 1);
    expm[i].dice_error_object     = iftAllocFloatArray(params->max_number_of_objects + 1);
...
void destroyExperimentResults(ExperimentResults *expM, ExperimentParameters *params)
{
  ...
  //         for (int k=0; k<= params->max_number_of_objects; k++)
  //             expM[i].distance_error_matrix[j][k] = 99;
  ...
  for (i = 0; i < params->number_of_images; i++)
  {
    ...
    // }
    if (expM[i].arr_final_iteration != NULL)
      free(expM[i].arr_final_iteration);

    if (expM[i].distance_error_object != NULL)
      free(expM[i].distance_error_object);

    if (expM[i].dice_error_object != NULL)
      free(expM[i].dice_error_object);

    expM[i].distance_error_object = NULL;
    expM[i].dice_error_object     = NULL;
    expM[i].arr_final_iteration   = NULL;
...
void loadParameters (ExperimentParameters *params, char *config_filename)
{
  ...
  while (!feof(fp))
  {
    ...
    else if (strcmp(pch, "LABEL_DATABASE") == 0)
    {
      pch = strtok (NULL, ", ");
    ...
    else if (strcmp(pch, "FILE_EXTENSION") == 0)
    {
      pch = strtok (NULL, ", ");
    ...
    else if (strcmp(pch, "RELAXATION") == 0)
    {
      pch = strtok (NULL, ", ");
      params->relaxation = atoi(pch);
    }
    ...
    else if (strcmp(pch, "SMOOTH_ITERATIONS") == 0)
    {
      pch = strtok (NULL, ", ");
      params->smooth_iterations = atoi(pch);
    }
    else if (strcmp(pch, "SMOOTH_FACTOR") == 0)
    {
      pch = strtok (NULL, ", ");
      params->smooth_factor = atof(pch);
    }
    else if (strcmp(pch, "SMOOTH_FOOT") == 0)
    {
      pch = strtok (NULL, ", ");
      params->smooth_foot = atoi(pch);
    }

    /* SUPERVOXELS */
    else if (strcmp(pch, "SUPERVOXEL") == 0)
    {
      pch = strtok (NULL, ", ");
      params->supervoxel = atoi(pch);
    }
    else if (strcmp(pch, "SUPERVOXEL_METHOD") == 0)
    {
      pch = strtok (NULL, ", ");
      params->supervoxel_method = atoi(pch);
    }
    else if (strcmp(pch, "SUPERVOXEL_DESCRIPTOR") == 0)
    {
      pch = strtok (NULL, ", ");
      params->supervoxel_descriptor = atoi(pch);
    }
    else if (strcmp(pch, "BASINS_ADJACENCY") == 0)
    {
      pch = strtok (NULL, ", ");
      params->basins_adjacency = atof(pch);
    }
    else if (strcmp(pch, "SUPERVOXEL_ADJ") == 0)
    {
      pch = strtok (NULL, ", ");
      params->supervoxel_adj = atof(pch);
    }
    else if (strcmp(pch, "SUPERVOXEL_VOLUME") == 0)
    {
      pch = strtok (NULL, ", ");
      params->supervoxel_volume = atof(pch);
    }
...
void nameAssembler(char *dst, char *path, char *algorithm, ExperimentParameters *params)
{
  char *database;
  database = iftSplitStringOld(params->image_database, "/", 2);
  if (strcmp(algorithm, "IFT") == 0)
    sprintf(dst, "%s%s_%s_smoothit=%d_relaxtype=%d_global=%d.txt", path, database, algorithm, params->smooth_iterations, params->relaxation, 0);
  else
    sprintf(dst, "%s%s_%s_beta=%.0f_relaxtype=%d_global=%d.txt", path, database, algorithm, params->gc_beta, params->relaxation, 0);
}
void printHeader(ExperimentParameters *params, char *target, char *algorithm)
{
  ...
  char full_path[256];
  FILE *output = NULL;
  nameAssembler(full_path, "./results/", algorithm, params);
  ...
  if (strcmp(algorithm, "IFT") == 0)
  {
    ...
    if (params->relaxation == 0)
      fprintf(output, "Smoothing: OFF\n");
    else if (params->relaxation == 1)
    {
      fprintf(output, "Smoothing: Smooth Last\n");
      fprintf(output, "Smooth iterations: %d\n", params->smooth_iterations);
    }
    else if (params->relaxation == 2)
    {
      fprintf(output, "Smoothing: Smooth Every Iteration\n");
      fprintf(output, "Smooth iterations: %d\n", params->smooth_iterations);
    }
    fprintf(output, "Smoothing: Differential\n");
  }
  else
  {
...
// void printIterationProfress(ExperimentParameters *params, ExperimentResults *exp_metrics)
// {
//     fprintf(output, "--------------------------------\n");
...
//     }
// }
void printReport(ExperimentParameters *params, ExperimentResults *exp_metrics, char *target, char *algorithm)
{
  FILE *output = NULL;
  int i = 0, j = 0, k = 0;
  char full_path[256];
  ...
  iftCreateDirectory("./results/stats");
  ...
  nameAssembler(full_path, "./results/stats/finalasd_", algorithm, params);
  ...
  {
    for (i = 0; i < params->number_of_images; i++)
      temp_array[i] = exp_metrics[i].distance_error_object[k];
    average = iftMeanFloatArrayDiffZero(temp_array, params->number_of_images);
    std_dev = iftStddevFloatArray(temp_array, params->number_of_images);
    fprintf(output, "%d: %f +- %.2f;\t", k, average, std_dev);
  }
  ...
  nameAssembler(full_path, "./results/stats/finaldice_", algorithm, params);
  saveFinalValues(full_path, temp_array, params->number_of_images);
  average = iftMeanFloatArrayDiffZero(temp_array, params->number_of_images);
  std_dev = iftStddevFloatArray(temp_array, params->number_of_images);
  fprintf(output, "%d: %f +- %.2f;\t", k, average, std_dev);
  ...
  {
    for (i = 0; i < params->number_of_images; i++)
      temp_array[i] = exp_metrics[i].dice_error_object[k];
    average = iftMeanFloatArrayDiffZero(temp_array, params->number_of_images);
    std_dev = iftStddevFloatArray(temp_array, params->number_of_images);
    fprintf(output, "%d: %f +- %.2f;\t", k, average, std_dev);
  }
  ...
  nameAssembler(full_path, "./results/stats/seedsadded_", algorithm, params);
  saveFinalValues(full_path, temp_array, params->number_of_images);
  ...
  memset(temp_array, 0, sizeof(float)*params->number_of_images);
  for (i = 0; i < params->number_of_images; i++)
    temp_array[i] = exp_metrics[i].number_of_supervoxels;
  average = iftMeanFloatArrayDiffZero(temp_array, params->number_of_images);
  std_dev = iftStddevFloatArray(temp_array, params->number_of_images);
  fprintf(output, "Number of supervoxels: %.0f +- %.2f &\t", average, std_dev);
  nameAssembler(full_path, "./results/stats/nbrsupervoxels_", algorithm, params);
  saveFinalValues(full_path, temp_array, params->number_of_images);
  ...
  nameAssembler(full_path, "./results/stats/setuptime_", algorithm, params);
  saveFinalValues(full_path, temp_array, params->number_of_images);
  ...
  nameAssembler(full_path, "./results/stats/finaliteration_", algorithm, params);
  saveFinalValues(full_path, temp_array, params->number_of_images);
  ...
  nameAssembler(full_path, "./results/stats/totaltime_", algorithm, params);
  saveFinalValues(full_path, temp_array, params->number_of_images);
  ...
  for (i = 0; i < params->number_of_images; i++)
  {
    if (exp_metrics[i].final_iteration != 0)
    {
      if(strcmp(algorithm, "DGC") == 0)
        temp_array[i] = exp_metrics[i].total_time / exp_metrics[i].sum_final_iteration * 1.0;
      else
