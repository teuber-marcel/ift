...
FEAT_EXTRACTION_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/RegistrationError/extract_svoxel_feats.py"
...
os.system("make iftRegErrorMagnitude -C $NEWIFT_DIR/demo/BrainAnomalyDetection/RegistrationError")
...
def main():
    ...
    if not args.skip_registration_error_computation:
        print("####### REGISTRATION ERROR MAGNITUDES")

        reg_error_mag_cmd = f"iftRegErrorMagnitude --image-path {args.image} " \
                            f"--template {args.template} --mask {args.label_image} " \
                            f"--out-reg-error-mag-map {reg_error_path}"
        if args.bias:
