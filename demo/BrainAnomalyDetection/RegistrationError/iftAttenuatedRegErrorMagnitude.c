...
int main(int argc, const char *argv[]) {
    ...
    if (iftCompareStrings(function, "none")) {
        weights = iftCreateConstantFImage(label_img->xsize, label_img->ysize, label_img->zsize, 1.0);        
    }
    ...
    iftImage *reg_error = iftRegErrorMagnitude(img, template_img, bias);
    
    #pragma omp parallel for
    for (int p = 0; p < reg_error->n; p++) {
        reg_error->val[p] *= weights->val[p];
    }
