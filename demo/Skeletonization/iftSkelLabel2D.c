...
{
  iftImage       *img=NULL, *skel=NULL, *skel_255=NULL, *border=NULL;
  ...
  if (argc!=2)
    iftError("Usage: iftSkelLabel2D <label.[pgm,png,jpg,tif]>","main");

  A      = iftCircular(sqrtf(2.0));

  img    = iftReadImageByExt(argv[1]);
  
  if (!iftIsBinaryImage(img)){
    iftImage *aux= iftThreshold(img,127,255,255);
    iftDestroyImage(&img);
    img = aux;
  }
  
  t1     = iftTic();
  msskel   = iftMSSkel2D(img,A,IFT_INTERIOR, NULL, NULL);
  t2     = iftToc();
  fprintf(stdout,"skeletonization in %f ms\n",iftCompTime(t1,t2));

  saveSkeleton(msskel,img,"msskel.png");
  ...
  while (T!=0.0)
  {
    ...
    border = iftFindAndLabelObjectBorders(img, B);
    ...
    iftWriteImageByExt(skel_255,"skel.png");
