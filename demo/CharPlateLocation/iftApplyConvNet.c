...
int main(int argc, char **argv) {
	...
	char archIn[300];            strcpy(archIn , argv[1]);
	char archOut[300];           strcpy(archOut, argv[2]);
	char image_directory[300];   strcpy(image_directory, argv[3]);
	char extension[300];         strcpy(extension,argv[4]);
	int  output_format         = atoi(argv[5]);
	char output_name[300];       strcpy(output_name,argv[6]);
...
void validateInputs(char *archIn, char* archOut, char *image_directory, char* extension, int output_format, char* output_name) {
	...
	if (strcmp(archOut,"NULL") != 0) {
	  ...
	  char tmp[300];strcpy(tmp,archOut);
	...
	if (output_format == 0) {
	  if (!iftDirExists(output_name))
            iftCreateDirectory(output_name);
//	    iftError("Error: Output directory doesn't exist!!!", "validInputs");
	  ...
	  char tmp[300];strcpy(tmp,output_name);
	  if (!iftDirExists(dirname(tmp)) )  {
            strcpy(tmp,output_name);
            iftCreateDirectory(dirname(tmp));
