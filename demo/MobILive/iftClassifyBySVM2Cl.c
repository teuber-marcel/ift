...
void iftFalseRates(iftDataSet *Z,float* pfar,float* pfrr,float* pffs){
  ...
  for (i = 0; i < Z->nsamples; i++){
    if (Z->sample[i].status != IFT_TRAIN){
      ...
      if ((Z->sample[i].class == 1) && (Z->sample[i].label == 2)) nfalsefake++;
      if ((Z->sample[i].class == 2) && (Z->sample[i].label == 1)) nfalsereal++;
...
//  iftRandomSeed();
  ...
  for (i=0; i < n; i++) {
    ...
    if (atoi(argv[8])==0){
      ...
      if (Z1[2]->nclasses == 2) {
	char txt[200];
	sprintf(txt,"svm.%d.model",i);
	svm_save_model(txt,svm->model[0]);
      }
...
//    acc[i] = iftSkewedTruePositives(Z2[2]); // Compute accuracy on test set
  ...
  if (Z2[2]->nclasses == 2) {
    ...
    fprintf(stdout,"acc=%.3f(%.4f), far=%.3f(%.4f), frr=%.3f(%.4f), ffs=%.3f(%.4f)\n",
	    mean,stdev,meanfar,stdevfar,meanfrr,stdevfrr,meanffs,stdevffs);
    free(far);free(frr);free(ffs);
  } else {
    iftMeanStd(acc,n,&mean,&stdev);
    fprintf(stdout,"Accuracy of classification is mean=%.3f, stdev=%.4f\n",mean,stdev); 
  }
  free(acc);
