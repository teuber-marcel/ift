...
def main():
    ...
    """
    #Defines the size of the output regions
    window_size = 200.
    output_format = 'jpg'
    ...
    #for i,img_name in enumerate(['2008_001563']):
    for i,img_name in enumerate(image_list):
        ...
        for obj in obj_trees:
            ...
            resizing_factor = (window_size)/max(obj.shape[0:2])
            obj = transform.rescale(obj,resizing_factor,mode='nearest')

            output_obj = numpy.zeros((window_size,window_size,3))
            initial_coord = ( int((window_size - obj.shape[0])/2), int((window_size - obj.shape[1])/2) )
            final_coord = ( initial_coord[0] + obj.shape[0] , initial_coord[1] + obj.shape[1] )
            output_obj[initial_coord[0]:final_coord[0],initial_coord[1]:final_coord[1]] = obj

            #Replicates
            if min(obj.shape[0:2]) == 199:
                print('Passed')
            elif obj.shape[0] == 200:
                obj = numpy.tile(obj,(1,8,1))
                window_left = obj[:,-1-initial_coord[1]:]
                window_right = obj[:,0:(window_size - final_coord[1])]

                output_obj[:,0:initial_coord[1]+1] = window_left
                output_obj[:,final_coord[1]:] = window_right
            elif obj.shape[1] == 200:
                obj = numpy.tile(obj,(8,1,1))
                window_top = obj[-initial_coord[0]:,:]
                window_bottom = obj[0:(window_size - final_coord[0]),:]

                output_obj[0:initial_coord[0]:,:] = window_top
                output_obj[final_coord[0]:,:] = window_bottom
            else:
                raise Exception('Invalid cropping')
