...
//  iftImage        *img=NULL;
  ...
  if (argc != 8)
      iftError("Usage: iftDataClusterByDivideAndConquerOPF2Levels <dataset.zip> <nb_partitions> <nb_train_samples "
               "(0:FOR_USE_ALL_SAMPLES)> <kmax1(perc or val)> <kmax2(perc or val)> <propagation_type(0:root_label/1:cluster_majority)> <debug(0:NO/1:YES)--flag to output messages and generate images with tsne>",
               "main");
...
//  iftRandomSeed(IFT_RANDOM_SEED);
  ...
  t1 = iftTic();
  iftDivideAndConquerUnsupOPF2Levels(Z, atoi(argv[2]),atoi(argv[3]), iftNormalizedCut, atof(argv[4]), atof(argv[5]),
          debug);
  t2     = iftToc();
