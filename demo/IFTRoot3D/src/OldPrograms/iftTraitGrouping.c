...
ift3DRootData *iftCreate3DRootDataNew(int nroots, int ntraits)
{
  int i;
  ...
  sprintf(R->trait[0].name,"LongestRootLength(cm)");
  sprintf(R->trait[1].name,"TotalRootLength(cm)");
...
ift3DRootData *iftRead3DRootDataNew(char *basename)
{
  ift3DRootData *R=NULL;
  char       filename[200];
  FILE      *fp;
  ...
  sprintf(filename,"%s.csv",basename);
  if ((fp = fopen(filename,"r"))==0)
    iftError(MSG2,"iftRead3DRootDataNew");
  ...
  R = iftCreate3DRootDataNew(i-2,2);
...
void iftReadSelectedTraits(ift3DRootData *R, char *filename)
{
  int i;
  FILE *fp;
  ...
  token=strtok(buffer,",");
  for (i=0; i < R->ntraits; i++) {
    strncpy(R->trait[i].name,token,strlen(token));
    R->trait[i].name[strlen(token)+1]='\0';
    token=strtok(NULL,",");
  }
...
void iftWrite3DRootDataGroupsNew(ift3DRootData *R, char *basename)
{
  int i,j,s,ntraits=0;
  FILE *fp;
  char filename[200],*newbasename;
  ...
  sprintf(spopname[0],"TEJ");
  sprintf(spopname[1],"TRJ");
  sprintf(spopname[2],"AUS");
  sprintf(spopname[3],"IND");
...
int main(int argc, char *argv[]) 
{
  ...
  if (argc!=4)
    iftError("Usage must be: iftTraitGrouping <basename> <selectedtraits.csv> <maximum scale (0,1]","main");

  // Reading csv file
  
  t1     = iftTic();
  
  //  R = iftRead3DRootDataNew(argv[1]);
  R = iftRead3DRootData(argv[1]);
  iftReadSelectedTraits(R,argv[2]);
  Z = ift3DRootDataToDataSet(R);
  iftNormalizeFeatures(Z);
  iftSelectUnsupTrainSamples(Z,1.0);
  kmax = (int)(atof(argv[3])*Z->nsamples);
  ...
  iftDestroyDataSet(&Z);
  //  iftWrite3DRootDataGroupsNew(R,argv[1]);
