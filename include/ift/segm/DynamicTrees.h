...
static inline void iftInsertDynamicSet(iftDynamicSet *S,    iftMImage *mimg, int p)
{
    if (S->size) {
...
//! swig(newobject)
iftDTForest *iftCreateDTForest(   iftMImage *mimg,    iftAdjRel *A,    iftLabeledSet *seeds,
...
static inline double iftDistDynamicSetMImage(   iftDynamicSet *S,    iftMImage *mimg, int p)
{
    double dist = 0;
    for (int i = 0; i < S->dim; i++) {
...
static inline double iftCosineDistanceDynamicSetMImage(   iftDynamicSet *S,    iftMImage *mimg, int p)
{
    double inner_product = 0;
    double norm_mean = 0;
    double norm_p = 0;
    for (int i = 0; i < S->dim; i++) {
        inner_product += S->mean[i] * mimg->val[p][i];
        norm_mean     += S->mean[i] * S->mean[i];
        norm_p        += mimg->val[p][i] * mimg->val[p][i];
    }

    norm_mean = sqrt(norm_mean);
    norm_p    = sqrt(norm_p);

    return 1.0 - inner_product / (norm_mean * norm_p);
}
 ...
 * @param alpha         (optional) saliency weight, is set to 1.0 if objmap is not given
 * @return label mapping
 */iftImage *iftDynTreeRootWithCosineDistance(iftMImage *mimg, iftAdjRel *A, iftLabeledSet *seeds, int delta,
                                                float gamma, iftImage *objmap, float alpha);
...
//! swig(newobject)
iftFImage *iftDynTreeRootCostMap(iftMImage *mimg, iftAdjRel *A, iftLabeledSet *seeds);
