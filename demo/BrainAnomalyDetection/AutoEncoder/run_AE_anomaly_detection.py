...
RECONSTRUCTION_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/AutoEncoder/reconstruct_image.py"
SUPERVOXEL_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/AttentionMap/isf_on_attention_map.py"
FEAT_EXTRACTION_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/AttentionMap/extract_svoxel_histogram_feats.py"
OC_CLASSIFICATION_SCRIPT_PATH = "$NEWIFT_DIR/demo/BrainAnomalyDetection/AttentionMap/classify_svoxel_datasets_by_ocsvm.py"
...
def build_argparse():
    prog_desc = \
'''
Detect brain anomalies by AutoEncoder reconstruction errors and supervoxel classification.
'''
    ...
    parser.add_argument('-e', '--autoencoder', type=str, required=True,
    ...
    parser.add_argument('-x', '--training-reconstruction-error-set', type=str, required=True,
                        help='Directory or CSV with the pathnames from the training control reconstruction errors.')
    parser.add_argument('-o', '--out-classification-mask', type=str, required=True,
                        help='Output Label Image with the detected lesions.')
    parser.add_argument('-s', '--bias', type=str, help='Pathname from the bias (e.g. normal reconstruction error) used '
                                                       'to attenuate the resulting image reconstruction error')
    ...
    parser.add_argument('--skip-reconstruction-error-computation', action='store_true',
                        help='Skip the reconstruction error computation for the input image. '
                             'It only works if the reconstruction error is already saved in the auxiliary directory')
...
def print_args(args):
    ...
    print(f'- Training Set: {args.training_reconstruction_error_set}')
...
def main():
    ...
    rec_image_path = os.path.join(working_dir, "rec_image.nii.gz")
    rec_error_path = os.path.join(working_dir, "rec_errors.nii.gz")

    if not args.skip_reconstruction_error_computation:
        print("####### AE RECONSTRUCTION")
        reconstruction_cmd_args = f"--image {args.image} --autoencoder {args.autoencoder} " \
                                  f"--out-reconstructed-image {rec_image_path} " \
                                  f"--out-reconstruction-error {rec_error_path} " \
                                  f"--label-image {args.label_image}"
        if args.bias:
            reconstruction_cmd_args = f"{reconstruction_cmd_args} --bias {args.bias}"

        os.system(f"python {RECONSTRUCTION_SCRIPT_PATH} {reconstruction_cmd_args}")
    ...
    if not args.skip_supervoxel_segmentation:
        ...
        svoxel_cmd_args = f"--image {args.image} --attention-map {rec_error_path} " \
    ...
    if not args.skip_feat_extraction:
        ...
        print("\n\n####### Feature Extraction")
        feat_extract_cmd_args = f"--attention-map {rec_error_path} --supervoxels {svoxels_path} " \
                                f"--training-attention-map-set {args.training_reconstruction_error_set} " \
