...
namespace Interactive{
  ...
  BEGIN_EVENT_TABLE(InteractiveDialog, BaseDialog)
    EVT_SIZE    (       InteractiveDialog::OnSize)
  END_EVENT_TABLE()


  InteractiveDialog :: InteractiveDialog(wxWindow *parent,
					 ModuleInteractive *mod)
  : BaseDialog(parent, (char *)"Semi-automatic"){
    wxBitmap *bm[4];
    wxSize size(240, 380);
    wxColour wxcolor;
    ...
    live_xhandler = new LiveHandler('x', mod);
    live_yhandler = new LiveHandler('y', mod);
    live_zhandler = new LiveHandler('z', mod);
    ...
    Connect( id_but, wxEVT_COMMAND_BUTTON_CLICKED,
	     wxCommandEventHandler(InteractiveDialog::OnChangeMode),
	     NULL, NULL );
    Connect( id_res, wxEVT_COMMAND_BUTTON_CLICKED,
	     wxCommandEventHandler(InteractiveDialog::OnReset),
	     NULL, NULL );
    Connect( id_undo, wxEVT_COMMAND_BUTTON_CLICKED,
	     wxCommandEventHandler(InteractiveDialog::OnUndo),
	     NULL, NULL );
    Connect( id_run, wxEVT_COMMAND_BUTTON_CLICKED,
	     wxCommandEventHandler(InteractiveDialog::OnRun),
	     NULL, NULL );
    Connect( id_add, wxEVT_COMMAND_BUTTON_CLICKED,
	     wxCommandEventHandler(InteractiveDialog::OnAddObj),
	     NULL, NULL );
    //Connect( id_relax, wxEVT_COMMAND_BUTTON_CLICKED,
       //wxCommandEventHandler(InteractiveDialog::OnRelax),
       //NULL, NULL );
  }
  ...
  wxScrolledWindow *InteractiveDialog::CreateObjectPanel(){
    ...
    for(i=0; i<n; i++){
      ...
      Connect( id, wxEVT_COMMAND_BUTTON_CLICKED,
	       wxCommandEventHandler(InteractiveDialog::OnDelete),
	       NULL, NULL );

      id = v_but_color[i]->GetId();
      Connect( id, wxEVT_COMMAND_BUTTON_CLICKED,
	       wxCommandEventHandler(InteractiveDialog::OnChangeColor),
	       NULL, NULL );

      id = v_but_eye[i]->GetId();
      Connect( id, wxEVT_COMMAND_BUTTON_CLICKED,
	       wxCommandEventHandler(InteractiveDialog::OnChangeVisibility),
	       NULL, NULL );
  ...
  void InteractiveDialog::DestroyObjectPanel(){
    int i,n,id;
  ...
  void InteractiveDialog :: OnAddObj(wxCommandEvent& event){
    wxSize size(240, 380);
    int i,r,n,o;
    char msg[500];
  ...
  void InteractiveDialog :: OnDelete(wxCommandEvent& event){
    wxSize size(240, 380);
    char msg[1024];
    int i,n,id;
  ...
  void InteractiveDialog :: UnmarkAllForRemoval(){
    int Cmax,cod,o;
  ...
  void InteractiveDialog :: OnChangeColor(wxCommandEvent& event){
    int i,n,id,Cmax,cod;
  ...
  void InteractiveDialog :: OnChangeVisibility(wxCommandEvent& event){
    int i,n,id,Cmax,cod;
  ...
  void InteractiveDialog::OnChangeMode(wxCommandEvent& event){
    InteractiveDialog::ModeType mode = GetOperationMode();
    
    if(mode==InteractiveDialog::ADDMARKER){
    ...
    else if(mode==InteractiveDialog::DELMARKER){
    ...
    else if(mode==InteractiveDialog::LIVEMARKER){
  ...
  InteractiveDialog::ModeType InteractiveDialog::GetOperationMode(){
    int sel = but->GetSelection();

    if(sel==0)      return InteractiveDialog::NAVIGATOR;
    else if(sel==1) return InteractiveDialog::ADDMARKER;
    else if(sel==2) return InteractiveDialog::DELMARKER;
    else if(sel==3) return InteractiveDialog::LIVEMARKER;
    else            return InteractiveDialog::NAVIGATOR;
  }


  void InteractiveDialog::OnReset(wxCommandEvent& WXUNUSED(event)){
    mod->Reset();
  }


  void InteractiveDialog::OnRun(wxCommandEvent& WXUNUSED(event)){
    mod->Run();
  }


  void InteractiveDialog::OnUndo(wxCommandEvent& WXUNUSED(event)){
    //mod->Undo();
  }
  ...
  */
  ...
  AdjRel *InteractiveDialog :: GetBrush(){
    return bPicker->GetBrush();
  }

  wxCursor *InteractiveDialog :: GetBrushCursor(int zoom){
    return bPicker->GetBrushCursor(zoom);
  }

  void InteractiveDialog :: NextBrush(){
    bPicker->NextBrush();
  }

  void InteractiveDialog :: PrevBrush(){
    bPicker->PrevBrush();
  }


  void InteractiveDialog :: OnCancel(wxCommandEvent& event){
    mod->Stop();
    //BaseDialog::OnCancel(event);
  }

  void InteractiveDialog :: OnOk(wxCommandEvent& event){
    mod->Finish();
    //BaseDialog::OnOk(event);
  }


  void InteractiveDialog :: OnSize(wxSizeEvent& event){
    BaseDialog::OnSize(event);
  }

  void InteractiveDialog :: ChangeObjSelection(wxPanel *objbkg){
    wxPanel *bkg=NULL;
    wxColour wxcolor;
    int n,i;
