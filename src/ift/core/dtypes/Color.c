...
iftColorTable *iftCreateRandomColorTable(int n_colors, int normalization_value) {
  ...
  if (n_colors <= 0)
    ...
    for (int c = 0; c < n_colors; c++) {
      ctb->color[c].val[0] = iftRandomInteger(0,normalization_value);
      ctb->color[c].val[1] = iftRandomInteger(0,normalization_value);
      ctb->color[c].val[2] = iftRandomInteger(0,normalization_value);
      ctb->color[c]        = iftRGBtoYCbCr(ctb->color[c], normalization_value);
    }
...
iftColorTable *iftConvertYCbCrColorTableToRGBColorTable(  iftColorTable *ctb, int normalization_value) {
 ...
 **/
iftColor iftRGBtoYCbCrBT2020(iftColor cin,   int rgbBitDepth,   int yCbCrBitDepth)
{
 ...
 **/
iftColor iftYCbCrBT2020toRGB(iftColor cin,   int yCbCrBitDepth,   int rgbBitDepth)
{
    int minLum, minChr;
...
iftColorTable *iftCreateGradientColorTable(  iftStrArray *hexa_colors, int n_colors) {
