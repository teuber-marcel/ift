...
int main(int argc, const char *argv[]) {
    ...
    iftMoveElastixDefFields(df_symm_objs, iftJoinPathnames(3, out_dir, "SymmObjectDefFields", img_id));
    iftMoveElastixDefFields(inverse_df_symm_objs, iftJoinPathnames(3, out_dir, "InverseSymmObjectDefFields", img_id));
    iftWriteImageByExt(mov_flip_img_reg, iftJoinPathnames(3, out_dir, "MovingFlipRegisteredImage", iftCopyString("%s.nii.gz", img_id)));
    iftWriteImageByExt(fix_img, iftJoinPathnames(3, out_dir, "FixedImage", iftCopyString("%s.nii.gz", img_id)));
    iftWriteImageByExt(fix_mask, iftJoinPathnames(3, out_dir, "FixedMask", iftCopyString("%s.nii.gz", img_id)));
...
iftFileSet *iftRegisterSymmObjects(  iftImage *img,   iftImage *fix_mask,   iftImage *mov_flip_mask,
    ...
    *fix_img = iftMask(img, fix_mask);
    ...
    iftFileSet *df = NULL;
    iftImage *mov_flip_img_reg = iftRegisterImageByElastix(mov_flip_img, *fix_img, NULL, NULL, elastix_params, NULL, &df);
    ...
    *reg_mov_img = iftMask(mov_flip_img_reg_mask_hist, fix_mask);
    ...
    iftImage *fix_img_reg = iftRegisterImageByElastix(*fix_img, mov_flip_img, NULL, NULL, elastix_params, NULL, inverse_df_symm_objs);
