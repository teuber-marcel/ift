...
// for retinopathy - Red Lesions (RL) - 2nd test
int   vet_input_norm[8] = {3,3,3,3,0,0,0,0}; //
int   vet_k_bank    [8] = {3,5,7,9,3,5,7,9};
int   vet_nkernels  [4] = {16,32,64,128};
int   vet_pooling   [8] = {3,5,7,9,3,5,7,9}; //
int   vet_stride    [4] = {2,4,8,16};
float vet_alpha     [4] = {1.,2.,3.,10.};
int   vet_norm      [8] = {3,3,3,3,0,0,0,0}; //

//// for MobILive database - 1st submission
//int   vet_input_norm[8] = {3,5,7,9,11,13,0,0}; //
//int   vet_k_bank    [8] = {3,5,7,9,11,13,7,9};
//int   vet_nkernels  [4] = {32,64,128,256};
//int   vet_pooling   [8] = {3,5,7,9,11,13,3,5}; //
//int   vet_stride    [4] = {1,2,4,8};
//float vet_alpha     [4] = {1.,2.,3.,10.};
...
int iftVerifyImageDimensionsAlongNetwork(iftConvNetwork *convnet)
{
  ...
  if( (nbands[convnet->nstages-1] * xsize[convnet->nstages-1] * ysize[convnet->nstages-1] * zsize[convnet->nstages-1] <= 1000) || 
      (nbands[convnet->nstages-1] * xsize[convnet->nstages-1] * ysize[convnet->nstages-1] * zsize[convnet->nstages-1] >= 100000) ){
    ret = 0;
  }
...
int main(int argc, char **argv) {
  if (argc != 8)
    iftError("Please provide the following parameters:\n<random_seed> <xsize> <ysize> <nbands> <nlayers> <output_directory> <num_convnets>\n\n", "main");
  ...
  char outputfile[100],dirArchOut[100]; strcpy(dirArchOut, argv[6]);
  if (dirArchOut[strlen(dirArchOut)-1] == '/')
    dirArchOut[strlen(dirArchOut)-1] = '\0';
  int nconvnets = atoi(argv[7]);
  ...
  while(ncurconvnet < nconvnets ) {
    ...
    convnet->neg_activ_thres = -ACTIV_THRES_LIMIT;
    convnet->pos_activ_thres = 0.;

    iftCreateRandomKernelBanks(convnet);
    iftCreateAdjRelAlongNetwork(convnet);

    if (  !stop && (iftVerifyImageDimensionsAlongNetwork(convnet) ) && 
      ...
      iftWriteFloatValue(stdout,convnet->neg_activ_thres,"NEG_ACTIV_THRES");
      iftWriteFloatValue(stdout,convnet->pos_activ_thres,"POS_ACTIV_THRES");
      */
      hlayers[nlayers]++;
