...
/**
 * @brief Apply the Median Filter on an Input FImage.
 * @param  img Input Image to be filtered.
 * @param  Ain Input adjacency used in filtering.
 * @return     Filtered FImage.
 */
//! swig(newobject, stable)
iftFImage *iftFMedianFilter(iftFImage *fimg, iftAdjRel *Ain);
...
/**
 * @brief Computes the convolution between an array of multichannel images 
 * (i.e., an image batch) and a kernel bank, and then applies a given stride.
 * 
 * @param mimg: Input multiband image array
 * @param A   : Adjacency relation (shape of the kernels)
 * @param kernels: Kernel bank (matrix)
 * @param stride: Stride
 * 
...
/**
 * @brief Applies ReLU to a multichannel image.
 * 
 * @param mimg: Multichannel image
 * @param bias: Bias vector
 * @param inplace: boolean value for the inplace computation
 * 
...
/**
 * @brief Applies ReLU to an array of multichannel images.
 * 
 * @param mimg: Multichannel image array (batch)
 * @param bias: Bias vector
 * @param inplace: boolean value for the inplace computation
 * 
 * @return activation image  
 ...
 */
//! swig(newobject, stable)
  iftMImageArray *iftMReLUArray(iftMImageArray *mimg, float *bias,
				bool inplace);



/**
 * @brief Applies Pooling to a multichannel images.
 * 
 * @param mimg: Multichannel image
 * @param pool_type: pooling type (no_pool, avg_pool, or max_pool)
 * @param A: adjacency relation of the pooling operation
 * @param stride: stride >= 1
 * 
 * @return activation image after pooling  
 * 
...
/**
 * @brief Applies Pooling to a multichannel image array (batch).
 * 
 * @param mimg: Multichannel image array
 * @param pool_type: pooling type (no_pool, avg_pool, or max_pool)
 * @param A: adjacency relation of the pooling operation
 * @param stride: stride >= 1
 * 
 * @return activation image array after pooling  
 * 
 * @author Alexandre Falcao
 * @date Feb, 2024
 */
