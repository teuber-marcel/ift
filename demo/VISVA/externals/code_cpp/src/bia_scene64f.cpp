...
namespace bia{
  ...
  Scene64f *Create(int xsize,int ysize,int zsize){
    ...
    if(scn == NULL) 
      bia::Error((char *)MSG1,(char *)"Scene64f::Create");
  ...
  void     Destroy(Scene64f **scn){
    ...
    aux = *scn;
    if(aux != NULL){
      if(aux->data     != NULL) bia::FreeFltArray(&aux->data);
  ...
  void     Copy(Scene64f *dest, Scene64f *src){
    if(dest->xsize!=src->xsize ||
      ...
      bia::Error((char *)"Incompatible data",(char *)"Scene64f::Copy");
    dest->dx     = src->dx;
    dest->dy     = src->dy;
    dest->dz     = src->dz;
  ...
  void     Write(Scene64f *scn, char *filename){
    ...
    if(fp == NULL) 
      bia::Error((char *)MSG2,(char *)"Scene64f::Write");
